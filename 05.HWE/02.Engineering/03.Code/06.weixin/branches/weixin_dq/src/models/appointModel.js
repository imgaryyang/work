...
export default {
  ...
  effects: {
    ...
    *forScheduleList(obj, { select, call, put }) {
      ...
      if (refreshing) {
        ...
        const newCond = {
          ...
          endDate: moment().add(scheduleDateRange, 'days').format('YYYY-MM-DD'),
        };
    ...
    *forAppointSource({ payload }, { put }) {
      yield put(save({ isLoading: true }));

      const { appointSources: appointSourceData } = payload;
      if (!Array.isArray(appointSourceData)) {
        Toast.fail('号源数据格式错误', 3);
      } else if (appointSourceData.length > 0) {
        // const appointSourceData = appointSources.map((item) => { return { ...item, clinicTime: item.clinicTime.slice(11, 16) }; });
        yield put(save({ appointSourceData }));
      }
    ...
    *forAppointRecords({ payload }, { select, call, put }) {
      ...
      const { data: hasCardData } = yield call(forReservedList, { ...payload, hosId, hosNo, startDate });
      const { data: noCardData } = yield call(forReservedNoCardList, { ...payload, hosId, hosNo, startDate });
    ...
    *forCancel({ payload }, { select, call }) {
      const { currHospital: { id: hosId, no: hosNo } } = yield select(model => model.base);
      const { data } = yield call(forCancel, { ...payload, hosId, hosNo, hisUser: Global.hisUser });
      if (data && data.success) {
        return true;
      } else if (data && !data.success) {
    ...
    *forSign({ payload }, { select, call }) {
      const { currHospital: { id: hosId, no: hosNo } } = yield select(model => model.base);
      const { data } = yield call(forSign, { ...payload, hosId, hosNo, hisUser: Global.hisUser });
      if (data && data.success) {
        return true;
      } else if (data && !data.success) {
        return Promise.reject(data.msg || '未知错误');
