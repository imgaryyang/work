...
export default {
  ...
  effects: {
    ...
    *loginByOpenId({ payload }, { call, put }) {
      ...
      const { openid, route, funcIdx } = payload;
      // 取二级菜单
      let subfuncs = {};
      if (funcIdx >= 0) {
        const func = Global.Config.services.hfc[funcIdx];
        if (func.route === 'subFuncs') {
          subfuncs = {
            subFuncsTitle: func.name,
            subFuncs: func.children,
          };
        }
      }

      yield put({ type: 'save', payload: { route, funcIdx, ...subfuncs } });
    ...
    *loginByUserId({ payload }, { call, put }) {  // eslint-disable-line
      Toast.loading('正在登录，请稍候...');
      const { userId, route, funcIdx } = payload;
      // 取二级菜单
      let subfuncs = {};
      if (funcIdx >= 0) {
        const func = Global.Config.services.hfc[funcIdx];
    ...
    *loadAds(arg, { call, put }) {
      const { data } = yield call(ads);
      if (data && data.success) {
        const { result } = data;
        yield put({
          type: 'setState',
