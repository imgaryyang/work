...
import com.lenovohit.hwe.pay.support.wxpay.common.report.ReporterFactory;
import com.lenovohit.hwe.pay.support.wxpay.common.report.protocol.ReportReqData;
import com.lenovohit.hwe.pay.support.wxpay.common.report.service.ReportService;
import com.lenovohit.hwe.pay.support.wxpay.protocol.pay_protocol.ScanPayReqData;
import com.lenovohit.hwe.pay.support.wxpay.protocol.pay_protocol.ScanPayResData;
...
import com.lenovohit.hwe.pay.support.wxpay.protocol.reverse_protocol.ReverseReqData;
import com.lenovohit.hwe.pay.support.wxpay.protocol.reverse_protocol.ReverseResData;
import com.lenovohit.hwe.pay.support.wxpay.service.ReverseService;
import com.lenovohit.hwe.pay.support.wxpay.service.ScanPayQueryService;
import com.lenovohit.hwe.pay.support.wxpay.service.ScanPayService;
...
public class ScanPayBusiness {
    ...
    public void run(ScanPayReqData scanPayReqData, ResultListener resultListener) throws Exception {
        ...
        scanPayResData.setResponseString(payServiceResponseString);
        scanPayResData.setConfigs(scanPayReqData.getConfigs());
        //异步发送统计请求
        //*
        ...
        ReportReqData reportReqData = new ReportReqData(
                ...
                scanPayReqData.getConfigs()
        );
        long timeAfterReport;
        if (scanPayReqData.getConfigs().getBoolean("useThreadToDoReport")) {
        ...
        if (scanPayResData.getReturn_code().equals("FAIL")) {
            ...
            //--------------------------------------------------------------------
            String key = scanPayReqData.getConfigs().getString("key");
            if (!Signature.checkIsSignValidFromResponseString(key, payServiceResponseString)) {
            ...
            if (scanPayResData.getResult_code().equals("SUCCESS")) {
                ...
                if (errorCode.equals("AUTHCODEEXPIRE") || errorCode.equals("AUTH_CODE_INVALID") || errorCode.equals("NOTENOUGH")) {
                    ...
                    if (doPayQueryLoop(payQueryLoopInvokedCount, outTradeNo, scanPayReqData.getConfigs())) {
