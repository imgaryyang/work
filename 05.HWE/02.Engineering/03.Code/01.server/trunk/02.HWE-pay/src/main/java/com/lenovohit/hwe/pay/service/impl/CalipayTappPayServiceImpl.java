...
public class CalipayTappPayServiceImpl implements PayBaseService {
	...
	public void prePay(Settlement settlement) {
        ...
        AlipayTradeAppPayRequestBuilder builder = new AlipayTradeAppPayRequestBuilder()
            .setSubject(settlement.getSettleTitle()).setOutTradeNo(settlement.getSettleNo())
            .setTotalAmount(settlement.getAmt().toString()).setBody(settlement.getSettleDesc())
            .setTimeoutExpress(config.getString("trade_out_time")/*Configs.getTradeOutTime()*/)
            .setPassbackParams(settlement.getId()).setProductCode("QUICK_MSECURITY_PAY")
        	...
        	.setConfigs(config);
        AlipayTradeAppPayResult result = Alipay.tradeAppPay(builder);
        AlipayTradeAppPayResponse response = result.getResponse();
        switch (result.getTradeStatus()) {
            ...
            case UNKNOWN:
	    		...
	    		settlement.setTradeRspMsg(response.getMsg());
	    		settlement.setRespText(response.getBody());
                log.error("支付宝App预下单系统异常，预下单状态未知，交易关闭!!!");
                break;
        }
