...
//			bill.setOperation(operation);
	...
	private void initRefundSettle(Settlement settle) throws PayException{
		try {
	        ...
	        Bill bill = this.billManager.get(settle.getBillId());
	        if(!StringUtils.isEmpty(settle.getOriBillId())){
	        	Settlement oriSettle = this.settlementManager.findOne("from Settlement where billId = ? ", settle.getOriBillId());
...
//	        settle.setprintBatchNo
	...
	class TradeCallback extends Thread{
		...
		public TradeCallback(Bill bill, Settlement settle){
			this.bill = bill;
			this.settle = settle;
		}
        ...
        @Override
        public void run() {
//        	TradeCallbackService tcService = (TradeCallbackService) SpringUtils.getBean(bill.getBizBean());
        	Map<String, Object> tradeModel = new HashMap<String, Object>();
        	tradeModel.put("bizType", bill.getBizType());
        	tradeModel.put("bizNo", bill.getBizNo());
        	tradeModel.put("amt", bill.getAmt());
        	tradeModel.put("realAmt", bill.getRealAmt());
        	tradeModel.put("status", bill.getStatus());
        	tradeModel.put("tranTime", bill.getTranTime());
...
//        	tcService.callback(tradeModel);
        	...
        	restTemplate.postForEntity(bill.getBizUrl(), tradeModel, String.class);
        }
    }
