...
public class HisPatientManagerImpl implements HisPatientManager{
	...
	/**
	 * 根据病人信息所有病人档案- 患者卡信息(PATIENT044)
	 * @param param
	 * @return
	 */
	public HisListResponse<Patient> getPatients(Patient param){
		Map<String,Object> reqMap = new HashMap<String, Object>();
        //入参字段映射
    	putNullToMap(reqMap, "IDCard", param.getIdNo());
    	putNullToMap(reqMap, "CardNo", param.getMiCardNo());
    	
    	RestListResponse response = hisRestDao.postForList("PATIENT0044", RestRequest.SEND_TYPE_LOCATION, reqMap);
		...
		if(response.isSuccess() && null != resMaplist){
			for(Map<String, Object> resMap : resMaplist){
				...
				Object balance = resMap.get("BALANCE");
				String balanceStr = (balance==null || "".equals(balance))?"0.0":balance.toString();
				patient.setBalance(new BigDecimal(balanceStr));
	...
	public HisEntityResponse<Patient> createProfile(Patient param){
		...
		putNullToMap(reqMap, "birthday", param.getBirthday());
