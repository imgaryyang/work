...
public class UnionPayManagerImpl extends UnionPayManager {
...
//			List<CheckDetailHis> cdhl = new ArrayList<CheckDetailHis>();
			...
			for(CheckDetailBank cdb : cdbl){
				...
				result.setTradeType("S22".equals(cdb.getTradeType())?Settlement.SETTLE_TYPE_PAY:Settlement.SETTLE_TYPE_REVERSE);
				result.setTradeDate(DateUtils.date2String(DateUtils.string2Date(cdb.getTradeDate(), "yyyyMMdd"), "yyyy-MM-dd"));
				result.setTradeTime(DateUtils.date2String(DateUtils.string2Date(cdb.getTradeTime(), "HHmmss"), "HH:mm:ss"));
				result.setClearDate(DateUtils.date2String(DateUtils.string2Date(cdb.getClearDate(), "yyyyMMdd"), "yyyy-MM-dd"));
				//1.自助机结算单数据
				settlement = settlementManager.findOne("from Settlement where tradeNo = ? and settleType = 'SP' and to_char(tradeTime,'yyyy-MM-dd')=?", cdb.getTradeNo(), result.getTradeDate());
				if(Settlement.SETTLE_TYPE_REVERSE.equals(result.getTradeType()) || null == settlement){
				...
				//2.HIS银行交易明细数据
				detail = checkDetailHisManager.findOne("from CheckDetailHis where ztbz = '1' and jylx = 'HB01' and jsckh = ? and yhrq=?", cdb.getTradeNo(), result.getTradeDate().replaceAll("-", ""));
				if(Settlement.SETTLE_TYPE_REVERSE.equals(result.getTradeType()) || null == detail){
...
//		清分日期,交易日期,交易时间,商户号,终端号,交易金额,清分金额,流水号,交易类型,交易卡号,发卡银行,卡类型
	...
	private boolean downloadFile(CheckRecord checkRecord) {
		...
		try {
			...
			if( ftp.downloadFile(checkConfig[5], checkFile, checkRecord.getFilePath() + checkFile) > 0){
