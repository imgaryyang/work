...
class HotConsult extends Component {
  static displayName = 'HotConsult';
  static description = '热门咨询';
  ...
  renderImage(data) {
    return (
      <View style={styles.imageBg}>
        <Image source={{ uri: `${Global.getImageHost()}${data.item.fileName}` }} style={{ width: 57, height: 57 }} />
      </View>
    );
  }
  ...
  render() {
    if (!this.state.doRenderScene) {
      return HotConsult.renderPlaceholderView();
    }
...
export default connect(mapStateToProps, mapDispatchToProps)(HotConsult);
