...
class EasyFieldFrame extends Component {
  ...
  _getLabel() {
    ...
    const { width, height, backgroundColor, ...other } = this._labelConfig.style;

    const widthStyle = position === 'left' ? { width: this._labelConfig.width } : (width ? { width } : null);
    const heightStyle = position === 'left' ? { height: this._inputInnerHeight } : (height ? { height } : null);
   ...
   */
  _getClearIcon() {
    const clearIcon = Platform.OS === 'android' && this.props.showClearIcon && this.props._value ?
      (
   ...
   */
  _getButton() {
    const fontSize = this._textInputConfig.button.fontSize ? { fontSize: this._textInputConfig.button.fontSize } : null;
    let color = this._textInputConfig.button.color ? { color: this._textInputConfig.button.color } : null;
    const width = this._textInputConfig.button.width ? { width: this._textInputConfig.button.width } : null;
    const bgColor = this._textInputConfig.button.bgColor ? { backgroundColor: this._textInputConfig.button.bgColor } : null;
    ...
    const text = this.props.buttonDisabled && this.props.buttonDisabledText ? this.props.buttonDisabledText : this.props.buttonText;
  ...
  _getAdjustButton(type) {
    // const color = this._textInputConfig.adjustButton.color ? { color: this._textInputConfig.adjustButton.color } : null;
    const bgColor = this._textInputConfig.adjustButton.bgColor ? { backgroundColor: this._textInputConfig.adjustButton.bgColor } : null;

    const icon = type === 'plus' ?
    ...
    const abs = this.props.showAdjustButton ? (
      <View
        style={[
   ...
   */
  _getError() {
    if (!this.props._error) { return null; }

    const errMsg = this.props._error;
    return (
   ...
   */
  render() {
    const { fontSize, color, fontWeight, lineHeight, textAlign, ...other } = this._fieldContainerStyle;

    let position = this._labelConfig.position;
    position = typeof this.props.labelPosition === 'string' ? this.props.labelPosition : position;

    const topLabel = position === 'top' ? this._getLabel() : null;
    const leftLabel = position === 'left' ? this._getLabel() : null;

    const leftIcon = this._textInputConfig.icon.position === 'left' ? this._getIcon() : null;
    const rightIcon = this._textInputConfig.icon.position === 'right' ? this._getIcon() : null;
