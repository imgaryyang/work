...
class Records extends Component {
  ...
  async fetchData(currHospital, currPatient, currProfile) {
    try {
      ...
      const query = { ...currProfile, proNo: currProfile.no, startDate, endDate };
      const responseData = await list(query);
      ...
      >
        <View style={{ flex: 1 }} >
          <View style={{ flexDirection: 'row' }}>
            <Text style={styles.date}>{moment(item.treatStart).format('YYYY-MM-DD HH:mm')}</Text>
