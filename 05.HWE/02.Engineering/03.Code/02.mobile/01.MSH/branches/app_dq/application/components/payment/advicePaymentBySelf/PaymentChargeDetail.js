...
class PaymentChargeDetail extends Component {
  ...
  static getSections(list) {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      for (let j = 0; j < datas.length; j++) {
        if (datas[j][0].groupNo === list[i].groupNo) {
          flag = 1;
          az = j;
          break;
        }
      ...
      if (flag === 1) {
        ...
        wdy.push(list[i]);
        datas.push(wdy);
        sections.push({ key: `${list[i].groupNo}`, value: `处方 ${list[i].groupNo}`, data: wdy, department: list[i].department, doctor: list[i].doctor });
      }
    }
    return sections;
  }
  ...
  async prePay() {
    ...
    for (let idx = 0; idx < data.length; idx += 1) {
      if (selectRecordIds.indexOf(data[idx].groupNo) > -1) {
  ...
  renderItem({ item, index }) {
    const checkbox = index === 0 ? (<Checkbox
      checked={_.indexOf(this.state.selectRecordIds, item.groupNo) !== -1}
      size={23}
    />) : null;
    return (
      <TouchableOpacity
        style={{ flex: 1, flexDirection: 'row', marginBottom: 2, paddingLeft: 20, paddingRight: 20 }}
        onPress={() => this.onSelect(item.groupNo)}
      >
        <View style={{ width: 30 }}>
          { checkbox }
        </View>
  ...
  renderSectionComp() {
    return (
      <View>
        <View style={{ flexDirection: 'row', paddingLeft: 20, paddingRight: 20, paddingBottom: 3, paddingTop: 8, alignItems: 'center' }} >
  ...
  render() {
    ...
    const content = currProfile.type !== '1' ? (
      ...
      <ScrollView style={styles.scrollView}>
        ...
        <View style={{ flexDirection: 'row', margin: 15, marginTop: 30, marginBottom: 40 }} >
          <Button text="自费结算" onPress={this.prePay} disabled={currProfile.type !== '1'} theme={Button.THEME.ORANGE} screenProps={this.props.screenProps} />
        </View>
      </ScrollView>
    );
    return (
