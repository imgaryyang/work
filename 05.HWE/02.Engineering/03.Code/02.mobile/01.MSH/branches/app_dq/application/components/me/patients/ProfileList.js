...
class ProfileList extends Component {
  ...
  async bind() {
    ...
    if (this.form.validate()) {
      try {
        const { value, hospital } = this.state;
   ...
   */
  renderItems() {
    const { profiles, allowBind, showPatientInfo, base } = this.props;
    return profiles.map((item, idx) => {
      const { id, hosName, no, name, idNo, mobile } = item;
      ...
      // ) : null;
      // console.log('this.bindedCards:', this.bindedCards);
      ...
      return (
        <Card radius={6} fullWidth={false} style={styles.cardContainer} key={`card_${id}_${idx + 1}`}>
