...
class Item extends PureComponent {
  ...
  render() {
    ...
    const { image, createdAt, caption } = this.props.data;
    const img = image ? (
      <Portrait
        width={Global.screen.width - 30}
        height={(Global.screen.width - 30) * (177 / 340)}
        imageSource={{ uri: `${Global.getImageHost()}${image}?timestamp=${new Date().getTime()}` }}
        bgColor={Global.colors.IOS_BG}
      />
    ) : null;
    const containerStyle = { marginLeft: 15, marginRight: 15, overflow: 'hidden' };
    const marginTop = index === 0 ? { marginTop: 15 } : null;
    return (
      <Card radius={6} noPadding style={{ ...containerStyle, ...marginTop }} >
        <TouchableOpacity
          onPress={this.onPress}
        >
          {img}
          <View style={{ flex: 1, margin: 15 }} >
            <Text style={{ fontSize: 12, color: Global.colors.FONT_LIGHT_GRAY1, marginBottom: 5 }}>{moment(createdAt).format('YYYY-MM-DD HH:MM')}</Text>
            <Text style={{ fontSize: 15, color: Global.colors.FONT }}>{caption}</Text>
          </View>
        </TouchableOpacity>
      </Card>
    );
