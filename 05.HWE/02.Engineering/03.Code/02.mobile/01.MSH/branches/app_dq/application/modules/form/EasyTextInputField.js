...
class EasyTextInputField extends Component {
  ...
  componentWillReceiveProps(props) {
    if (props['value']) { this.setState({ value: props['value'] }); } else { this.setState({ value: null }); }
  }
   ...
   */
  onChangeText(value) {
    if (typeof this.props.onChange === 'function') { this.props.onChange(this.props.name, `${value}`); }
  }
  ...
  _clear() {
    ...
    if (typeof this.props.onChange === 'function') { this.props.onChange(this.props.name, ''); }
  }

  _adjust(type) {
    try {
      let v = this.state.value ? parseInt(this.state.value, 10) : 0;

      if (type === 'plus') v += 1;
      else if (type === 'minus') v -= 1;

      this.setState({ value: `${v}` }, () => {
        if (typeof this.props.onChange === 'function') { this.props.onChange(this.props.name, `${v}`); }
      });
    } catch (e) {
      console.log(e);
    }
  ...
  _getTextInput(_fontSize, _color, _fontWeight, _lineHeight, _textAlign) {
    ...
    const {
      config, name, value, dataType, label, help, error,
      iconLib, icon, showClearIcon, textAlign, buttonText, buttonOnPress,
      showAdjustButton, leftComponent, rightComponent, onChange,
      registerChild, required, minLength, style, editable, ...otherBaseProps
    } = this.props;

    const editableStyle = editable === false ? { color: Global.colors.FONT_LIGHT_GRAY1 } : {};
    // console.log(editable, editableStyle);
    const { ...otherProps } = this._getPropsByDataType();
    ...
    return (
      <TextInput
        ...
        style={[styles.textInput, _fontSize, _color, _fontWeight, _lineHeight, _textAlign, editableStyle]}
  ...
  /**
     ...
     */
  render() {
    const { fontSize, color, fontWeight, lineHeight, textAlign } = this._inputContainerStyle;
