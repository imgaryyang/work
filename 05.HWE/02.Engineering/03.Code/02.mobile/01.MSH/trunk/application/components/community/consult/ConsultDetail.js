...
class ConsultDetail extends Component {
  ...
  render() {
    if (!this.state.doRenderScene) {
      return ConsultDetail.renderPlaceholderView();
    }

    let type = '';
    if (this.state.value.consultType === '1') {
      type = '咨询一';
    } else if (this.state.value.consultType === '2') {
      type = '咨询二';
      ...
      <View style={[Global.styles.CONTAINER, { backgroundColor: Global.colors.IOS_GRAY_BG }]}>
        <ScrollView>
          <EasyCard radius={8} style={{ marginTop: 6 }}>
            ...
            <View style={{ flexDirection: 'row', paddingTop: 3 }}>
              <Text style={{ fontSize: 14, color: '#2C3742' }}>回复：</Text>
              <Text style={{ marginLeft: 8, fontSize: 14, alignItems: 'center', width: Global.getScreen().width - 65 }}>{ this.state.value.reply ? this.state.value.reply.sendContent : '暂无回复'}</Text>
            </View>
          </EasyCard>
        </ScrollView>
      </View>
...
export default connect(mapStateToProps)(ConsultDetail);
