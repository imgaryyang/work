...
import { hisTestDetail } from '../../services/records/RecordService';
    ...
    return (
      <View style={styles.renderRow} >
        <View style={styles.rowName}>
          ...
          {detail.reference !== null ? (<Text style={styles.reference}>参考：{detail.reference || ''}</Text>) : null}
        </View>
        <View style={{ flex: 2 }}>
...
class ShowPacsDetail extends Component {
  ...
  async loadCheckDetail() {
    try {
      ...
      const responseData = await hisTestDetail(query);
  ...
  render() {
    ...
    if (!this.state.doRenderScene) {
      return ShowPacsDetail.renderPlaceholderView();
    }
    const LisDesc = config.LISdesc;
    const desc = LisDesc[name];
    return (
