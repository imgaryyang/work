...
import { createForegift } from '../../../services/payment/ChargeService';
import { patientPayment } from '../../../services/RequestTypes';
import config from '../../../../Config';
import { getPrePay } from '../../../services/payment/AliPayService';


const dismissKeyboard = require('dismissKeyboard');

class InpatientOnlineRecharge extends Component {
  static displayName = 'InpatientOnlineRecharge';
  static description = '微信/支付宝住院预缴';
  ...
  constructor(props) {
    ...
    this.createForegift = this.createForegift.bind(this);
  ...
  */
  ...
  async createForegift(param) {
    const { user } = this.props.auth;
    const bizUrl = patientPayment().foregiftCallback;
    try {
      this.props.screenProps.showLoading(true);
      const responseData = await createForegift(param);
...
export default connect(mapStateToProps)(InpatientOnlineRecharge);
