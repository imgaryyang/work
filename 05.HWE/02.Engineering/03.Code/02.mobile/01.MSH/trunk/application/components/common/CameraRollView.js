...
class CameraRollView extends Component {
  ...
  state = {
    ...
    doRenderScene: false,
    maxCount: this.props.navigation.state.params.maxCount ? this.props.navigation.state.params.maxCount : 1,
    type: this.props.navigation.state.params.type ? this.props.navigation.state.params : 'single',
  ...
  onSelect(photo, rowID, index) {
    ...
    if (this.state.selectedPhotos.length < this.state.maxCount) {
    ...
    /* if (this.state.selectedPhotos.length === 0 && index === 1) {
      ...
      for (let i = 0; i < this.state.assets.length; i++) {
        if (CameraRollView.contains(this.state.selectedPhotos, this.state.assets[i].node.image)) {
          this.state.assets.splice(
            i, 1,
            {
              node:
                {
                  timestamp: this.state.assets[i].node.timestamp,
                  group_name: this.state.assets[i].node.group_name,
                  type: this.state.assets[i].node.type,
                  selected: false,
                  image: this.state.assets[i].node.image,
                },
            },
          );
        }
  ...
  confirm() {
    if (this.state.selectedPhotos.length === 0) { this.toast('您尚未选择照片'); } else {
      const photos = this.state.selectedPhotos;
      // 回调列表更新数据
  ...
  renderFooterSpinner() {
    if (this.state.more) {
      return (<View style={[Global.styles.CENTER, { width: Global.getScreen().width, height: 80 }]}><Text style={{ color: Global.colors.FONT_GRAY }} >正在载入，请稍候...</Text></View>);
    }
    return null;// (<View style={[Global.styles.CENTER, { width: Global.getScreen().width, height: 80 }]}><Text>没有照片了！</Text></View>);
  }
  ...
  renderImage(asset, rowID) {
    ...
    if (CameraRollView.contains(this.state.selectedPhotos, asset.node.image)) {
      return (
        <TouchableOpacity key={`${getRandomNum(32)}`} style={[Global.styles.CENTER, styles.imageHolder]} onPress={() => this.onSelect(asset, rowID, 2)} >
          <Image
      ...
      return (
        <TouchableOpacity key={`${getRandomNum(32)}`} style={[Global.styles.CENTER, styles.imageHolder]} onPress={() => this.onSelect(asset, rowID, 1)} >
          <Image
            key={asset.node.timestamp}
            source={asset.node.image}
            style={styles.image}
  ...
  render() {
    ...
    const buttonState = this.state.selectedPhotos.length === 0;
    return (
...
const styles = StyleSheet.create({
  ...
  image: {
    width: ((Global.getScreen().width - 8) / 4),
    height: ((Global.getScreen().width - 8) / 4),
    margin: 1,
  },
  info: {
    flex: 1,
