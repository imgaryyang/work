    ...
    return (
      ...
      >
        <EasyCard hideBottomBorder hideTopBorder fullWidth style={{ height: 80, backgroundColor: '#FFFFFFFF', flexDirection: 'column' }}>
          ...
          <View>
            <Text style={{ fontSize: 16, width: Global.getScreen().width - 65 }}>{item.consultDetail.length > 20 ? `${item.consultDetail.substring(0, 20)}...` : item.consultDetail} </Text>
          </View>
        </EasyCard>
      </TouchableOpacity>
    );
...
class CurrentConsult extends Component {
  static displayName = 'CurrentConsult';
  static description = '当前咨询';
  ...
  gotoReply(item, index) {
    ...
    const newItem = {};
    newItem.consultType = item.consultType;
    newItem.consultDetail = item.consultDetail;
    newItem.createdBy = item.createdBy;
    newItem.id = item.id;
    newItem.status = item.status;
    newItem.reply = item.replyList && item.replyList.length > 0 ? item.replyList[0] : '';
  ...
  render() {
    if (!this.state.doRenderScene) {
      return CurrentConsult.renderPlaceholderView();
    }
...
export default connect(mapStateToProps)(CurrentConsult);
