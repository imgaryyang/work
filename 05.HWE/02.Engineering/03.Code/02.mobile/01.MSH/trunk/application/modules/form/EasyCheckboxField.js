...
class EasyCheckboxField extends Component {
  ...
  _generateValue(value) {
    ...
    if (this.props.type === 'single') {
      if (value === this.state.value && !this.props.notNull) { return null; }
      return value;
    } else {
      if (this.state.value === null) { return value; }
      let v = `;${this.state.value};`;
      if (v.indexOf(`;${value};`) === -1) { return `${this.state.value};${value}`; } else {
        v = v.replace(`;${value};`, ';');
        if (v === ';') { return null; }
  ...
  _getCheckboxItems() {
    return this.props.dataSource.map((item, idx) => {
      const checked = this._optionSelected(item.value);
      ...
      return (
        <TouchableOpacity
          key={`${this.props.name}_cb_${idx + 1}`}
          onPress={() => {
            // 如果选择域被禁用，则禁用点击事件
   ...
   */
  _getCheckboxGroup() {
    const fd = this.props.display === 'row' ? { flexDirection: 'row' } : null;
    return (
