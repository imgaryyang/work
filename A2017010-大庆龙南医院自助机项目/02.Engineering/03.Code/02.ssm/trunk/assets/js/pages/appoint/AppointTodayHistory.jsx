...
class AppointHistory extends TimerModule {
  ...
  constructor(props) {
    ...
    this.loadHistory = this.bind(this.loadHistory,this);
    this.renderItems    = this.bind(this.renderItems,this);
    
    this.showSignConfirm  = this.bind(this.showSignConfirm,this);
    this.closeSignConfirm  = this.bind(this.closeSignConfirm,this);
    
    this.showCancelConfirm  = this.bind(this.showCancelConfirm,this);
    this.closeCancelConfirm  = this.bind(this.closeCancelConfirm,this);
    
    this.showConfirm = this.bind(this.showConfirm,this);
    
    this.doCancel  = this.bind(this.doCancel,this);
    this.requestCancel = this.bind(this.requestCancel,this);
    this.doSign  = this.bind(this.doSign,this);
    this.requestSign = this.bind(this.requestSign,this);
  ...
  onBack(){
	baseUtil.goHome('appointTodayBack'); 
  }
  onHome(){
	baseUtil.goHome('appointTodayHome'); 
  }
  loadHistory(){
...
//	  }
  ...
  render() {
    ...
    history.map (function(row,index){
    	...
    	if(isToday)todyHistory.push(row); 
    });
    
    return (
      (!todyHistory || todyHistory.length <= 0 ) ? ( 
      ...
      <NavContainer  title='预约记录' onBack={this.onBack} onHome={this.onHome} >
          ...
          <Card radius = {false} className = 'appoint_his_rows' >
            {this.renderItems(todyHistory)}
          </Card>
          {
        	todyHistory.length > pageSize ? (
	        	<Row style = {{padding : '1.5rem'}} >
		            <Col span = {8}><Button text = "上一页" disabled={start== 0} onClick = {this.prePage} /></Col>
		            <Col span = {8}>&nbsp;</Col>
		            <Col span = {8} ><Button text = "下一页" disabled={limit >= todyHistory.length } onClick = {this.nextPage} /></Col>
	            </Row>
            ):null
          }
