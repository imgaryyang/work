...
import styles               from './PrescriptionHistory.css';
...
class PrescriptionHistory extends TimerModule {
  ...
  constructor(props) {
    ...
    this.getPrescriptionType = this.bind(this.getPrescriptionType,this);
  ...
  componentDidMount () {
	let patient = baseUtil.getCurrentPatient();
  ...
  loadDetail(row){
	  ...
	  let fetch = Ajax.get("/api/ssm/treat/prescriptionRecord/info", row, {catch: 3600});
	  fetch.then(res => {
		if(!(res && res.success && res.result )){
	  		baseUtil.error("无法获取报告内容，请至分诊台人工办理打印业务！");
	  		return;
		}
		try{
			...
			console.info(e);
			baseUtil.error("处方格式错误，请至分诊台人工办理打印业务！");
  		}
	  }).catch((e)=>{
		  baseUtil.error("无法获取报告内容，请至分诊台人工办理打印业务！");
	  });
  ...
  async print (result) {
	try{
		printUtil.printPrescriptionRecord(result);
	...
	let fetch = Ajax.put("/api/ssm/treat/prescriptionRecord/print/"+record.prescriptionNo, {}, {catch: 3600});
	fetch.then(res => {
	  if(res && res.success ){
    	...
    	this.setState({record:null},()=>{
    		baseUtil.warning("无法获取处方内容，请至分诊台人工办理打印业务！");
		});
    });
  }
  nextPage(){
  ...
  onSelectDay(day){
	  let date = day ? moment(day).format('YYYY-MM-DD') : null;
	  let currentDate = moment(new Date()).format('YYYY-MM-DD');
	  if(date === currentDate){//只能打印当天
  ...
  doSearch(){
	  ...
	  let fetch = Ajax.get("/api/ssm/treat/prescriptionRecord/list", query, {catch: 3600});
	  fetch.then(res => {
  ...
  render() {
    ...
    return (
      <NavContainer title='门诊处方' onBack={this.onBack} onHome={this.onHome} >
	      <Row style = {{ paddingLeft: '1.5rem',paddingRight:'1.5rem'}}>
		    <Col span = {3} className = 'prescription_toolBarItem' style = {{textAlign: 'left',paddingRight: '.5rem'}} >开始日期</Col>
			<Col span = {6} className = 'prescription_toolBarItem' style = {{paddingRight: '.5rem'}} >
			  	<Card  onClick = {()=>{this.showDatePicker('startTime')}} ><img src = {calendarIcon} />{startTime}</Card>
			</Col>
			
			<Col span = {6} className = 'prescription_toolBarItem' style = {{paddingRight: '.5rem'}} ></Col>
			
			<Col span = {3} className = 'prescription_toolBarItem' style = {{textAlign: 'left',paddingRight: '.5rem'}} >结束日期</Col>
			<Col span = {6} className = 'prescription_toolBarItem' style = {{paddingRight: '.5rem'}} >
				<Card  onClick = {()=>{this.showDatePicker('endTime')}} ><img src = {calendarIcon} />{endTime}</Card>
			</Col>
		  </Row>
          <Row>
            <Col span = {6} className = 'list_title' >处方日期</Col>
            <Col span = {6} className = 'list_title' >开单科室</Col>
            <Col span = {4} className = 'list_title' >开单医生</Col>
            <Col span = {4} className = 'list_title' >处方总金额</Col>
            <Col span = {4} className = 'list_title list_center' >操作</Col>
          </Row>
          <Card  radius = {false} className = 'prescription_rows' >
            {this.renderItems()}
          </Card>
          {
  ...
  renderItems () {
	  ...
	  return filteredRecords.map ( (row, idx) => {
        const { pharmacyWindow,date,prescriptionNo,department,patientNo,unit,patientName,gender,
        	age,costType,address,mobilPhone,clinicalDiagnosis,remarks,dispensing,auditor,distributor,
        	checker,doctor,totalAmount,prescriptionType,printCount,printFlag } = row;
        return (
          <div key = {'_prescription_his_items_' + idx} className = 'prescription_row' >
