...
class Page extends TimerPage {
	...
	onMiCardPushed(cardInfo){
		...
		this.setState({step:2,medium:cardInfo.medium,address:cardInfo.Address},()=>{//选择一个默认类型
	...
	buildProfileByMi(miCardInfo){
	  ...
	  if('男' == xb)gender = '1';
	  if('女' == xb)gender = '2';
	  var birthday =csrq ? moment(csrq).format('YYYY-MM-DD'):'';//
	  const profile={
    ...
    issue(patient){
		...
		if( relationCard && (relationType == '01' || relationType == '02') && patient.no != patient.medicalCardNo){
			baseUtil.error('您已经办理过管局医保关联就诊卡');
			return;
    ...
    reIssue(patient){
		...
		if( relationCard && (relationType == '01' || relationType == '02') && patient.no != patient.medicalCardNo){//自费档存在且卡号存在
			...
			baseUtil.error('您还未办理过管局医保关联就诊卡');
		}
    }
