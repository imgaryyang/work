...
import MiddleMenuBlock from '../components/homepage/MiddleMenuBlock.jsx';
import BigMenuBlock from '../components/homepage/BigMenuBlock.jsx';
...
class Page extends Component {
	constructor (props) {
		...
		this.state = { menus : [],subModal:false,subMenus:[] };
		this.noLoginPaths=[
           '/',
           '/homepage',
           '/card/id/reissue',
	...
	onBack(){
		this.setState({subModal:false,subMenus:[]});
    }
    onHome(){
    	this.setState({subModal:false,subMenus:[]});
    }
	...
	closeMedicalCard(){
		try{ medicalCard.close() }catch(e1){console.info(e1)}
		try{ lightUtil.medicalCard.turnOff(); }catch(e2){console.info(e2)}
		return 1;
	}
	loadOptMenus(){
		log('home-加载运维菜单');
	...
	menuSelect(menu){
		if(menu.children && menu.children.length > 0){
			this.setState({subModal:true,subMenus:menu.children});
			return;
		}
	...
	goMenu(menu){
		...
		//特殊页面不校验登录
	    for(var path of this.noLoginPaths){console.info(pathname, path);
	...
	render () { 
	  var scope = this;
	  var baseSpan = 8;
	  var {menus,subMenus,subModal} = this.state;
	  var bodyHeight = document.body.clientHeight -240;
	  
	  var bigMenus = [],middleMenus=[];
	  for(var i=0;i<menus.length;i++){
		  if(i<3)bigMenus.push(menus[i]);
		  else middleMenus.push(menus[i]);
	  }
	  
	  
	  var modalWidth = (subMenus.length < 4)?  subMenus.length * 300 : 1200;
	  var modalHeight = Math.ceil(  subMenus.length/4)* 300 ;
	  console.info( Math.ceil(  subMenus.length/4) );
	  var modalTop = ( document.body.clientHeight - modalHeight )/2  ;
	  return (
	  <div >
	  {
		  subModal?(
				  <NavContainer  title='选择' onBack={this.onBack} onHome={this.onHome} >
			      ...
			      <div className = 'homepage_header' >
			        <Row >
			          <Col span = {3}  onClick = {this.clickLogo}>
			            <img src="./images/logo-dq.png" className = 'homepage_headerLogo' />
			          </Col>
			            <Col span = {16} className = 'homepage_headerTitle' >{this.props.patient.name||'大庆龙南医院自助服务系统'}</Col>
						 {
							 (this.props.patient.cardType && this.props.patient.cardType == "operator")?(
				  ...
				  <div className = "homepage_container" style={{height:bodyHeight+'px'}} >
					<div className = "homepage_center" >
					  <Row className = "homepage_row" >
					  {
						bigMenus.map(function(menu, index){
							let span = baseSpan * menu.colspan;
							return (
							  <Col span = {span} key = {index} className = 'homepage_col' >
							    <BigMenuBlock onSelect = {scope.menuSelect} menu = {menu} />
						      </Col>
							)
						})
					  }
					  {
						  middleMenus.map(function(menu, index){
								let span = 6 * menu.colspan;
								return (
								  <Col span = {span} key = {index} className = 'homepage_col' >
								    <MiddleMenuBlock onSelect = {scope.menuSelect} menu = {menu} />
							      </Col>
								)
							})
					  }
