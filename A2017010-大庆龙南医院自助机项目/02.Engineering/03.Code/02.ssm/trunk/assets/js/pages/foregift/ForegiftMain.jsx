...
class Page extends TimerModule {
	constructor (props) {
		...
		this.channels={
		  'balance': { payChannelCode : 'balance',payTypeCode:'cash'},// 余额
	...
	componentWillMount(){
		...
		else if(patient.balance<=0 && channel == 'balance'){
	...
	buildSettle(order){
		log('buildSettle order ',order);
		const { channel } = this.state;
		log('buildSettle channel ',channel);
		var payChannel = this.channels[channel];
		log('buildSettle payChannel ',payChannel);
		if(!payChannel){
	...
	preCreate(order){log('preCreate ',order);
