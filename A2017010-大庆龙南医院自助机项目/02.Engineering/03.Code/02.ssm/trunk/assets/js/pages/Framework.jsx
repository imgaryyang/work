	...
	login(loginInfo){
		...
		fetch.then(res => {
			...
			if(!patient || !patient.no){
				log('frame-登录结果-未注册的患者');
				baseUtil.error('未注册的患者');
				return;
			}
			log('frame-登录结果-账户状态[{accStatus:'+patient.accStatus+',accStatusName:'+patient.accStatusName+']');
			
			if(patient.cardType != "operator" && (patient.accStatus == '-1' || patient.accStatusName == '-1')){
				log('frame-登录结果-未开通预存');
				this.setState({patient:patient||{}},()=>{//未开通预存不校验
					baseUtil.fireEvents('login');
				});
				return; 
			}
			if(patient.cardType != "operator" && (patient.accStatus == '0' || patient.accStatusName == '0')){
				log('frame-登录结果-账户已被挂失');
				baseUtil.error('您的账户已被挂失,请携带证件到窗口解挂！');
				return;
			}
			if(patient.cardType != "operator" && patient.accStatus != '1'){
				log('frame-登录结果-状态异常'+patient.accStatusName);
