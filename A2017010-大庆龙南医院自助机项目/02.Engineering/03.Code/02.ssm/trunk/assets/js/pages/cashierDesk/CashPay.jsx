...
class PrepaidCash extends TimerPage {
  ...
  constructor(props) {
    ...
    this.listenCash = this.bind(this.listenCash,this);
    this.recordError = this.bind(this.recordError,this);
    
    this.successHandler= this.bind(this.successHandler,this);
    this.errorHandler= this.bind(this.errorHandler,this);
    this.unConfirmHandler= this.bind(this.unConfirmHandler,this);
    this.fullHandler= this.bind(this.fullHandler,this);
    this.acceptHandler= this.bind(this.acceptHandler,this);
	this.returningHandler= this.bind(this.returningHandler,this);
	this.idleHandler= this.bind(this.idleHandler,this);
	this.stopHandler = this.bind(this.stopHandler,this);
	this.showTimer = this.bind(this.showTimer,this);
	this.mask= this.bind(this.mask,this);
	this.unMask= this.bind(this.unMask,this);
	this.submitOrder = this.bind(this.submitOrder,this);
	this.clickButton = this.bind(this.clickButton,this);
	this.afterPay = this.bind(this.afterPay,this);
	this.renderInfoConfirm = this.bind(this.renderInfoConfirm,this);
  ...
  componentWillUnmount() {
	  log('现金-现金页面离开-componentWillUnmount'); 
	  try{
		  cashBox.stopListenCash();
	  }catch(e){
		  this.recordError('','Unmount钱箱关闭出错'+JSON.stringify(e));
		  baseUtil.error("Unmount钱箱关闭出错，请联系管理人员'");
	  }
  }
  componentDidMount() {
  ...
  listenCash(){
	  this.setState({showModal:false,msg:'',time:cashTimeout},()=>{//消除遮罩
		  cashBox.listenCash({
  ...
  render() {
	...
	const { order,limit,time } = this.state;
    ...
    return (
      <NavContainer title='现金' onBack={this.onBack} onHome={this.onHome} >
	      ...
	      <Card  style = {{height: '100%', textAlign: 'center'}} >
	        <div className = 'cash_payAmt' ><font>{name}</font>，您本次预存金额&nbsp;<font>{order ? order.realAmt : 0}</font>&nbsp;元，倒计时<font>{(time/1000)}s</font></div>
	        </Card>
	    </div>
        <Card  style = {cardStyle} >
            <Row className = 'cash_tip' >
              ...
              <Col span = {21} >
            	...
            	<li><Icon type="caret-right" />&nbsp;&nbsp;本机支持面额<font style= {{color: '#DB5A5A', fontSize: '3.5rem'} }>&nbsp;100、50&nbsp;</font>元的人民币纸币</li>
	        ...
	        <Card >
	          <Row>
		          <Col span = {24} style = {txtStyle} >
		            <font style={fontStyle}>1、</font>当前患者<font style={fontStyle}>{order.patientName}</font>,&nbsp;&nbsp;<font style={fontStyle}>右侧红灯变绿</font>后存入。
		          </Col>
	          </Row>
	          <Row>
		          <Col span = {24} style = {txtStyle} >
	          ...
	          <Row>
		          <Col span = {24} style = {txtStyle}>
		            <font style={fontStyle}>4、</font>只接受<font style={fontStyle}>50元</font>以上纸币
		          </Col>
	          </Row>
	          <Row>
		          <Col span = {24} style = {txtStyle}>
