...
function createConnection(){
	...
	if(resultCode != 0){
		throw new Error("Socket连接建立失败,host:"+HOST+',port:'+PORT);
	}
	return json;
}
//关闭Socket连接
...
function request(str,config){
	...
	//if(!config.quiet)baseUtil.mask('socket'+str);//handleError("beforeSend",str);
...
//		}
	...
	// if(!config.quiet)baseUtil.unmask('socket'+str);//handleError("afterSend",str);
	log("****************socket end **************");
