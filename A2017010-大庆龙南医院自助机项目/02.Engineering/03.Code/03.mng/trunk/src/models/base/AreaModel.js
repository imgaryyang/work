import dva from 'dva';
import { loadAreaPage, saveArea,deleteArea,deleteAllAreas} from '../../services/base/AreaService';
export default {
	...
	effects: {
		*load({ payload }, { select, call, put }) {
	    	...
	    	var defaultPage = yield select(state => state.area.page);
	    	...
	    	const { data } = yield call( loadAreaPage, start, pageSize, query);
	    	yield put({
	    ...
	    *'delete'({ id }, { call, put }) {
	        ...
	        const { data } = yield call(deleteArea, id);
	     ...
	     *deleteSelected({ payload }, { select, call, put }) {
	        ...
	        yield put({ type: 'setState', payload: { spin: true } });
	        const { data } = yield call(deleteAllAreas, selectedRowKeys);
	        yield put({ type: 'setState', payload: { spin: false } });
	        if (data && data.success) {
	          yield put({ type: 'loadTypes' });
	          yield put({ type: 'load' });
	...
	reducers: {
		"init"( state, { data, page }) {
			...
			var areas = result || [];
			return {...state, data : areas, page : p };
		},
