package com.lenovohit.ssm.payment.support.bankPay.utils;
    ...
    public T parse(byte[] rsp, String charset) throws BaseException {
		...
		try {
			...
			if(this.clazz == BankRefundResponse.class){
				...
				brfr.setRefundTotal(StringUtils.trim(subBytes(rsp, 20, 30, charset)));
				brfr.setRefundTotalAmt(StringUtils.trim(subBytes(rsp, 30, 46, charset)));
