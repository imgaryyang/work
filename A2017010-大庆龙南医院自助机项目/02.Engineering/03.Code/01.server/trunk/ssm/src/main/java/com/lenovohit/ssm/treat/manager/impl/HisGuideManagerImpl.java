...
public class HisGuideManagerImpl implements HisGuideManager{
	...
	@Override
	public HisListResponse<Guide> getTradeGuides(Clinical param) {
		Map<String,Object> reqMap = new HashMap<String,Object>();
        //入参字段映射
        reqMap.put("JYID", param.getTradeId());// 交易ID	
        
        RestListResponse response = hisRestDao.postForList("MZSF000004", RestRequest.SEND_TYPE_LOCATION, reqMap);
		HisListResponse<Guide> result = new HisListResponse<Guide>(response);
		...
		if(response.isSuccess() && null != resMaplist){
			for(Map<String, Object> resMap : resMaplist){
	        	...
	        	guide.setAddr(object2String(resMap.get("ADDR")));//地址	addr
	        	guide.setJyid(object2String(resMap.get("JYID")));//交易ID	jyid
	        	guide.setPfck(object2String(resMap.get("PFCK")));//配方窗口	pfck
	        	guide.setDoctorName(object2String(resMap.get("YSXM")));//就诊医生ysxm
				resList.add(guide);
			}
			result.setList(resList);
		}
