...
public class BankPayManagerImpl extends BankPayManager {
	...
	public void syncCheckFile(CheckRecord checkRecord) {
		...
		switch (result.getTradeStatus()) {
			case SUCCESS:
				checkRecord.setTotal(Integer.valueOf(response.getRefundTotal()));
				checkRecord.setAmt(new BigDecimal(response.getRefundTotalAmt()));
	...
	public void importCheckFile(CheckRecord checkRecord) {
		...
		try {
            ...
            while ((tempString = br.readLine()) != null) {  
            	...
            	cdb.setClearDate(checkRecord.getChkDate());
...
//			List<CheckDetailHis> cdhl = new ArrayList<CheckDetailHis>();
			...
			for(CheckDetailBank cdb : cdbl){
				...
				if(null == detail && null != settlement){
...
//					}
				...
				log.info("银行对账记录流水号【"+ result.getTradeNo() + "】，金额【"+result.getAmt().toString() + "】，对账状态【"+result.getHisCheckStatus() + "】。");
			}
			this.checkDetailResultManager.batchSave(cdrl);
//			this.checkDetailHisManager.batchSave(cdhl);
...
//		SR17061600003334|201706160052209914|HB02|6212262502018152861|14.00
	...
	private boolean downloadFile(CheckRecord checkRecord, BankDownloadResponse response) {
		boolean flag = true;
		SFTPUtils sftp = null;
		try {
			String checkString = checkRecord.getPayChannel().getRefCheckUrl();
			...
			sftp.login();
			sftp.download("", checkRecord.getChkFile(), checkRecord.getFilePath() + checkRecord.getChkFile());
		} catch (Exception e) {
			flag = false;
			log.error("银行【"+ checkRecord.getPayChannel().getName() + "】【"+ checkRecord.getChkDate() +"】日 同步对账文件失败：" + e.getMessage());
			e.printStackTrace();
		} finally {
			if(sftp != null) {
				sftp.logout();
