...
/**
 * 	needCardPrinter//需要证卡打印机
	needMSPrinter//需要凭条打印机
	needReportPrinter//校验报告打印机
	needAccount//需要开通预存
	notAtMidnight//午夜不允许办理业务
	needLogin//需要患者登录 最后一个校验 后续所有校验逻辑应移植到react-router的onEnter中。
	needCash//使用现金功能
 */
class Page extends Component {
	...
	needCardPrinter(callback) {//需要证卡打印机
		log('homepage-check needCardPrinter');
	...
	needMSPrinter(callback) {//需要凭条打印机
		log('homepage-check needMSPrinter');
	...
	needReportPrinter(callback) {//校验报告打印机
		log('homepage-check needReportPrinter');
	...
	needAccount(callback) {//需要开通预存
		log('homepage-check needAccount');
	}
	notAtMidnight(callback) {//午夜不允许办理业务
		log('homepage-check notAtMidnight');
