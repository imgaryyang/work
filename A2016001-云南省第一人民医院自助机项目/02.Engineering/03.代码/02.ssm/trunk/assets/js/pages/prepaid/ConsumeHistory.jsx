...
class PrepaidHistory extends React.Component {
  ...
  doSearch(){
	...
	let fetch = Ajax.get("/api/ssm/treat/deposit/records/consume",query,{catch: 3600});
	fetch.then(res => {
	  if(res && res.success){
  ...
  render() {
    ...
    return (
      <NavContainer title='消费记录' onBack={this.onBack} onHome={this.onHome} >
		  ...
		  <Row>
	          ...
	          <Col span = {6} className = 'list_title list_amt' >收费金额</Col>
	          <Col span = {6} className = 'list_title list_center' >收费类型</Col>
	          <Col span = {6} className = 'list_title' >收款员</Col>
	      </Row>
  ...
  renderItems (records) {
    ...
    return records.map ( (row, idx) => {
      const { patientNo, patientName, amount, type, time, doctor, cashier } = row;
      var typeName = type == "1"?"挂号":type == "2"?"门诊收费":type == "3"?"体检收费":type == "4"?"医院授权透支冲账":"支付";
