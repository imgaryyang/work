...
class PrepaidHistory extends React.Component {
  ...
  doSearch(){
	...
	let fetch = Ajax.get("/api/ssm/treat/deposit/records/payHistory",query,{catch: 3600});
	fetch.then(res => {
	  if(res && res.success){
		  ...
		  baseUtil.error("查询消费记录出错");
	  }
	}).catch((ex) =>{
		baseUtil.error("查询消费记录异常");
	})  
  }
      ...
      <NavContainer title='缴费明细' onBack={this.onBack} onHome={this.onHome} >
	      <Card  style = {{marginBottom: '2rem',fontSize: '3rem'}} >
	  		<span>
	  			...
	  			&nbsp;&nbsp;&nbsp;账户余额<font style = {{color: '#BC1E1E', fontSize: '4rem'}} >{(balance||0).formatMoney()}</font>&nbsp;&nbsp;元
	  		</span>
	  	  </Card>
	      <Row style = {{paddingLeft: '1.5rem',paddingRight: '1.5rem'}}>
			...
			<Col span = {6} className = 'comsume_toolBarItem' style = {{paddingRight: '.5rem'}} ></Col>
		  ...
		  <Row>
	          <Col span = {5} className = 'list_title' >发生时间</Col>
	          <Col span = {10} className = 'list_title list_center' >收费项</Col>
	          <Col span = {3} className = 'list_title list_amt' >金额</Col>
	          <Col span = {3} className = 'list_title list_center' >自负比例</Col>
	          <Col span = {3} className = 'list_title' >开单医生</Col>
	      </Row>
  ...
  renderItems (records) {
    ...
    return records.map ( (row, idx) => {
    	const {itemCosts, itemName, payDate, myselfScale, doctorName } = row;
      	const amt = parseFloat(itemCosts||0).formatMoney();
      	const scale = myselfScale * 100
        return (
          <div key = {'_order_items_' + idx} className = 'comsume_row' >
  	          <Row type="flex" align="middle" >
		          <Col span = {5} className = 'list_item' >
		          	{moment(payDate).format('YYYY-MM-DD HH:mm')}
		          </Col>
