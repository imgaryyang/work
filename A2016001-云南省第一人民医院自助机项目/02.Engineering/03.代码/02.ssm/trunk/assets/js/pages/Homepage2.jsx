...
class Page extends Component {
	...
	menusToTree(menus){
		...
		for(var menu of menus){
			if(!menu.parent){
				//console.info('no parent ',menu);
				root.push(menu);
			}else{
				var parent = map[menu.parent];
				if(!parent){ 
					//console.info('parent not fund ',menu);
					root.push(menu);
				}else{
	...
	menuSelect(menu){
		...
		if(!this.checkReportPrinter(menu))return;
	...
	checkReportPrinter(menu){//校验报告打印机
		var code = menu.code;
		if(code == 'checkRecords' || code == 'bloodcheck' || code == 'caseHistoryRecords'){
			var json = printer.getReportPrinterStatus();
			console.info("checkReportPrinter.json", json);
			if(json.state != 0 ){
				baseUtil.error(json.msg);
				return false;  
			}
