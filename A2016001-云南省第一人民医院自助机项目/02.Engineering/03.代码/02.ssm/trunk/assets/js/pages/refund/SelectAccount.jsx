...
class Page extends React.Component {
  ...
  constructor(props) {
	...
	this.onSelectIdType = this.onSelectIdType.bind(this);
	this.listenIdCard = this.listenIdCard.bind(this);
  ...
  onSelect(account){
	...
	if(accType=='0'){// 银行卡
		...
		this.setState({account},()=>{
			this.onSelectIdType('0');//默认选择他人银行卡
		});
	}else{
		console.info('选择非银行卡');
	  ...
	  <NavContainer title='退款账户' onBack={this.onBack} onHome={this.onHome} >
	    <Card  style = {{margin: '0 2rem 0 2rem', padding: '2rem 2rem 0 2rem', fontSize: '3rem'}} >
	      <span>账户余额：
	      	...
	      	请选择您的退款账户，信用卡、微信、支付宝充值只能原交易退款，现金充值请到收费窗口人工处理。
	      </span>
	    </Card>
	  	{
  ...
  renderEmpty(){
	  return <Empty info = '暂无可退款账户，现金充值请到收费窗口人工处理。' />;
  }
  renderCard(accounts){
