...
class Page extends Component {
	constructor (props) {
		...
		this.goOptHome = this.goOptHome.bind(this);
		...
		baseUtil.register('goOptHome',this.goOptHome);
	...
	componentWillMount () {
		...
		this.machineLogin();
		setInterval(() => this.heartbeat(), 1000*60*5);//定时监控
	}
	startMedicalCard(){
	...
	goOptHome(mark){
		...
		const { location } =  this.props;
		log('gohome ',{mark,location});
		this.props.history.push({ pathname: '/opt/main',})
	}
	getCurrentPatient(){
	...
	heartbeat(){    
		console.info("定时请求");
		Ajax.get("/api/ssm/base/redis/save",null, {catch: 3600,quiet:true});
	}
	login(loginInfo){
		...
		fetch.then(res => {
			...
			if(patient.cardType != "operator" && patient.accStatus != '1'){
				baseUtil.error('您的账户已经'+patient.accStatusName+',禁止在自助机操作,请到窗口人工处理');
				return;
			}
			if(patient.cardType != "operator" && patient.status != '1'){
				baseUtil.error('您的账户已经注销,禁止在自助机操作,请到窗口人工处理');
				return;
			}
