...
class DepositConsume extends Component {
  ...
  constructor(props){
	  ...
	  this.consumeCallback = this.consumeCallback.bind(this);
  ...
  consumeCallback(success){
	...
	fetch.then(res => {	log("消费-登记扣款返回:",res);
		if(res && res.success){
			...
			if(success)this.afterPay(newOrder);//只有成功了才回调
		}else if( res && res.msg ){
			this.error(res.msg);
    	}else{
    		this.error("登记扣款结果失败");
        ...
        <div style = {{ padding: '6rem 0 3rem 0'}} >
          <Card  style = {{height: height+'px', textAlign: 'center'}} >
            <div className = 'consume_payAmt' >
            	...
            	将被扣费&nbsp;<font>{order.selfAmt ? order.selfAmt : 0}</font>&nbsp;元
            </div>
          </Card>
        </div>
