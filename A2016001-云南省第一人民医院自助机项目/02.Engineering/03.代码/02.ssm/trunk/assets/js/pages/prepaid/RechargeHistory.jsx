...
class PrepaidHistory extends React.Component {
    ...
    return (
      <NavContainer title='充值记录' onBack={this.onBack} onHome={this.onHome} >
		  ...
		  <Row>
	          ...
	          <Col span = {3} className = 'list_title list_center' >操作类型</Col>
	          <Col span = {4} className = 'list_title list_amt' >金额</Col>
	          <Col span = {3} className = 'list_title list_center' >状态</Col>
	          <Col span = {3} className = 'list_title' >支付方式</Col>
	          <Col span = {4} className = 'list_title' >账户</Col>
	      </Row>
  ...
  renderItems (records) {
    ...
    return records.map ( (row, idx) => {
        ...
        var accountNo = cardNo;
        if(paymentWay!='支付宝')
        	accountNo = cardNo.substr(0,4)+"******"+cardNo.substr(cardNo.length-4,cardNo.length)
        return (
