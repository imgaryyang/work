...
function init(){
	...
	//TODO 土鳖，虽然列表清了，任务还是不打印。
	//	LODAP.SET_PRINT_MODE("CONTROL_PRINTER:-1","PURGE");//清空打印列表
}
...
function  waitForPrint(JOB_ID){
	console.info("waitForPrint.JOB_ID",JOB_ID)
	var _waitNum = 1;
	async function wait(JOB_ID){
		var status = 1;
		while(status == 1 && _waitNum < LODAP_WAIT_NUM){
			await baseUtil.sleep(12*1000);
			status = lodopPrintStatus(JOB_ID);
			log('第【'+_waitNum +'】次lodopPrintStatus status ',status);
			_waitNum++;
		}
	}
	wait();
...
function lodopPrintStatus(JOB_ID){
	if(dev_mode){return 0;}
	var LODOP = LodopFuncs.getLodop();
	var status = LODOP.GET_VALUE("PRINT_STATUS_EXIST",JOB_ID);
	return status;
}
//获取报告打印机状态
...
function  printMedicalRecord(msg){
	...
	// LODOP.print.....调用打印机方法
	var LODOP = LodopFuncs.getLodop();
	var taskName = "门诊病历"+(new Date()).getTime();
	var inited = LODOP.PRINT_INITA("0cm","0.1cm",800,800,taskName);
	if(!inited){
		log('printer','打印机初始化异常-'+taskName);
		baseUtil.error('打印机初始化异常,请联系管理员');
	}
	LODOP.ADD_PRINT_HTML("0cm","0.1cm","100%","100%",msg);
	//A5纸打印
 ...
 * @returns
 */
function printCommonAssay(imagePath){
	if(dev_mode){return;}
	lightUtil.form.blink();
	init();
	// LODOP.print.....调用打印机方法
	let LODOP = LodopFuncs.getLodop();
	var taskName = "检查单报告"+(new Date()).getTime();
	var inited = LODOP.PRINT_INIT(taskName);
	if(!inited){
		log('printer','打印机初始化异常-'+taskName);
		baseUtil.error('打印机初始化异常,请联系管理员');
	}
...
function printBloodAssay(result){
	...
	**/
	...
	let specimenNo = result.id.orderno;
	...
	let orderDate = result.orderdate;//标本接收时间
	let checkDoctor = result.jsdoctor;//
	let jyDoctor = (result.details)[0].jydoctor;
	let reportDate = result.auditDate;//审核时间
	let reviewer = result.reviewer;//审核者
	let auditDate = (result.details)[0].jydate;//报告时间
	
	let LODOP = LodopFuncs.getLodop();
	var taskName = "血液科报告"+(new Date()).getTime();
	var inited = LODOP.PRINT_INIT(taskName);
	if(!inited){
		log('printer','打印机初始化异常-'+taskName);
		baseUtil.error('打印机初始化异常,请联系管理员');
	...
	LODOP.SET_PRINT_STYLEA(0,"FontSize",10);
	LODOP.ADD_PRINT_TEXT(153,87,500,26,diagnosis);
	LODOP.SET_PRINT_STYLEA(0,"FontSize",10);
	...
	LODOP.ADD_PRINT_TEXT((startTextHeight+(fixedHeight*(temp+1))),30,125,26,"报告时间：");
	LODOP.SET_PRINT_STYLEA(0,"FontSize",10);
	LODOP.ADD_PRINT_TEXT((startTextHeight+(fixedHeight*(temp+1))),136,162,26,auditDate);
	LODOP.SET_PRINT_STYLEA(0,"FontSize",10);
	LODOP.ADD_PRINT_TEXT((startTextHeight+(fixedHeight*(temp+1))),301,76,26,"检测者：");
	LODOP.SET_PRINT_STYLEA(0,"FontSize",10);
	LODOP.ADD_PRINT_TEXT((startTextHeight+(fixedHeight*(temp+1))),361,151,26,jyDoctor);
	LODOP.SET_PRINT_STYLEA(0,"FontSize",10);
	
	LODOP.ADD_PRINT_TEXT((startTextHeight+(fixedHeight*(temp+1))),531,94,26,"审核者：");
	LODOP.SET_PRINT_STYLEA(0,"FontSize",10);
	LODOP.ADD_PRINT_TEXT((startTextHeight+(fixedHeight*(temp+1))),589,162,26,reviewer);
	LODOP.SET_PRINT_STYLEA(0,"FontSize",10);
	
	LODOP.ADD_PRINT_TEXT((startTextHeight+(fixedHeight*(temp+2))),30,354,26,"审核时间：");
	LODOP.SET_PRINT_STYLEA(0,"FontSize",10);
	LODOP.ADD_PRINT_TEXT((startTextHeight+(fixedHeight*(temp+2))),136,362,26,reportDate);
	LODOP.SET_PRINT_STYLEA(0,"FontSize",10);
	LODOP.ADD_PRINT_TEXT((startTextHeight+(fixedHeight*(temp+2))),531,254,26,"注：此结果仅对此标本有效");
	LODOP.SET_PRINT_STYLEA(0,"FontSize",10);
	LODOP.SET_PRINT_MODE("CATCH_PRINT_STATUS",true);
//	LODOP.SET_PRINTER_INDEX("Canon LBP3000");
	LODOP.SET_PRINTER_INDEX(-1);
 ...
 * @returns
 */
function printCashDetails(result, patient){
	if(dev_mode){return;}
	if(result.batchNo==null){
	baseUtil.warning("暂未收钞，请收钞后再打印");return;}
	...
	var amt1=result.amt1;
	var amt2=result.amt2;
	var amt5=result.amt5;
	var amt10=result.amt10;
	var amt20=result.amt20;
	var amt50=result.amt50;
	var amt100=result.amt100;
	var count1=result.count1;
	var count2=result.count2;
	var count5=result.count5;
	var count10=result.count10;
	var count20=result.count20;
	var count50=result.count50;
	var count100=result.count100;
	
	var headerText = "	清钞数据凭证";//倍高
	var patientNo = result.patientNo;
	var patientName = result.patientName;
	var batchDay=result.batchDay;
	var batchNo=result.batchNo;
	var Amt = result.amt.formatMoney();
	var Count = result.count.formatMoney();
	var Mac=result.machineMac;
	var hisUser = result.machine.hisUser;//取自助机在HIS系统中的编号
	var operatorNo = patient.no;
	var operatorName = patient.name;
	var attentionContent = "	请自行妥善保管";
	var machineCode=result.machineCode;
	var setPrintport = MS_PRINTER.SetPrintport("COM5",38400);//设置串口
	var setInit = MS_PRINTER.SetInit();//打印机初始化
	var getStatus = MS_PRINTER.GetStatus();
	var amtTypeSum=result.amtTypeSum;
	var amtType=[1,2,5,10,20,50,100];
	var countSum=[count1,count2,count5,count10,count20,count50,count100];
	var moneySum=[amt1,amt2,amt5,amt10,amt20,amt50,amt100];
	/**
	...
	**/
	if(getStatus == 0){
		...
		*/
		...
		MS_PRINTER.PrintString("清钞数据存根                  请妥善保管");
		MS_PRINTER.SetLinespace(50);//设置行间距
		MS_PRINTER.PrintString("BANKNOTE DATA STUB	   KEEP IT CAREFULLY");
		MS_PRINTER.SetLinespace(50);//设置行间距
		MS_PRINTER.PrintString("-----------------------------------------");
		MS_PRINTER.SetSizetext(2, 1);
		MS_PRINTER.PrintString("机器编号："+machineCode+"    清钞日期: "+batchDay);
		MS_PRINTER.SetSizetext(1, 1);
		MS_PRINTER.PrintString("-----------------------------------------");
		MS_PRINTER.PrintString("面额："+amtType[0]+"|    "+amtType[1]+"|    "+amtType[2]+"|    "+amtType[3]+"|    "+amtType[4]+"|  "+amtType[5]+"|"+amtType[6]);
		MS_PRINTER.PrintString("笔数："+countSum[0]+"|    "+countSum[1]+"|    "+countSum[2]+"|    "+countSum[3]+"|    "+countSum[4]+"|  "+countSum[5]+"|"+countSum[6]);
		MS_PRINTER.PrintString("金额："+moneySum[0]+"|    "+moneySum[1]+"|    "+moneySum[2]+"|   "+moneySum[3]+"|    "+moneySum[4]+"|  "+moneySum[5]+"|"+moneySum[6]);
		MS_PRINTER.PrintString("-----------------------------------------");
		MS_PRINTER.SetSizetext(2, 1);
		MS_PRINTER.PrintString("币种数 "+amtTypeSum+"    "+"总笔数 "+Count+"    "+"总金额 "+Amt);
		MS_PRINTER.SetSizetext(1, 1);
		MS_PRINTER.PrintString("-----------------------------------------");
		MS_PRINTER.PrintString("批次序号："+batchNo);
		MS_PRINTER.PrintString("创建时间："+createTime);
		MS_PRINTER.PrintString("机器 MAC："+Mac);
		MS_PRINTER.PrintString("操 作 员："+hisUser);
		MS_PRINTER.PrintString("清钞人员："+operatorName);
 ...
 * 打印报告通知凭条
 */
function printAssayMsg(patient,result,msg){
	if(dev_mode){return;}
	lightUtil.ticket.blink();
	init();
	var title = "报告通知单";
	var patientName = patient.name;//病人姓名
	var patientGender = result.patientGender;//性别
	var patientNo = patient.no;//病人编号
	var patientAge = result.patientAge;//年龄
	var subjectName = result.subjectName;//检验项目
	var barcode = result.barcode;//单号
	var applydate = result.applydate;//开立时间
	var assayMsg = "无法获取报告内容，请至柜台办理打印业务！";
	...
	**/
	if(getStatus == 0){
		...
		MS_PRINTER.SetSizetext(2, 1);
		MS_PRINTER.SetLinespace(55);//设置行间距
		MS_PRINTER.PrintString("姓名："+patientName);
		MS_PRINTER.PrintString("患者号："+patientNo);
		MS_PRINTER.PrintString("性别："+patientGender);
		MS_PRINTER.PrintString("年龄："+patientAge);
		MS_PRINTER.PrintString("");
		MS_PRINTER.PrintString("检验项目："+subjectName);
		MS_PRINTER.PrintString("单号："+barcode);
		MS_PRINTER.PrintString("开立时间："+applydate);
		MS_PRINTER.SetSizetext(2, 2);
		MS_PRINTER.SetLinespace(55);//设置行间距
		MS_PRINTER.PrintString("通知："+msg);
...
function printAppoint(result){
	...
	var scheduleDeptName = result.scheduleDeptName?"专病："+result.scheduleDeptName:"";//"专病";
	...
	**/
	if(getStatus == 0){
		...
		MS_PRINTER.PrintString(scheduleDeptName);//专病
...
function printRefundReceipt(result,order,baseInfo,machineInfo){
	...
	**/
	if(getStatus == 0){
		...
		if(result.payerAccount){
			var accNo  = result.payerAccount;
			var accountNo = accNo.substr(0,4)+"******"+accNo.substr(accNo.length-4,accNo.length)
			MS_PRINTER.PrintString("退款账户："+accountNo);
		}
		if(result.payerName)MS_PRINTER.PrintString("退款户名："+result.payerName);
 ...
 * @returns
 */
function printDoCardFees(rechargeOrder,consumeOrder,profile,machineInfo){
	if(dev_mode){return;}
	lightUtil.ticket.blink();
	init();
	// MS_PRINTER.print.....调用打印机方法
	...
	var patientNo = profile.no;//
	var gender = profile.gender;//1-男 2-女
	var genderText = "";
	if(gender == '1'){ genderText = "男"; }
	if(gender == '2'){ genderText = "女"; }
	var birthday = profile.birthday;
	var age = jsGetAge(profile.birthday.substring(0,10));
	var balance = profile.balance.formatMoney();
	var operator = machineInfo.hisUser;//取自助机在HIS系统中的编号
	var rechargeTime ,consumeTime;
	if(rechargeOrder && rechargeOrder.createTime)rechargeTime =  rechargeOrder.createTime;
	else rechargeTime = moment(new Date()).format('YYYY-MM-DD HH:mm:ss');
	
	if(consumeOrder && consumeOrder.createTime)consumeTime =  consumeOrder.createTime;
	else consumeTime = moment(new Date()).format('YYYY-MM-DD HH:mm:ss');
	var attentionContent = "请自行妥善保管";
	...
	**/
	if(getStatus == 0){
		...
		**/
		...
		MS_PRINTER.PrintString("          "+patientName+"   "+genderText+" "+age+"岁");
		if(rechargeOrder && rechargeOrder.orderNo){
			MS_PRINTER.PrintString("------------------------------------------------");
			MS_PRINTER.PrintString("充值收据号："+rechargeOrder.orderNo);
			MS_PRINTER.PrintString("  充值金额："+rechargeOrder.realAmt);
			MS_PRINTER.PrintString("  充值日期："+rechargeTime.substring(0,10));
		}
		if(consumeOrder && consumeOrder.orderNo){
			MS_PRINTER.PrintString("------------------------------------------------");
			MS_PRINTER.PrintString("收费收据号："+consumeOrder.orderNo);
			MS_PRINTER.PrintString("  建档费用："+consumeOrder.realAmt);
			MS_PRINTER.PrintString("  收费日期："+consumeTime.substring(0,10));
		}
		MS_PRINTER.PrintString("------------------------------------------------");
 ...
 * @returns
 */
function printMakeUpCardFees(rechargeOrder,consumeOrder,profile,machineInfo){
	if(dev_mode){return;}
	lightUtil.ticket.blink();
	init();
	// MS_PRINTER.print.....调用打印机方法
	...
	if(getStatus == 0){
		...
		**/
		...
		if(consumeOrder && consumeOrder.orderNo){
			MS_PRINTER.PrintString("------------------------------------------------");
			MS_PRINTER.PrintString("收费收据号："+consumeOrder.orderNo);
			MS_PRINTER.PrintString("  补卡费用："+consumeOrder.realAmt);
			MS_PRINTER.PrintString("  收费日期："+consumeTime.substring(0,10));
		}
		MS_PRINTER.PrintString("------------------------------------------------");
		MS_PRINTER.PrintString("账户余额："+balance);
...
function printGuideCotent(baseInfo,machineInfo,clinical){
	...
	MS_PRINTER.PrintString(patientName+"      健康号:"+patientNo);
	MS_PRINTER.SetLinespace(30);//设置行间距
	MS_PRINTER.PrintString("------------------------------------------------");
...
const printer={
	...
	getReportPrinterStatus:getReportPrinterStatus,
	printMakeUpCardFees:printMakeUpCardFees,
	printForegift:printForegift,
	printAssayMsg:printAssayMsg,
}
