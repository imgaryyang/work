...
var mac=LODAP.getLodop().GET_SYSTEM_INFO('NetworkAdapter.1.PhysicalAddress');
!function (name, context, definition) {
	  ...
	  function init(o, fn) {
	    ...
	    function error(resp, msg, t) {
	    	var ctn = true;
	    	if(statusMsgEnum[resp.status]){
	    		if(window["_handler_"+resp.status]){
	    			ctn = window["_handler_"+resp.status].call(this,resp,msg,t);
	    		}
	    	}
	    	if(typeof(ctn) == "boolean" && ctn == false){
	    		return;
	    	}
	      resp = self.request
	      self._responseArgs.resp = resp
	  ...
	  function request(config,options){
		...
		config.url = url;	
		  ...
		  options = options||{};
		  if(!options.quiet){			  
			  try { baseUtil.mask(url);}catch(e){log('ajax mask error ',e)}
		  }
		  var unmask = function(){
			  if(!options.quiet){
				  baseUtil.unmask(url);
		  ...
		  var re = reqwest(config).then((res)=>{
			  try { unmask(url); }catch(e){log('then unmask error ',e)}
			  return res;
		  });
		  re._errorHandlers.push(()=>{console.info('catch ',url);
			  try { unmask(url); }catch(e){log('catch unmask error ',e)}
		  });
		  //var catchFn  = re['catch'];
//		  re['catch'] = (fn)=>{console.info('catchFn ',url,fn);
//			  try { unmask(); }catch(e){log('catch unmask error ',e)}
//			  console.info('catchFn ',re.fail);
//			   return re.fail(fn);
//		  }
		  return re;
		}
