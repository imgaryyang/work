...
class CreateCard02 extends React.Component {
  ...
  */
  ...
  setClock () {
	if(!this.timer)return;//如果组件已经销毁，则停止
  ...
  sendAuthCode() {
	  ...
	  this.props.dispatch({type: 'patient/sendAuthCode',payload: {
   ...
   */
  submit() {
    this.setState({ submitWaiting: true,acDisabled: true,second:0},()=>{
    	this.props.dispatch({type: 'patient/checkAuthCode',payload: {
        	mobile:this.state.mobile,
