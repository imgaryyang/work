...
class PaymentChannels extends React.Component {
  ...
  paymentChannels={
	  balancepay  : {code:'balancepay',name:'就诊卡余额',pathname:'/homepage'},
	  wxpay       : {code:'wxpay'     ,name:'微信'      ,pathname:''},
	  alipay      : {code:'alipay'    ,name:'支付宝'    ,pathname:''},
	  unionpay    : {code:'unionpay'  ,name:'银联'      ,pathname:''}
  }
  /**
  * 初始化状态
  */
  ...
  goPayChannel(code){
	  const channel = this.paymentChannels[code];
	  if(!channel)return;
...
export default connect(({patient,payment}) => ({patient,payment}))(PaymentChannels);
