...
class PaymentRecords extends React.Component {
  ...
  */
  ...
  render() {
    ...
    const { accountBill, accountBillLoaded } = this.props.order;
    
    if (accountBillLoaded === true && accountBill.length == 0){
    	return (
  ...
  renderItems () {
	  if (!this.props.order.accountBill){
		  return null;
	  }
	  else{
		  return this.props.order.accountBill.map (
			  (row, idx) => {
			    const { paidDate, orderTypeName, selfPaid, paidTypeName, items } = row;
			    return (
