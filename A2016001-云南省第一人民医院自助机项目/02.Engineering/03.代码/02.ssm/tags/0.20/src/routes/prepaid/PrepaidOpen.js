...
class PrepaidOpen extends React.Component {
  ...
  componentWillMount() {
    //测试用，正式版本去掉
    if (!this.props.patient.baseInfo){
    	this.props.dispatch({type: 'patient/login'});
    }
  }
  ...
  confirm () {
    ...
    let patient = this.props.patient.baseInfo;
    patient.PrepaidOpened = true;
    this.props.dispatch({
      type: 'patient/openDeposit',
      payload: {
    	  patient: patient
      }
    });
...
export default connect(({patient, message}) => ({patient, message}))(PrepaidOpen);