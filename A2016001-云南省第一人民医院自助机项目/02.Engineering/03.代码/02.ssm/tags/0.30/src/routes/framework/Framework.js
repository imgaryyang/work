...
class Framework extends Component {
  ...
  componentWillMount () {
	  this.props.dispatch({type:'frame/machineLogin'});
  }
  componentWillReceiveProps(nextProps){
    ...
    //未登录
    if(this.props.patient.baseInfo.no){//如果之前是登录状态
    	this.setState({modalDisplay:''},()=>{setTimeout(this.goHomeTimer,1000)});  
    }else{console.info('需要登录： ' , this.props.patient.baseInfo);
  ...
  render () {
    const { current,error } = this.props.frame;
    ...
    return (
      <div>
        ...
        <Message />
        <Confirm info = {error.msg||'' } visible = { error.msg?true:false } 
	        buttons = {[{text: '确定', onClick: () =>{
	         this.props.dispatch({type:'frame/setState',payload:{ error:{msg:null,quit:true} }});
	       	 if(error.quit)this.props.dispatch(routerRedux.push('/homepage'));
	        }},]}
	       />
