...
class PrepaidHistory extends React.Component {
  ...
  doSearch(){
    ...
    this.props.dispatch({
      type: 'deposit/loadRechargeHistory',
      payload: {
  ...
  render() {
	var records =this.props.deposit.recharge.records;
	var title="预存记录";
	const {startTime,endTime } = this.state;
    ...
    return (
        <WorkSpace fullScreen = {true} style = {{paddingTop: '2rem'}} >
          ...
          <Row>
            <Col span = {6} className = {listStyles.title} >发生时间</Col>
            <Col span = {4} className = {listStyles.title + ' ' + listStyles.center} >操作类型</Col>
            <Col span = {6} className = {listStyles.title + ' ' + listStyles.amt} >金额</Col>
            <Col span = {4} className = {listStyles.title + ' ' + listStyles.center} >状态</Col>
            <Col span = {4} className = {listStyles.title} >支付方式</Col>
          </Row>
        ...
        return (
          <div key = {'_order_items_' + idx} className = {styles.row} >
	          <Row type="flex" align="middle" >
		          ...
		          <Col span = {6} className = {listStyles.item + ' ' + listStyles.amt} >
		            {amt}
		          </Col>
		          <Col span = {4} className = {listStyles.item + ' ' + listStyles.center} >
		            {status=="0"?"处理中":'成功'}
...
export default connect(({deposit}) => ({deposit}))(PrepaidHistory);
