...
class AliPay extends React.Component {
  ...
  componentDidMount() {
    ...
    if(settlement.id){console.info("支付宝结算单已存在监听结算单状态：",settlement.id);
  ...
  componentWillReceiveProps(nextProps){
	  ...
	  if(!old.id && now.id){console.info("支付宝结算单创建完毕 开始监听结算单状态 ",now.id);
	  this.listenSettlement();
	  }
  }
  ...
  listenSettlement () {
	  if(this.timing){
		  const { settlement } = this.props.payment;
		  console.info("支付宝结算单"+settlement.id+"状态 ",settlement.status);
...
export default connect(({payment}) => ({payment}))(AliPay);
