          ...
          <Row gutter = {2 * config.navBar.padding * config.remSize} type = 'flex' justify = 'center' >
            <Col className = {styles.col} style = {{paddingBottom: 2 * config.navBar.padding * config.remSize + 'px'}} span = {12} >
              <Card shadow = {true} style = {cardStyle} onClick = {() => this.goTo('00', '就诊卡预存转预缴')} >
                <span className = {styles.balance} style = {{top: (cardHeight / 6) + 'px'}} >就诊卡账户当前余额<br/><h1>{this.props.account.account.Balance.formatMoney()}&nbsp;元</h1></span>
                <font>就诊卡预存转预缴</font>
              </Card>
            </Col>
            <Col className = {styles.col} style = {{paddingBottom: 2 * config.navBar.padding * config.remSize + 'px'}} span = {12} >
...
export default connect(({user, account, message}) => ({user, account, message}))(InpatientPrepaidNav);
