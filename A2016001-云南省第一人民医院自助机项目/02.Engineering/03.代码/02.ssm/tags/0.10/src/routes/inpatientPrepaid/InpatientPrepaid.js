...
class InpatientPrepaid extends React.Component {
  ...
  */
  ...
  componentWillMount() {
    ...
    if (!this.props.inpatient.PrepaidBalance)
      this.props.dispatch({
  ...
  acctTransToInpatientPrepaid () {
    ...
    this.props.dispatch({
      type: 'account/acctTransToIpptPrepaid', 
      payload:{
        amt: this.state.amt
      }
    });
  ...
  submit () {
    ...
    if (type == '00' && parseInt(this.state.amt) > Balance) {
      this.props.dispatch({
        type: 'message/show',
        payload: {
          msg: (
    ...
    if (PrepaidBalance + parseInt(this.state.amt) > config.inpatientPrepaid.limit) {
      this.props.dispatch({
        type: 'message/show',
        payload: {
          msg: (
    ...
    if (type == '00') {
      ...
      this.setState({
        bal: PrepaidBalance + parseInt(this.state.amt)
...
export default connect(({user, account, order, inpatient, message}) => ({user, account, order, inpatient, message}))(InpatientPrepaid);
