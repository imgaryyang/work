...
//	private static final int SYNC_ONCE_MAX_REPEAT = 3;					//一次同步最大重发次数
	...
	public void syncCheckFile() throws BaseException {
		...
		for(PayChannel pc : pcl){
			if(StringUtils.isNotEmpty(pc.getCheckTime()) && StringUtils.isEmpty(pc.getRefCheckTime())
				...
				checkRecord = this.checkRecordManager.findOne("from CheckRecord where chkDate = ? and chkType = ? and payChannel.id = ?", 
			...
			if(StringUtils.isNotBlank(pc.getRefCheckTime())
					&& checkDate.after(DateUtils.string2Date(checkDateStr  +  pc.getRefCheckTime(), "yyyy-MM-ddHH:mm:ss"))){
	...
	public void syncReturnFile() throws BaseException {
		...
		for(PayChannel pc : pcl){
			if(StringUtils.isNotEmpty(pc.getRetCheckTime()) 
				...
				checkRecord = this.checkRecordManager.findOne("from CheckRecord where chkType = '3' and chkDate = ? and payChannel.id = ?", 
				...
				if(null != checkRecord && (StringUtils.equals(checkRecord.getStatus(), CheckRecord.CHK_STAT_INITIAL)
					...
					checkRecord.setChkDate(checkDateStr);
					checkRecord.setOptType(CheckRecord.CHK_OPTTYPE_AUTO);
					checkRecord.setSyncType(pc.getRetCheckUrl().substring(0, pc.getRetCheckUrl().indexOf(":")));
					checkRecord.setSyncTime(DateUtils.getCurrentDate());
					checkRecord.setSyncNum(1);
	...
	public void checkReturnOrder() throws BaseException {
		...
		Date checkDate = DateUtils.addDays(new Date(), -1);
		String checkDateStr = DateUtils.date2String(checkDate, "yyyy-MM-dd");
		List<CheckRecord> crl = this.checkRecordManager.find("from CheckRecord where optType = '0' and chkDate = ? and chkType = '3' and status=?", checkDateStr, CheckRecord.CHK_STAT_IMP_SUCCESS);
		for(CheckRecord cr : crl){
			cr.setChkTime(DateUtils.getCurrentDate());
			new ScheduleEngine(cr, SYNC_TYPE_RET_CHECK, cr.getPayChannel().getCode()).start();
	...
	private class ScheduleEngine extends Thread {
		...
		public void run(){
            ...
            case "0308":
            	if (StringUtils.equals(SYNC_TYPE_DOWNLOAD, this.mode) 
						...
						|| StringUtils.equals(SYNC_TYPE_RET_CHECK, this.mode)){
			...
			if(StringUtils.equals(SYNC_TYPE_RET_CHECK, this.mode)){
				this.payBaseManager.checkReturnOrder(checkRecord);
			}
		...
		private void syncRefund(Settlement settlement){
			...
			if(StringUtils.equals(settlement.getStatus(), Settlement.SETTLE_STAT_REFUND_SUCCESS)){//2.1 退款成功处理
				//已经做过HIS交易的直接返回成功
			...
			} else if(StringUtils.equals(settlement.getStatus(), Settlement.SETTLE_STAT_REFUND_FAILURE) ){//2.2退款失败
			...
			} else if(StringUtils.equals(settlement.getStatus(), Settlement.SETTLE_STAT_REFUND_CANCELED) ){//2.4 退款失败|| 退款被撤销
				//考虑HIS已记账情况下的订单不通知HIS，需手工处理
				if(StringUtils.equals(order.getStatus(), Order.ORDER_STAT_TRAN_SUCCESS) ||
					StringUtils.equals(order.getStatus(), Order.ORDER_STAT_REFUND_CANCELED) || 
					StringUtils.equals(order.getStatus(), Order.ORDER_STAT_CLOSED)) {
				...
				//HIS未记账或者退款取消状态下
					...
					order.setStatus(Order.ORDER_STAT_REFUND_SUCCESS);
					order.setTranTime(DateUtils.getCurrentDate());
					order.setRealAmt(settlement.getAmt());
					...
					if(Order.ORDER_STAT_TRAN_SUCCESS.equals(order.getStatus())){
						...
						HisEntityResponse<HisOrder> _bizResponse = (HisEntityResponse<HisOrder>)bizAfterRefund(order, settlement);
