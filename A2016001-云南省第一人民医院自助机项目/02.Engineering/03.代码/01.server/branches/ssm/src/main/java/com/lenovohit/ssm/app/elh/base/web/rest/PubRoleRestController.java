...
@RestController
@RequestMapping("/hwe/app/pubrole")
public class PubRoleRestController extends BaseRestController {
	...
	public Result forCreate(@ModelAttribute AppRole model ) {
		...
		List<Role> lst = this.appRoleManager.find(validHql,
				model.getName(), model.getId());
		if (lst.size() > 0) {
			throw new BaseException("该名称已存在,请重新填写!");
		}*/
	...
	public Result forDelete() {
		...
		this.appRoleManager.executeSql(accSql);
	...
	public Result forUpdate(@ModelAttribute AppRole model){  
