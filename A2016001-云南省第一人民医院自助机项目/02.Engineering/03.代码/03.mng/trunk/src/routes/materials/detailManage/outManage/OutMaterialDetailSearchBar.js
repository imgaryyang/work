...
class OutMaterialDetailSearchBar extends Component {
  ...
  render() {
    ...
    const { materials } = this.props.outMaterialDetail;
    const options = materials.map(d => <Option key={d.id}>{d.name}</Option>);
    const props = {
    		  name: 'file',
    		  action: '/api/ssm/base/materialDetailOut/import',
    		  headers: {
    ...
    return (
      <Form inline onSubmit={this.handleSubmit.bind(this)}>
        ...
        <FormItem>
    			<Button onClick={this.handleExport.bind(this)} size="large" icon="download" >导出出库记录</Button>
    	</FormItem>
    	<FormItem>
        	<Upload {...props}>
	    		<Button><Icon type="upload" /> 导入出库记录记录</Button>
	    	</Upload>
	    </FormItem>
      </Form>
    );
...
const MaterialDetailSearchBarForm = Form.create()(OutMaterialDetailSearchBar);
export default connect(({ machine, outMaterialDetail }) => ({ machine, outMaterialDetail }))(MaterialDetailSearchBarForm); 
