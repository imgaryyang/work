...
class MenuTree extends React.Component {
	...
	componentWillMount(props){
		this.props.dispatch({
			type : 'mngAuth/loadMenus',
		});
//		if(this.props.mngAuth.roleId != null){
//			this.props.dispatch({
//				type : 'mngAuth/loadMenuKeys',
//			});
//		}
	}
	...
	onCheck(selectedKeys,{halfCheckedKeys}){//console.info(arguments,halfChecked);
		...
		var menu = {...this.props.mngAuth.menu, selectedKeys:selectedKeys};
		this.props.dispatch({
			type : 'mngAuth/setState',
			payload : { menu: menu}
		});
	}
	forSave(){
		this.props.dispatch({
			type : 'mngAuth/assignMenu',
		});
	}
	forReset(){
		this.props.dispatch({
...
//	}
//	getCheckedKey(){
//		let { data,selectedKeys } = this.props.mngAuth.menu;
...
//	}
	...
	getCheckedKey(){
		console.info('this.props.mngAuth.menu ',this.props.mngAuth.menu);
		const { data,selectedKeys } = this.props.mngAuth.menu;
		const temp = {};
		for(var key of selectedKeys){
			temp[key]=true;
		}
	...
	renderMenuNode(menus){
		const disable = this.props.mngAuth.roleId?false:true;
		var nodes = [];
...
export default connect(({mngAuth})=>({mngAuth}))(MenuTree);
