...
class MachineList extends React.Component {
	...
	render(){
		...
		const columns = [
		   ...
		   {title:'型号',  	dataIndex :'modelCode', key:'modelCode'}, 
		   {title: '操作',	key: 'action',			render: (text, record) => (
		        <span>
			        <Icon type="edit" style={{cursor:'pointer',color:'blue'}} onClick={ this.onEdit.bind(this,record) } />
			        <span className="ant-divider" />
			        <Icon type="delete" style={{cursor:'pointer',color:'blue'}} onClick={ this.onDelete.bind(this,record) } />
		        </span>
		      ),
		   	}
		];
