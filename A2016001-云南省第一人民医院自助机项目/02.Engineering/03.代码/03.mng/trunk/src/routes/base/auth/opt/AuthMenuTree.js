...
class MenuTree extends React.Component {
	...
	componentWillMount(props){
		this.props.dispatch({
			type : 'operatorAuth/loadMenus',
		});
//		if(this.props.operatorAuth.roleId != null){
//			this.props.dispatch({
//				type : 'operatorAuth/loadMenuKeys',
//			});
//		}
	}
	...
	onCheck(selectedKeys,{halfCheckedKeys}){//console.info(arguments,halfChecked);
		...
		var menu = {...this.props.operatorAuth.menu, selectedKeys:selectedKeys};
		this.props.dispatch({
...
//	}
//	getCheckedKey(){
//		let { data,selectedKeys } = this.props.operatorAuth.menu;
...
//	}
	...
	getCheckedKey(){
		console.info('this.props.operatorAuth.menu ',this.props.operatorAuth.menu);
		const { data,selectedKeys } = this.props.operatorAuth.menu;
		const temp = {};
		for(var key of selectedKeys){
			temp[key]=true;
		}
	...
	render(){//checkStrictly
		const { menu } = this.props.operatorAuth;
	...
	renderMenuNode(menus){
		const disable = this.props.operatorAuth.roleId?false:true;
		var nodes = [];
...
export default connect(({operatorAuth})=>({operatorAuth}))(MenuTree);
