...
class EditorForm extends React.Component {
	...
	render(){
		...
		const {parents} = this.props.modelManage;
		const options = parents.map(d => <Option key={d.id}>{d.name}</Option>);
		const {orgs} = this.props.org;
		const orgOptions = orgs.map(d => <Option key={d.id}>{d.name}</Option>);
		return (
			...
			<Form onSubmit={this.handleSubmit}>	
				...
				<FormItem style={{display:'none'}}>
				{
					getFieldDecorator('hospitalName',{ initialValue:data.hospitalName})(<Input />)
				}
				</FormItem>
				<FormItem style={{display:'none'}}>
				{
				...
				<Row>
					...
					<FormItem label="机构"  {...formItemLayout}>
				 	{
				 		getFieldDecorator('mngId',{
				 			initialValue : data.mngId,
...
export default connect(({machine, modelManage, org})=>({machine, modelManage, org}))(Editor);
