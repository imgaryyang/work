...
class Detail extends Component {
  ...
  constructor(props) {
    ...
    this.handleCallSync = this.handleCallSync.bind(this);
  ...
  handleCallBack() {
	  const { backTracking } = this.props;
	  const { details } = backTracking;
	  ...
	  Modal.confirm({
        ...
        onOk: () => { 
        	this.props.dispatch({
       	      	type: 'backTracking/handleCallBack',
       	      	payload: {...details}
   	  		});
    	},
      });
  ...
  handleCallSync() {
	  ...
	  Modal.confirm({
        ...
        onOk: () => { 
        	this.props.dispatch({
       	      	type: 'backTracking/handleCallSync',
       	      	payload: {...details}
   	  		});
    	},
      });
  ...
  render() {
    const { backTracking } = this.props;
    const { detailSpin, detailVisible, details } = backTracking;
    const hisColumns = [
      { title: '患者',
      ...
      >
        <Spin spinning={detailSpin} >
          <div >
            ...
            <Row>
				<Col span={10}> </Col>
				<Col span={1}><Button onClick={this.handleCallBack}>补记</Button></Col>
				<Col span={1}> </Col>
				<Col span={1}><Button onClick={this.handleCallSync}>同步</Button></Col>
				<Col span={10}> </Col>
			</Row>
          </div>
        </Spin>
