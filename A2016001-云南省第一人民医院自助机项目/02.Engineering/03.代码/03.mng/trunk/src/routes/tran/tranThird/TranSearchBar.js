...
class TranSearchBar extends Component {
  ...
  render() {
    const { form, thridTran } = this.props;
    const { getFieldDecorator } = form;
    const { query } = thridTran;
    return (
      <Form inline style={{ marginBottom: '10px' }} >
        ...
        <FormItem>
	      <Button onClick={this.handleAdd} size="large" >银行补录</Button>
	    </FormItem>
      </Form>
    );
  }
...
export default connect( ({ thridTran }) => ({ thridTran }),)(TranSearchBarForm);
