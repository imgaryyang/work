...
class TranDetail extends Component {
  ...
  handleCallBack() {
	  const { thridTran } = this.props;
	  const { details } = thridTran;
  ...
  render() {
    const { thridTran } = this.props;
    const { detailSpin, detailVisible, details } = thridTran;
    const hisColumns = [
      { title: '患者',
      ...
      >
        <Spin spinning={detailSpin} >
          <div >
            ...
            <Row>
				<Col span={11}> </Col>
				<Col span={1}><Button onClick={this.handleCallBack}>交易同步</Button></Col>
				<Col span={11}> </Col>
			</Row>
          </div>
        </Spin>
...
export default connect(({ thridTran }) => ({ thridTran }),)(TranDetail);
