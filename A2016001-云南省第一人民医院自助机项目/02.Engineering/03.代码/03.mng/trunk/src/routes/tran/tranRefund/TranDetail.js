...
class TranDetail extends Component {
  ...
  handleCallBack() {
	  const { refundTran } = this.props;
	  const { details } = refundTran;
  ...
  render() {
    const { refundTran } = this.props;
    const { detailSpin, detailVisible, details } = refundTran;
    const hisColumns = [
      { title: '患者',
...
export default connect(({ refundTran }) => ({ refundTran }),)(TranDetail);
