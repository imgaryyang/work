...
class TranCashMain extends Component {
  ...
  onSearch(values) {
    this.props.dispatch({
      type: 'cashTran/load',
      payload: {payChannelCode:"'0000'", ...values },
    });
  }

  showDetail(record) {
    this.props.dispatch({
      type: 'cashTran/loadDetails',
      payload: {
    	  briefRecord: record,
      },
    });
  ...
  render() {
    const { cashTran, base } = this.props;
    const { wsHeight } = base;
    const { spin, data } = cashTran;
        ...
        render: (value, record) => {
          return (
            <div>
              ...
              <Button onClick={() => this.handleCallBack(record)} size="small" >提交</Button>
            </div>
          );
        },
      },
...
export default connect( ({ cashTran, base }) => ({ cashTran, base }),)(TranCashMain);
