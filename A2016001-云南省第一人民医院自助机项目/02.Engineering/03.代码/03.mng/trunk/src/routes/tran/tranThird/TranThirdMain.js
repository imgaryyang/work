...
import TranAdditional 		from './TranAdditional';
import { order, settlement, prestore } from '../../base/Dict';

import styles 			from './TranThird.less';
...
class TranRefundMain extends Component {
  ...
  onSearch(values) {
    this.props.dispatch({
      type: 'thridTran/load',
      payload: {payChannelCode:"'9999','9998','0306','0308'", ...values },
    });
  }

  showDetail(record) {
    this.props.dispatch({
      type: 'thridTran/loadDetails',
      payload: {
    	  briefRecord: record,
      },
    });
  ...
  render() {
    const { thridTran, base } = this.props;
    const { wsHeight } = base;
    const { spin, data } = thridTran;
        ...
        render: (value, record) => {
          return (
            <div>
              ...
              <Button onClick={() => this.handleCallBack(record)} size="small" >交易同步</Button>
            </div>
          );
        },
      },
    ...
    return (
      <Spin spinning={spin} >
        <div style={{ height: `${wsHeight}px`, overflow: 'hidden' }} >
          <TranSearchBar onSearch={this.onSearch} showAdditional={this.showAdditional}/>
          <CommonTable
...
export default connect( ({ thridTran, base }) => ({ thridTran, base }),)(TranRefundMain);
