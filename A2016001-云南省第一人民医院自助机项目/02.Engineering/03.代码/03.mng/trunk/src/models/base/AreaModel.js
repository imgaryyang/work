...
import {loadAllAreas,deleteAllAreas,saveArea} from '../../services/base/AreaService';
...
export default {
  ...
  effects: {
    ...
    *load({ payload }, { select, call, put }) {
      ...
      const { data } = yield call(loadAllAreas);
      if (data) {
        yield put({
          type: 'init',
          data
    ...
    *save({ params }, { select, call, put }) {
      ...
      yield put({ type: 'addSpin' });
      const { data } = yield call(saveArea, params);

      if (data && data.success) {
        yield put({ type: 'load',});
        // 如果是新增，新增后将被选树状菜单节点设置为新增节点
        if (!params.id) {
          const { selectedNode } = yield select(state => state.areaManage);
    ...
    *deleteSelected({ payload }, { select, call, put }) {
      const checkedKeys = yield select(state => state.areaManage.checkedKeys);
      console.log('checkedKeys',checkedKeys);
      yield put({ type: 'addSpin' });
      const { data } = yield call(deleteAllAreas, checkedKeys);
      yield put({ type: 'removeSpin' });
