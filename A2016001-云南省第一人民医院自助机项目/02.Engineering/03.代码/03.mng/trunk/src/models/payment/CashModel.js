import { loadBatchs,loadDetails,exportBatchs, mngCreateBatch } from '../../services/payment/CashService';
...
export default {
  ...
  effects: {
    ...
    *save({ machineId }, { select, call, put }) {
    	const { data } = yield call(mngCreateBatch, machineId);
    	if (data && data.success) {
      	  yield put({
      		  type : 'setState',
      		  payload : { batchVisible: false },
      	  });
      	  yield put({
    		  type : 'loadBatchs',
    	  });
