...
class PayChannelSelect extends Component {

  static propTypes = {
  };

  render() {
    const { dispatch, filterOption, showSearch, payChannel, ...other } = this.props;
    const dictItems = payChannel.dicts;
    const options = [];
...
export default connect(({ payChannel }) => ({ payChannel }))(PayChannelSelect);
