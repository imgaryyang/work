package com.lenovohit.mnis.org.manager.impl;
...
import com.lenovohit.mnis.org.model.Account;
import com.lenovohit.mnis.org.model.Role;
import com.lenovohit.mnis.org.model.User;


@Repository("authorityMananger")
public class MnisAuthorityManangerImpl implements AuthorityMananger {
	...
	public List<? extends AuthPrincipal> getPrincipals(AuthAccount account) {
		if(account instanceof Account ){
			...
			return userDao.find("from User where id = ? and isActive = ?", act.getUser().getId(),"1");
		}
		return null;
	}
	...
	public List<? extends AuthRole> findRolesBySubject(AuthPrincipal principal) {
		if(principal instanceof User ){
			...
			return roleDao.find("select rela.role from UserRoleRela rela  where rela.user.id = ? and rela.user.isActive = ?", user.getId(),"1");
		}
