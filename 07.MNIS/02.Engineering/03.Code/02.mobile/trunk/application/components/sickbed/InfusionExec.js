...
import { infusionOrder, infusionOrderByBarcode } from '../../services/sickbed/Sickbed';
import { setCurrPatient } from '../../actions/base/BaseAction';

class InfusionExec extends Component {
  static displayName = 'InfusionExec';
  static description = '输液';
  ...
  setData(data, inpatientNo, barcode) {
    this.setState({
      data,
      refreshing: false,
      selected: barcode || this.state.selected,
    });
  }
  ...
  async fetchOrderByBarcode(barcode) {
    try {
      ...
      const responseData = await infusionOrderByBarcode(barcode);
  ...
  async fetchOrder(inpatientNo) {
    try {
      this.setState({ refreshing: true });
      const responseData = await infusionOrder(inpatientNo);
      if (responseData.success) {
        const data = responseData.result;
  ...
  dataHandle(data, inpatientNo, barcode) {
    if (data.length !== 0) {
      ...
      if (!currPatient) { // 当前患者不存在
        ...
        Alert.alert(
          '提示',
          `输液条码 ${barcode} 对应的患者 ${patient.name} 与当前所选患者 ${currPatient.name} 不一致。${'\n'}${'\n'}点击"确定"按钮切换患者，点击"取消"按钮放弃当前操作。`,
          [
  ...
  renderItem({ item, index }) {
    // console.log(item);
    const iconName = this.state.selected === item.barcode ? 'radio-button-checked' : 'radio-button-unchecked';
    const iconColor = this.state.selected === item.barcode ? Global.colors.IOS_BLUE : Global.colors.FONT_GRAY;
    ...
    return (
      <Item
        ...
        onPress={() => {
          // 如支持手选，把这段代码放开即可
          // this.setState({ selected: this.state.selected === item.barcode ? null : item.barcode });
        }}
        style={lastRow}
      >
...
InfusionExec.navigationOptions = ({ navigation, screenProps }) => {
  return {
    headerTitle: '输液执行',
    headerRight: (
      <ScannerButton
        type={ScannerButton.SCAN_INFUSION_BARCODE}
...
export default connect(mapStateToProps, mapDispatchToProps)(InfusionExec);
