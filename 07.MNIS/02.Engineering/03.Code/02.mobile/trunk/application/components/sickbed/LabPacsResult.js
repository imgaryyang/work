...
import { pacsOrderReult } from '../../services/sickbed/Sickbed';
import { setCurrPatient } from '../../actions/base/BaseAction';

class LabPacsResult extends Component {
  static displayName = 'LabPacsResult';
  static description = '检查结果';
  ...
  constructor(props) {
    ...
    this.fetchPacsOrder = this.fetchPacsOrder.bind(this);
  ...
  componentDidMount() {
    ...
    if (this.props.base.currPatient) {
      this.fetchPacsOrder(this.props.base.currPatient.inpatientNo);
    }
  }
  componentWillReceiveProps(nextProps) {
    ...
    if ((!currPatient && nextPatient) || (currPatient && nextPatient && currPatient.inpatientNo !== nextPatient.inpatientNo)) {
      this.setState({
        ...
        this.fetchPacsOrder(nextProps.base.currPatient.inpatientNo);
      });
    }
  }
  /**
  ...
  /**
     * 获取检查结果医嘱
     */
  async fetchPacsOrder(inpatientNo) {
    try {
      this.setState({ refreshing: true });
      const responseData = await pacsOrderReult(inpatientNo);
  ...
  showDetail(item, index) {
    ...
    this.props.navigation.navigate('LabPacsResultDetail', {
      orderNO: item.orderNO,
      name: item.name,
      part: item.part,
      type: item.type,
      memo: item.memo,
      barcode: item.barcode, // 申请单号
...
LabPacsResult.navigationOptions = ({ navigation, screenProps }) => {
  return {
    headerTitle: '检查结果',
    headerRight: (
      <ScannerButton
...
export default connect(mapStateToProps, mapDispatchToProps)(LabPacsResult);
