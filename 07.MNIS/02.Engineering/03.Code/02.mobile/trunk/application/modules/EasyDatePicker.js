...
class EasyDatePicker extends Component {
  ...
  static description = 'DatePicker Component';
  ...
  async pickTime(config, onPicked) {
    ...
    }${config['minute'] ? config['minute'] : ''}`).toDate();
    ...
    this.setState({
      ...
      is24Hour: true, // (config['is24Hour'] === true || config['is24Hour'] === false ? config['is24Hour'] : true),
      onPicked,
    });
    if (Platform.OS === 'ios') {
      this.toggle();
  ...
  onOk() {
    if (typeof this.state.onPicked === 'function') { this.state.onPicked(this.state.date); }
    this.clear();
  }
