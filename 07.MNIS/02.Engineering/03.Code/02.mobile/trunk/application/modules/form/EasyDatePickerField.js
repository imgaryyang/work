...
class EasyDatePickerField extends Component {
  ...
  static defaultProps = {
	...
	componentWillReceiveProps(props) {
	  if (props['value']) { this.setState({ value: props['value'] }, () => this._parseValue(props['value'])); } else { this.setState({ value: null }); }
	}
  ...
  /**
	...
	_generateValue() {
	  ...
	  if (this.state.date) {
	    let day = moment(this.state.date),
	      value;
  ...
  /**
     ...
     */
	_getClearIcon() {
	  const clearIcon = this.state.value ? (
  ...
  /**
	...
	_getField(_fontSize, _color, _fontWeight, _lineHeight, _textAlign) {
	  ...
	  const phStyle = t1 ? null : { color: 'rgba(187,187,187,1)' };
	  t1 = t1 || this.props.placeholder;

	  return (
      <View style={{ flex: 1, height: this._inputHeight, flexDirection: 'row' }} >
        <EasyDatePicker ref={c => this._picker = c} />
        <TouchableOpacity
          style={{ flex: 1, flexDirection: 'row', height: this._inputInnerHeight, alignItems: 'center', justifyContent: 'center' }}
	...
	render() {
	  const { fontSize, color, fontWeight, lineHeight, textAlign, ...other } = this._inputContainerStyle;
