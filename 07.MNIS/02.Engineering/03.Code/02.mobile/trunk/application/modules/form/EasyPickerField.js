...
class EasyPickerField extends Component {
  ...
  static propTypes = {
     ...
     * 相比普通的差异:
     * 1.动画由slide改为fade;
     * 2.ListView居中显示,高度自适应内容,最大高度maxListHeight,最大宽度maxListWidth;
     * 3.确认和取消按钮隐藏,选择一行即确认,点击Modal区域即取消;
     */
  ...
  componentWillReceiveProps(props) {
    if (props.value) { this.setState({ value: props.value }); } else { this.setState({ value: null, text: '' }); }
  }
  ...
  _onPicked(item) {
    ...
    if (typeof this.props.onChange === 'function') { this.props.onChange(this.props.name, `${item.value}`); }
  }
   ...
   */
  _getClearIcon() {
    const clearIcon = this.state.text ?
      (
  ...
  render() {
    const {
      fontSize, color, fontWeight, lineHeight, textAlign, ...other
    } = this._inputContainerStyle;
