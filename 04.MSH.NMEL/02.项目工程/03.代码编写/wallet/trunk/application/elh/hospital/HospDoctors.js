			...
			<TouchableOpacity onPress = {() => this.onPressDetail(row)} >
				<Card fullWidth = {true} >
					<View style = {[{flexDirection: 'row'}]} >
						<Portrait imageSource = {{uri: Global._host + IMAGES_URL + row.portrait,}} width = {70} height = {70} radius = {8} />
						<View style = {{flex: 1, paddingLeft: 10}} >
							<View style = {{flexDirection: 'row'}} >
								<Text style = {{flex: 1}} ><B>{row.name}</B></Text>
								<Text style = {{flex: 1, textAlign: 'right', marginRight: 8}}><B>科室 : </B>{row.deptName}</Text>
							</View>
							<Text style = {[Global._styles.GRAY_FONT, {marginTop: 5}]} ><B>职称 : </B>{row.jobTitle}</Text>
