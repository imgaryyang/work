...
class PatientsList extends Component {
	...
	**/
	...
	async fetchData () {
		...
		try {
			...
			//this.datas.concat(responseData.result);
	...
	*/
	...
	renderRow (item, sectionId, rowId, highlightRow) {
		let cardText = (
				<View >
					<Button theme = {Button.THEME.ORANGE} outline = {true} stretch = {false} style = {{width: 50, height: 25, marginLeft: 10}} onPress = {() => this.addCard()} >
						<Text style = {{fontSize: 12, color: Global._colors.ORANGE}} >去绑卡</Text>
					</Button>
				</View>
			);
		...
		return (
			<View style = {[styles.item]} >
				<TouchableOpacity style = {[styles.item, Global._styles.CENTER,{marginLeft:10,}]} onPress={() => this.cardList(item.patientId, item.name, item.cardCount) }>
