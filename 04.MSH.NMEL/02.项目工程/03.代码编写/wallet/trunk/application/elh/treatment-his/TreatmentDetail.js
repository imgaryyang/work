...
class TreatDetail extends Component {
				...
				<ScrollView style = {styles.scrollView} keyboardShouldPersistTaps = {true} >
					<View style = {styles.container}>
						<View style = {[styles.itemRow]}>
							...
							<Portrait imageSource = {{uri: (Global._host + 'el/base/images/view/' + this.props.hospLogo)}} width = {26} height = {26} />
							<Text style = {{marginLeft: 5}} >{this.state.hospitalName}</Text>
						</View>
						<View style = {[styles.itemRow]}>
							<Text style = {{flex: 1, marginLeft: 30, fontSize: 14,}}>{this.state.patientName}</Text>
						</View>
						<View style = {Global._styles.SEP_LINE} />
						{/*<EasyIcon name = 'ios-arrow-forward-outline' size = {18} color = {Global._colors.IOS_ARROW} style = {[{width: 40}]} />*/}
						<View style = {[styles.itemRow]}>
							<View style = {{flex: 3}}>
								<Text style = {{flex: 1, marginLeft: 10, fontSize: 14,}}>{this.state.departmentName}</Text>
							</View>
							{/*<View style = {styles.FULL_VER_LINE} />*/}
							<View style = {{flex: 7}}>
								<View style = {[styles.itemRow]}>
									<Text style = {[styles.itemRowText1]}>主治医生：</Text>
									<Text st yle = {[styles.itemRowText2]}>{this.state.doctorName}</Text>
								</View>
								<View style = {[styles.itemRow]}>
									<Text style = {[styles.itemRowText1]}>就诊时间：</Text>
									<Text style = {[styles.itemRowText2]}>{this.state.createTime}</Text>
								</View>
								<View style = {[styles.itemRow,]}>
									<Text style = {[styles.itemRowText1]}>诊断结果：</Text>
									<Text style = {[styles.itemRowText2,{color: Global._colors.IOS_RED}]}>{this.state.medcialResult}</Text>
								</View>
							</View>
	...
	renderChkItem (item, sectionID, rowID, highlightRow) {
		...
		return (
			<View style = {styles.container}>
				{topLine}
				<TouchableOpacity style = {[styles.item, Global._styles.CENTER,]} onPress = {()=>{this.doDetail(item.id, item.patientName);}}>
