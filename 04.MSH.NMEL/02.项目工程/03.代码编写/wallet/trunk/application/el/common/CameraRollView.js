...
class CameraRollView  extends Component{
    ...
    static defaultProps = {
        ...
        batchSize: 12,
        groupTypes: 'SavedPhotos',// 获取照片的分组类型，只在ISO中生效 'Album','All','Event','Faces','Library','PhotoStream', 'SavedPhotos',
  ...
  // rowData is an array of images
  	...
  	_appendAssets(data) {
    	...
    	if (assets.length > 0) {
  		...
  		// console.log("newState.assets===================="+newState.assets.length);
  	...
  	onSelect (photo,rowID,index) {
  		...
  		// for( var ii = 0;ii<this.state.assets.length;ii++){
  		// 	var photoItems = this.state.assets.slice( ii, parseInt( ii ) + 1 );
  		// 	var photoItem = photoItems[0];
  		// 	// console.log(photoItem);
  		// 	if(photoItem.node.image.uri==photo.node.image.uri)
  		// 		{ this.state.assets.splice(ii, 1,
  		// 			{ node:
				// 	    { timestamp: photo.node.timestamp,
				// 	      group_name: photo.node.group_name,
				// 	      type: photo.node.type,
    		...
    		<View style = {[Global._styles.CONTAINER]} >
              ...
              enableEmptySections = {true}
	        	...
	        	<View style = {{flexDirection: 'row', marginTop: 0,marginLeft: 10,marginRight: 10}} >
                    ...
                    <Button text = "保存" onPress = {()=>{this.confirm()}} clear={true} disabled={buttonState} />
                </View>	
	        </View>
