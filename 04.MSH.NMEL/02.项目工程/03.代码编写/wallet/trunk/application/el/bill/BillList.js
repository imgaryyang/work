...
class Bill extends Component {
	...
	render () {
		...
		let emptyView = this.state.user && this.state.user.id != "" && !this.state._refreshing && !this.state._pullToRefreshing && !this.state._requestErr && this.rowIds.length == 0 ? (
			<Card fullWidth = {true} style = {{paddingBottom: 20}} >
				<Text style = {[Global._styles.MSG_TEXT, {marginTop: 30, marginBottom: 20}]} >暂无账单信息</Text>
			</Card>
		) : null;
