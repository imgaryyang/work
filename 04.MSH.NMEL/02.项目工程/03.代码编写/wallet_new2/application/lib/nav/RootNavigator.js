...
export default class RootNavigator extends Component {
    ...
    state = {
		subNavTopPos: new Animated.Value(Dimensions.get('window').height),
		showSubNav: false,
		subRoute: null,
    };
    ...
    */
    ...
    componentDidMount () {
		...
		if(Global._os == 'android') {
			BackAndroid.addEventListener('hardwareBackPress', () => {
				...
				*/
				...
				this._nav.pop();
				let curRouter =  routers[routers.length - 2];
				if(curRouter.pullToRefresh){
					curRouter.pullToRefresh();
				}
				return true;
			});
		}
		this.pop = () =>{
			this._nav.pop();
			let routers = this._nav.getCurrentRoutes();
			let curRouter =  routers[routers.length - 2];
			if(curRouter.pullToRefresh){
				curRouter.pullToRefresh();
			}
    ...
    */
    _getSceneConfig () {
    	return Global._os == 'ios' ? Navigator.SceneConfigs.PushFromRight : Navigator.SceneConfigs.PushFromRight;
    }
