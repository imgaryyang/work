		...
		return (
			<View style = {Global._styles.CONTAINER} >
				...
				<View style = {{alignItems: 'center'}}>
					<Image style = {{width:344, height:24, marginTop:15, margin:8}}  source={require('../res/images/wallet/bindcard1.png')} />
				</View>
				<ScrollView automaticallyAdjustContentInsets = {false} keyboardShouldPersistTaps = {true} style = {styles.scrollView} >
					<Form ref = {(c) => this.form = c} config = {FormConfig} showLabel = {this.state.showLabel} labelPosition = {this.state.labelPosition}
						...
						<Form.TextInput name = "cardNo" autoFocus={true} label = "  卡号" minLength = {16} maxLength = {19}
							 placeholder = "请输入卡号" dataType = "bankAcct" value= "6228480402564890015"/>
					</Form>
					<View style={styles.textDescView}>
						
						<View>
							<Text style={styles.textStyle}>注意事项</Text>
					...
					<View style = {{flexDirection: 'row', margin: 15, marginTop: 40,}} >
						<Button text = "下一步" style={{backgroundColor: Global.Color.RED,height: 48, borderWidth: 0,}} disabled = {this.state.disabled} onPress={() => this.submit()}/>
					</View>
				</ScrollView>
			</View>
		);
