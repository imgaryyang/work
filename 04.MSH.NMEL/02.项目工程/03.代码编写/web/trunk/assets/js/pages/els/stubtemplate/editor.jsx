...
class EditorForm extends Component {
			...
			{	title: '明细项',dataIndex: 'item',
				render: (text,d,index) => {
					...
					if(this.state.data.length==index+2) 
						...
						/>;
					else return <Input type="text" maxLength={48} {...setName('item'+index,{initialValue:text})} onBlur={this.onItemChange.bind(this,index)}/>;
				}
			}, 
			{	title: '是否金额项',dataIndex: 'isAmt',
				render: (text,d,index) => {					
					if(this.state.data.length==index+1 || index==0)
						...
						return <Checkbox checked={text=='1'?false:true} onChange={this.onIsAmtChange.bind(this,index)}>非金额项</Checkbox>
				}
			}, 
	...
	componentWillMount () {
		...
		let fetch = Ajax.get('api/els/stubtemplate/detail/'+this.props.data.id, null, {catch: 3600});
		fetch.then(res => {
			if(res.success){
				data = res.result;
				data.splice(data.length-1,0,{item:'',isAmt:'1',ioFlag:'1'});
	...
	onSubmit(e){
		...
		let fetch = Ajax.put('api/els/stubtemplate/update', data, {
	...
	render () {
		...
		return (<Form>
		        	<Row gutter={16} type='flex' align='middle' justify='center' style={{marginBottom:'10px'}} >
			        	<Col span={14}>
			        	<Input {...setName('template',{initialValue:this.props.data.template})} addonBefore="模板名" type="text" maxLength={48}  />
			        	</Col>
			        </Row>
		        	<div style={{boder:'thin solid red'}}></div>
					<Row type='flex' align='middle' justify='center' >
			        	<Col span={18}><Table columns={this.state.columns} dataSource={this.state.data} size="small" pagination={{pageSize:31}}/></Col>
			        </Row>
