...
import com.infohold.api.InfoholdCipherAPI;
import com.infohold.api.InfoholdDisposePinAPI;
import com.infohold.api.InfoholdOTPAPI;
import com.infohold.api.InfoholdPassWordAPI;
import com.infohold.bdrp.tools.security.SecurityHandle;
import com.infohold.handle.InfoholdHandle;
...
public class SecurityHandleImpl implements SecurityHandle {
	...
	InfoholdHandle handle = new InfoholdHandle(host, appId, timeout);
	
	InfoholdPassWordAPI pwdApi = new InfoholdPassWordAPI(handle);
	InfoholdOTPAPI otpApi = new InfoholdOTPAPI(handle);
	InfoholdDisposePinAPI disposeApi = new InfoholdDisposePinAPI(handle);
	InfoholdCipherAPI cipherAPI = new InfoholdCipherAPI(handle);
	...
	public String genCheckCode(String type, String sed, int count, long validInTime, Map<String, ?> params) {
		...
		try {
			handle.openHandle();
			checkCode = otpApi.infoholdGenSMSCodeBytime(sed, count, 6, 10);
		} catch (Exception e) {
			log.error(e);
		}
		return checkCode;
	...
	public boolean verifyCheckCode(String type, String sed, String code, int count, long validInTime,
		...
		try {
			handle.openHandle();
			isValid = otpApi.infoholdVerSMSCodeBytime(sed, code, count, 6, 10);
		} catch (Exception e) {
			log.error(e);
		}
		return isValid;
	...
	public String encryptData(String type, String sed, String plain, String masterKeyName, Map<String, ?> params) {
		...
		try {
			handle.openHandle();
			result = cipherAPI.infoholdEncUserData(sed, plain, Integer.valueOf(type), masterKeyName);
		} catch (Exception e) {
			log.error(e);
		}
	...
	public String decryptData(String type, String sed, String patterned, String masterKeyName, Map<String, ?> params) {
		...
		try {
			handle.openHandle();
			result = cipherAPI.infoholdDecUserData(sed, patterned, Integer.valueOf(type), masterKeyName);
		} catch (Exception e) {
			log.error(e);
		}
		return result;
