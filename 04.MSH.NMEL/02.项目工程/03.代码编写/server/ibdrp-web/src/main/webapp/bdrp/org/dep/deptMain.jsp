...
var $CTX = '${ctx}';
	$(document).ready(function(){
		...
		$('#tree').treegrid({
			...
			columns:[[
				...
				{title : '编号',field : 'code',width:'250'},
				{title : '描述',field : 'description',width:'200'},
				{title : '操作',field : 'aa',width:'150'}
			]],
	...
	function pagerFilter(data) {
		...
		$('#tree').treegrid('options').url = '${ctx}/bdrp/org/dep/page';
	...
	function loadChildren(row) {
		if (row) {
			$('#tree').treegrid('options').url = '${ctx}/bdrp/org/dep/page?parentId=' + row.id;
		}
	}
	...
	// 新增功能----跳转到新增页面
	function doAdd(){
		$('.one-center-panel').load('${ctx}/bdrp/org/dep/deptForm.jsp');
	...
	function doClickCell(field, row){
		if(field == "operate"){	// 如果点击的是操作那列
			...
			$('.one-center-panel').load('${ctx}/bdrp/org/dep/addOrg.jsp?parentId=' + id);
	...
	function doDblClickRow(row){
		var id = row.id;
		$('.one-center-panel').load('${ctx}/bdrp/org/dep/edit/' + id);
	...
	function doDelete(){
		...
		$.messager.confirm("确认删除", "你确定要删除选中项吗？", function(flag){
			if(flag == true){
				// 得到所有选中的ID值
				var id = arr[0].id;
				var url = "${ctx}/bdrp/org/dep/remove/" + id;				
				// Ajax异步提交
				$.ajax({
					type : "DELETE",
					url : url,
