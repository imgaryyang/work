...
$(document).ready(function(){
	...
	$('#parent').combotree({
		url : '${ctx}/bdrp/auth/access/functionTree.do',
		required : false
	});
	...
	$('.turn-page>a').on('click', function(){
		...
		$('#authzForm').form('submit',{
			url : '${ctx}/bdrp/auth/access/authzMain.ihp?start=' + start,
	...
	function search2(){
		...
		/* } */
		$('.one-center-panel').load('${ctx}/bdrp/auth/access/authzMain.ihp',{name:name,desc:desc,func:func});
	}
	...
	function del(id){
		...
		$.ajax({
		type :"POST",
		url : "${ctx}/bdrp/auth/access/delAuthz.ihp",
		data : {id : id},
		dataType : "html",
		success : function(msg){
			if(msg == "success"){
				$.messager.alert("提示","删除成功","info");
				$(".one-center-panel").load('${ctx}/bdrp/auth/access/authzMain.ihp');
			}
		}
	});
	}
	...
	function edit(id){
		$('.one-center-panel').load('${ctx}/bdrp/auth/access/editAuthz.ihp',{id:id});
	}
	...
	function PageUp(start){
		...
		$(".one-center-panel").load('${ctx}/bdrp/auth/access/authzMain.ihp',{authzid:authzid,start:start,name:name,desc:desc,func:func});
...
<table class="l1-tab" style='width:100%;'>
	...
	<tbody>
		<c:forEach var="list" items="${pagebean.result}"  varStatus="status">
					...
					<c:if test='${fn:length(list.property) == 0 }'>
					 	<td class=l1-col-info ></td>
					 	</c:if>
					 	<c:if test="${fn:length(list.property) != 0 }">
					 	<td class=l1-col-info >${list.property}</td>
					 	</c:if>
