...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//	iconCls : "icon-forward",	// 左上边出现一个空白图片，这样是标题文字有一一格，为了样式好看些
			...
			columns : [[
				...
				{field : 'accessName', title : '授权', width : 100},
	...
	function pagerFilter(data) {
		...
		$('#tree').treegrid('options').url = '${ctx}/bdrp/auth/menu/page?method='+method+'&context='+context;
		...
		if(null != data.result){
			for(var i=0;i<data.result.length;i++){
				...
				data.rows[i].accessName = data.result[i].access.name;
				data.rows[i].state= "closed";
				if(data.result[i].parentId){
					data.rows[i]._parentId = data.result[i].parentId;
				}
	...
	function loadChildren(row) {
		if (row) {
			$('#tree').treegrid('options').url = '${ctx}/bdrp/auth/menu/page?parentId=' + row.id;
		}
	}
	...
	// 新增功能----跳转到新增页面
	function doAdd(){
		$('.one-center-panel').load('${ctx}/bdrp/auth/menu/add.jsp');
	...
	function doClickCell(field, row){
		if(field == "operate"){	// 如果点击的是操作那列
			...
			$('.one-center-panel').load('${ctx}/bdrp/auth/menu/add.jsp?parentId=' + id);
	...
	function doDblClickRow(row){
		var id = row.id;
		$('.one-center-panel').load('${ctx}/bdrp/auth/menu/edit/' + id);
	...
	function doDelete(){
		...
		$.messager.confirm("确认删除", "你确定要删除选中项吗？", function(flag){
			if(flag == true){
				// 得到所有选中的ID值，并拼装字符串
				var id = arr[0].id;
				var url = "${ctx}/bdrp/auth/menu/remove/" + id;	
