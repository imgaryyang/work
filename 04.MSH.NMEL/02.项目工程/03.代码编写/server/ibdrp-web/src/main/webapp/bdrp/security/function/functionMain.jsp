...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//	iconCls : "icon-forward",	// 左上边出现一个空白图片，这样是标题文字有一一格，为了样式好看些
			...
			url : '${ctx}/puborgauthz/rootFunctionList.do',
	...
	function pagerFilter(data) {
		...
		$('#tree').treegrid('options').url = '${ctx}/puborgauthz/rootFunctionList.do?method='+method+'&context='+context;
	...
	// 新增功能----跳转到新增页面
	function doAdd(){
		$('.one-center-panel').load('${ctx}/puborgauthz/addFunction.ihp');
	...
	function doClickCell(field, row){
		...
		if(field == "operate"){	// 如果点击的是添加那列
			...
			$('.one-center-panel').load('${ctx}/puborgauthz/addFunction.ihp?id=' + id);
