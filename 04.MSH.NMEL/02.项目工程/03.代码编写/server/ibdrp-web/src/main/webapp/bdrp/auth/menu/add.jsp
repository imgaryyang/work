...
<script type="text/javascript">
	...
	//获取浮动框的数据并保存权限----得到第一个选择的数据
	function saveLinkAuthAccess(){
		var info = "";
		$("table tbody  input[type='checkbox']:checked").each(function(){
			info += $(this).val() + ",";
		});
		var arr = info.split(",");
		var authAccessId = arr[0];
		
		if(authAccessId != null && authAccessId.trim() != ""){
			// 得到该ＩＤ对应的权限名称
			var dataArr = msg.split(",");
			$("#accessId").val(dataArr[0]);
			$("#accessName").val(dataArr[1]);
			// 关闭浮动框
			$("#accessWindow").window('close');
		}else {
	...
	function conditionSearch(){
		// 名称
		var name = $("#authAccessName").val();
		// 功能
		var functionName = $("#authAccessFunction").val();
		// 备注
		var descp = $("#authAccessDescp").val();
	...
	// 浮动框----翻页
	function reloadAccess(start, limit){
		var name = $("#authAccessName").val();
		var functionName = $("#authAccessFunction").val();
		var descp = $("#authAccessDescp").val();
		
		var url = "${ctx}/bdrp/auth/menu/page?name="+name
...
<div class="box-container" style="display:inline-block;">
	<form method='post' id='menuForm' name='menuForm' action='' >
		...
		<div class="f-div-row">
			...
			<div class="f-div-input-c4">
				<input id="code" type="text" class=c-ff-input name="code" _nullable=false _nullablemsg=请填写编码  _maxlength=50 _maxlengthmsg='最大长度为50'>	<br/>
		...
		<div  class="f-div-row">
			...
			<div class="f-div-input-c4">
				<input id="name" type="text" class=c-ff-input name="name" _nullable=false _nullablemsg=请填写名称  _maxlength=50 _maxlengthmsg='最大长度为50'>
			...
			<div class="f-div-label-c4"><label>对应权限<font color="red">&nbsp;*&nbsp;</font>:</label></div>
			<div class="f-div-input-c4">
				 <input id="accessId" type="hidden" class=c-ff-input name="access.id">
				 <input id="accessName" type="text" class=c-ff-input readonly="readonly" name="acces.name" _nullable=false _nullablemsg=请填写名称>
				<p id="accessId1" style="display : none;"><font color="red">* 对应权限必须选择</font></p>
			</div>
		</div>
		<div  class="f-div-row">
			<div class="f-div-label-c4"><label>图标:</label></div>
			<div class="f-div-input-c4">
				<input id="iconPath" type="text" class=c-ff-input  name="iconPath">
			</div>
		...
		<div  class="f-div-row">
			...
			<div class="f-div-input-c4">
				<textarea id="descp" name="descp" rows="3" cols="20" _maxlength=255 _maxlengthmsg='最大长度为255'></textarea>
			</div>
		</div>
