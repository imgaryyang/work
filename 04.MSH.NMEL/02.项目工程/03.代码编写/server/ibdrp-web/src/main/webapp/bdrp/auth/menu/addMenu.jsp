...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("#serviceName").click(function(){
			...
			$("#serviceWindow").window({
				...
				modal : true,
				resizable : false,
				href : "${ctx}/bdrp/auth/menu/linkedAuthService.ihp",
	...
	function saveLinkAuthzService(){
		...
		var arr = info.split(",");
		var authzAccessId = arr[0];
		
		if(authzAccessId != null && authzAccessId.trim() != ""){
			// 得到该ＩＤ对应的服务名称
			$.ajax({
				type : "POST",
				url : "${ctx}/bdrp/auth/menu/getAuthzService.do",
				data : "id="+authzAccessId,
				success : function(msg){
					if(msg == "error"){
	...
	// 无任何条件查询
	function search(){
		var url = "${ctx}/bdrp/auth/menu/linkedAuthService.ihp?start="+0+"&limit="+10;
		// 通过Ajax读取
	...
	function conditionSearch(){
		// 名称
		var name = $("#authzAccessName").val();
		// 功能
		var functionName = $("#authzAccessFunction").val();
		// 备注
		var descp = $("#authzAccessDescp").val();
		
		var url = "${ctx}/bdrp/auth/menu/linkedAuthService.ihp?name="+name
...
<div class="box-container" style="display:inline-block;">
	<form method='post' id='menuForm' name='menuForm' action='' >
		...
		<div  class="f-div-row">
			...
			<div class="f-div-input-c4">
				 <input id="serviceId" type="hidden" class=c-ff-input name="acces.id">
				 <input id="serviceName" type="text" class=c-ff-input readonly="readonly" name="acces.name" _nullable=false _nullablemsg=请填写名称>
		...
		<div  class="f-div-row">
			...
			<div class="f-div-input-c4">
				<input id="iconPath" type="text" class=c-ff-input readonly="readonly" name="iconPath">
			</div>
