...
class Bill extends Component {
  ...
  renderRow(item, sectionId, rowId, highlightRow) {
    ...
    return (
      <View>
        <TouchableOpacity key={rowId} style={[Global._styles.CENTER, styles.item]}
                          onPress={() => this.onPressDetail(item, this.getIconConfig(item.billType))}>
          ...
          <View style={{width: 65, paddingLeft: 10}}>
            ...
            }}>{Filters.filterDateTimeToMonthAndDay(item.createTime)}</Text>
            <Text style={{
              fontSize: 12,
              color: Global._colors.FONT_LIGHT_GRAY
            }}>{Filters.filterDateTimeToTime(item.createTime)}</Text>
          </View>
          <View style={{flex: 1, paddingLeft: 20}}>
            <View style={{flexDirection: 'row',}}>
              ...
              <View style={{alignItems: 'flex-end', width: 60}}>
                <Text style={{
                  fontSize: 12,
                  color: this.getStatusConfig(item.status).color
                }}>{this.getStatusConfig(item.status).title}</Text>
              </View>
            </View>
