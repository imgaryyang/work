...
// 操作系统 ios | android
export const _os = Platform.OS;

// 设备像素比
export const _pixelRatio = PixelRatio.get();

// 导航栏高度
export const _navBarHeight = Platform.OS === 'ios' ? 64 : 44;
...
// 后台服务地址
export const _host = 'http://110.76.186.37/api/';
export const _iSEB_host = _os === 'ios' ? 'http://localhost:9000/' : 'http://10.12.253.5:9000/';
export let _hostTimeout = 5000;

// 用户当前位置
export const _currLocation = { code: '110100', name: '北京' };
...
export async function init() {
  try {
    ...
    const hostTimeoutInAS = await AsyncStorage.getItem(_ASK_HOST_TIMEOUT);
    const userInAS = await AsyncStorage.getItem(_ASK_USER);
    const bankCardsInAS = await AsyncStorage.getItem(_ASK_USER_BANKCARDS);
