	...
	async fetchData () {
		...
		try {
			...
			if(!responseData.success){
				return this.toast(responseData.msg);
			}
		...
		return (
			<View style = {Global._styles.CONTAINER} >
				...
				<ScrollView keyboardShouldPersistTaps = {true} >
					<View style = {[Global._styles.CENTER, {flexDirection: 'row', margin: 8, marginBottom: 0}]} >
						<SearchInput 
		...
		return (
			<View style = {[styles.item]} >
				<TouchableOpacity style = {[styles.item, Global._styles.CENTER,]} >
					...
					<View style={{flex:1,}}>
						<Text style = {[styles.bankBranchName,{flex: 1, marginLeft: 10, fontSize: 13, color: Global._colors.FONT_GRAY}]}>
							{item.name}
						</Text>
						<Text style = {{flex: 1, marginLeft: 10, fontSize: 12, color: Global._colors.FONT_LIGHT_GRAY}}>
							{item.contactWay}
						</Text>
						<Text style = {{flex: 1, marginLeft: 10, fontSize: 10, color: Global._colors.FONT_LIGHT_GRAY}}>
							{item.address}
						</Text>
					</View>
				</TouchableOpacity>
