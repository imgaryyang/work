...
class HospHome extends Component {
	...
	render () {
		...
		let emptyView = !this.state._refreshing && !this.state._requestErr && (!this.state.contacts || this.state.contacts.length == 0) ? (
			<Text style = {[Global._styles.MSG_TEXT, {marginTop: 30, marginBottom: 20}]} >暂无联系方式</Text>
		) : null;
		...
		return (
			<View style = {Global._styles.CONTAINER} >
				...
				<Card >
					...
					{this.getLoadingView('正在载入联系方式...', this.fetchContcatData)}
					{emptyView}
					{this.renderContacts()}
				</Card>
