...
class CreateForm extends Component{
	...
	save(data){
		var scope = this;
		//医院级别特殊处理
		let fetch = Ajax.post('api/el/base/news/org/create', data, {
		...
		return (
			<Form horizontal>
				...
				<FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 12 }} label="标题" >
	        		<Input placeholder="标题" {...getFieldProps('caption')} />
	        	</FormItem>
	        		<FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 12 }} label="作者" >
	        		<Input placeholder="作者" {...getFieldProps('feededBy')} />
	        	</FormItem>
	        	<FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 12 }} label="摘要" >
	        		<Input type="textarea" rows={4} placeholder="摘要" {...getFieldProps('digest')} />
	        	</FormItem>
	        	{
	        		(this.state.image)?(
	        	...
	        	<FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 12 }} label="正文" >
	        		<Input type="textarea" rows={10} placeholder="正文" {...getFieldProps('body')} />
	        	</FormItem>
