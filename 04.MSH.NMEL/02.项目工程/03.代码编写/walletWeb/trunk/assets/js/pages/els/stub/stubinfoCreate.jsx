...
class CreateForm extends Component {
	...
	onSubmit(e){
		...
		let fetch = Ajax.post('api/els/stubbatchinfo/create', data, {
		...
		fetch.then(res => {
			if(res.success){
				...
				this.setState({ optionEl:options, selSet:setName('person',{
							initialValue:data[0].id+'-'+data[0].name+'-'+data[0].idNo,
						})
				});
		    	return res;
		    }
	...
	render () {
		...
		let stubinfoItems = this.state.data.map((domain) => {
	        return <Col span={10}><FormItem {...formItemLayout} label={domain.item} >
	        		<Input {...setName(`item${domain.seqNo}`,{initialValue:''})}  />
	        	</FormItem></Col>;
	      });
		        ...
		        <Form horizontal onSubmit={this.onSubmit.bind(this)}>
		        	<Row gutter={16} type='flex' align='middle' justify='center' >
			        	...
			        	<Col span={19} ><FormItem labelCol={{span:2}} wrapperCol={{span:20}} label="备注">
			        		<Input type='text' {...setName('note',{initialValue:''})}/>
			        	</FormItem></Col>
			        </Row>
			        ...
			        <FormItem style={{textAlign:'center'}}>
			        	<Button type="primary" style={{marginRight:'3px'}} htmlType="submit">提交</Button>
...
const BatchinfoCreate = Form.create()(CreateForm);

module.exports = BatchinfoCreate;