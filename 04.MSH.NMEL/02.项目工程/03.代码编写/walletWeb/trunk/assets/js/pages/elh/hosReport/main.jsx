'use strict';

import { Component } from 'react';
import ReactDOM from 'react-dom';
import NavCard from '../../../components/NavCard.jsx';
import List from '../../el/news/list.jsx';
import Create from '../../el/news/create.jsx';
import Editor from '../../el/news/editor.jsx';
module.exports = class DoctorMain extends Component {
	
  constructor (props) {
    super(props); 
    this.state={
    	listVersion:1,
    	config : {fkType:"H1"},
    	items:[]
    }
    this.state.items.push(this.getHomeCmp());
  }
  getHomeCmp(){
	  let personList=(<List key = 'list' ref='list' name = '院报' config={this.state.config}  version={this.state.listVersion}
			  onCreate={this.createModel.bind(this)}
			  onEdit={this.editorModel.bind(this)}/>);
	  return personList;
  }
  back(){
	  this.state.items.pop();
	  this.setState({items:this.state.items});
  }
  refreshList(){
	  this.refs['list'].refresh();
//	  this.state.items=[];
//	  this.state.listVersion++;
//	  this.state.items[0] = this.getHomeCmp();
//	  this.setState({items:this.state.items,listVersion:this.state.listVersion});
  }
  selectCard(card){ 
	  var items=[],i=0;
	  for(i=0;i<this.state.items.length;i++){
		  items.push(this.state.items[i]);
		  if(this.state.items[i].key ==card.key)break;
	  }
	 // this.state.items.splice(i,(items.length-i-1));
	  this.state.items = items;
	  this.setState(this.state.items );
  }
  createModel(){
	  this.state.items.push(<Create onClose = {this.back.bind(this)} data={this.state.config} refreshList={this.refreshList.bind(this)} key = 'newDoctor'name = '新增'/>);
	  this.setState(this.state.items);
  }
  editorModel(data){
	  this.state.items.push(<Editor data={data} onClose = {this.back.bind(this)} refreshList={this.refreshList.bind(this)} key = 'editDoctor' name='编辑'/>);
	  this.setState(this.state.items);
  }
  render () {
	return (
    	<NavCard onBack={this.back.bind(this)} onSelect={this.selectCard.bind(this)} >
    		{this.state.items}
    	</NavCard>
    ) 
  }
}
