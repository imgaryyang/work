...
var TopGridMenu = React.createClass({
	...
	render: function() {
		...
		var items = this.props.items.map(
			({text, onPress}, idx) => {
				...
				return (
					<TouchableOpacity key={'TGM_' + idx} style={[Global.styles.CENTER, this.getStyles().item, itemStyle, firstItem, lastItem]} onPress={(item) => this.onPress(idx, text, onPress)} >
		...
		return (
			<View style={[Global.styles.CENTER, this.getStyles().holder]} >
				{items}
			</View>
		);
	},
