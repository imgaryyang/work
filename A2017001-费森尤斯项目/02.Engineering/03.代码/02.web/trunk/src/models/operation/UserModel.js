...
export default {
  ...
  effects: {
    ...
    *loadUserAccounts({ payload }, { select, call, put }) {
      const record = yield select(state => state.user4Opt.record);
      yield put({ type: 'setState', payload: { spin: true } });
    ...
    *saveUserAccount({ payload }, { select, call, put }) {
      const user = yield select(state => state.user4Opt.record);
    ...
    *load({ payload }, { select, call, put }) {
      ...
      const defaultPage = yield select(state => state.user4Opt.page);
    ...
    *loadUserInfo({ payload }, { call, put }) {
      ...
      if (data && data.success && data.result.length) {
        ...
        yield put({ type: 'loadDptListData', payload: { query: { hosId: record.hosId } } });
      }
      yield put({
        type: 'setState',
        payload: {
