...
export default {
  namespace: 'instrmStoreInfoQuery',
  ...
  effects: {
    *load({ payload }, { select, call, put }) {
      ...
      const defaultPage = yield select(state => state.instrmStoreInfoQuery.page);
      const activeKey = yield select(state => state.instrmStoreInfoQuery.activeKey);
      ...
      const { data } = yield call(loadStoreInfoPage, start, pageSize, query);
