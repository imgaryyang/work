...
export default {
  ...
  effects: {
    ...
    *loadDetail({ payload }, { select, call, put }) {
      ...
      const defaultRecord = yield select(state => state.instoredetail.record);
      ...
      searchObjs = { ...searchObjs, ...query };
      page = { ...defaultPage, ...page };
      const newRecord = { ...defaultRecord, ...record };
      const { pageNo, pageSize } = page;
      const start = (pageNo - 1) * pageSize;
      yield put({ type: 'setState', payload: { isSpin: true, record: newRecord } });
