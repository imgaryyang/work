...
export default {
  ...
  effects: {
     *loadBack({ payload }, { select, call, put }) {
      ...
      let searchObjs = yield select(state => state.phaRecipe.searchObjs);
    ...
    *recipe({ id }, { call, put }) {
      yield put({ type: 'toggleSpin' });
      const { data } = yield call(phaRecipeService.recipe, id);
      yield put({ type: 'toggleSpin' });
      if (data && data.success) {
         yield put({ type: 'loadBack' });
      }
    },
    *back({ id }, { call, put }) {
      yield put({ type: 'toggleSpin' });
      const { data } = yield call(phaRecipeService.back, id);
      yield put({ type: 'toggleSpin' });
      if (data && data.success) {
        yield put({ type: 'loadBack' });
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(({ pathname }) => {
        ...
        if (pathname === '/dispensary/phaRecipeBack') {
          dispatch({
            type: 'loadBack',
          });
          dispatch({
