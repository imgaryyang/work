	...
	effects: {
	  ...
	  //直接入库searchBar查询
	    ...
	    *saveDirectIn({ inputState }, { select, call, put }) {
	    	...
	    	const commStr = yield select(state => state.directIn.commStr);
...
//	    			o.saleCost = o.inSum * o.salePrice;//入库金额
    		      	   	...
    		      	   	yield put({ 
    		      			...
    		      			payload: { 
    		      				bizPrintAlertParams: { 
    		      					...
    		      					bizCode: inputData[0].inBill, // 业务单据编号
    		      					bizCodeLabel: '入库单号', // 业务单据编号名称
    		      					bizTip: '直接入库成功' // 业务操作成功提示
    		      				}
    		      			}
    		      		});
	    	...
	    	yield put({type: 'setState',payload:{spin:false, commStr:''}});
	...
	reducers: {
		"init"(state,{payload}) {
			var {data,page,deptId,hosId,userName,tradeName,newData} = (payload||{});
			...
			var data=result||[];
			var newData=result||[];
			return { ...state,data:data, newData, page:newPage,deptId:deptId,hosId:hosId,userName:userName,tradeName:tradeName,};
		},
