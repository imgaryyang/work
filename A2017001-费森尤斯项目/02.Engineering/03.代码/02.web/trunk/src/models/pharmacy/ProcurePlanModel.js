...
export default {
  ...
  effects: {
    * listStockWarnDurg({ payload }, { select, call, put }) {
      yield put({ type: 'setState', payload: { isSpin: true } });

      const { query } = (payload || {});

      // 查询按库存定义需要采购的药品
      const { data: ret } = yield call(buyBillService.listStockWarnDurg, query);

      if (ret && ret.success) {
        yield put({ type: 'setState', payload: { visible: false } });
        const details = ret.result;
        for (let i = 0; i < details.length; i++) {
          const data = details[i].drugInfo;
          ...
          let newData = {
            ...
            buyNum: details[i].buyNum,
            buyCost: details[i].buyNum * data.buyPrice,
            plusMinus: '1',
            buyBill: buy.buyBill,
          };
    ...
    * loadDrug({ payload }, { select, call, put }) {
      ...
      if (data && data.success) {
        ...
        procurePlan.newData = data.result;
      ...
      if (buy.buyDetail && buy.buyDetail.length > 0) {
        ...
        if (ret) {
          if (ret.success) {
            ...
            // yield put({ type: 'print/getPrintInfo', payload: {code:'013', bizId: bubBillData.buyBill} });
            ...
            yield put({
              ...
              payload: {
                bizPrintAlertParams: {
                  ...
                  bizTip: '采购计划提交成功', // 业务操作成功提示
                },
              },
            });

            /*
            var r=confirm("采购单提交成功，是否打印采购单？\n\n" + '采购单号: ' + bubBillData.buyBill);
              if (r==true)
                {
               yield put({ type: 'print/getPrintInfo', payload: {code:'013', bizId: bubBillData.buyBill} });
            ...
            /*
            Modal.confirm({
                title: '采购单提交成功，是否打印采购单？',
                content: '采购单号: ' + bubBillData.buyBill,
                okText: '现在打印',
                cancelText: '暂不打印',
                onOk(){
                  yield put({ type: 'print/getPrintInfo', payload: {code:'013', bizId: bubBillData.buyBill} });
    ...
    * deleteBuy({ paload }, { select, call, put }) {
      yield put({ type: 'setState', payload: { isSpin: true } });
      const { buy } = yield select(state => state.procurePlan);
