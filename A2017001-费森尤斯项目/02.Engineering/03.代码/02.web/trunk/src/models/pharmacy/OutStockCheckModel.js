...
// 1、加载请领信息
    *loadAppIn({ payload }, { select, call, put }) {
      ...
      const outStockCheck = yield select(state => state.outStockCheck);
      const { defaultPage, query } = outStockCheck;
    ...
    *addOutStockDetail({ record, payload }, { select, call, put }) {
      yield put({ type: 'setState', payload: { spin: true } });
      const recordTem = yield select(state => state.outStockCheck.record);
      ...
      const outStockCheck = yield select(state => state.outStockCheck);
      const { defaultPage, queryRight } = outStockCheck;
...
// 3.保存出库信息，调用出库接口
    *saveOutStockInfo({ value }, { select, call, put }) {
      ...
      const { dataOutStock, record, data: appData, page } = yield select(state => state.outStockCheck);
		...
		// 弹出业务单据打印提示
  	   	yield put({ 
  			...
  			payload: { 
  				bizPrintAlertParams: { 
  					visible: true, 
  					tmplateCode: '017', // 模版编号 
  					bizCode: retData[0].outBill, // 业务单据编号
  					bizCodeLabel: '出库单号', // 业务单据编号名称
  					bizTip: '请领审核出库成功' // 业务操作成功提示
  				}
  			}
  		});
      }

      const { query } = yield select(state => state.outStockCheck);
