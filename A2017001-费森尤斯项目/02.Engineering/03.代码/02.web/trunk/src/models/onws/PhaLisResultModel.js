...
import { loadPatLisPage, savePatLis, getPatLisPage } from '../../services/onws/PhaLisResultService';

export default {
  namespace: 'phaLisResult',
  ...
  effects: {
    ...
    *load({ payload }, { select, call, put }) {
      let { page } = (payload || {});
      const defaultPage = yield select(state => state.phaLisResult.page);
      const searchObjs = yield select(state => state.phaLisResult.searchObjs);
    ...
    *print({ payload }, { call, put }) {
      ...
      if (result) {
        const { printTemplate, map, printData } = result;
        yield Print.sendPrint(printTemplate, printData, map);
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(({ pathname }) => {
        if (pathname === '/onws/treatment/patLisSearch') {
          dispatch({
            type: 'utils/initDicts',
            payload: ['SEX', 'INFECTIOUS_DISEASE', 'SPECIMENT', 'STATE_FLAG'],
          });
        }
      });
    },
