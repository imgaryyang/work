...
export async function loadOutputDetailPage(start, limit, query) {
  return ajax.GET(`${apiRoot.matOutputInfo}OutputDetailPage/${start}/${limit}`, query || {});
}

export async function loadRecord(start, limit, query) {
  return ajax.GET(`${apiRoot.matOutputInfo}loadRecord/${start}/${limit}`, query || {});
}
