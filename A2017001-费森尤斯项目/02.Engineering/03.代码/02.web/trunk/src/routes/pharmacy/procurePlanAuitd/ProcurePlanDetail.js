...
class ProcurePlanDetail extends Component {
  ...
  refreshTable() {
    ...
    const { buyDetail } = this.props.procureAuitd;
    buyDetail.data = newData;
    this.props.dispatch({
      type: 'procureAuitd/setState',
      payload: {
        buyDetail,
      },
    });
  ...
  render() {
    const { procureAuitd, base, utils } = this.props;
    const { buyDetail } = procureAuitd;
    const { wsHeight } = base;
    const topHeight = ((wsHeight - 3 - 10 - 42) * 2) / 4;

    const { data, page } = buyDetail;
    const { deptId, createOper, buyBill, id } = data && data.length > 0 ? data[0].phaBuyBill : {};
    ...
    const columns = [
      ...
      { title: '计划数量', dataIndex: 'buyNum', key: 'buyNum', width: '70px', className: 'text-align-right', render: (text, record) => (record.buyNum + ' ' +record.buyUnit) },
      { title: '本科库存',
        dataIndex: 'deptSum',
        key: 'deptSum',
        width: '80px',
        render: (text, record) => (`${floor((record.deptSum ? record.deptSum : 0) / record.drugInfo.packQty)} ${record.drugInfo.packUnit} ${
          record.deptSum % record.drugInfo.packQty === 0 ? '' : `${record.deptSum % record.drugInfo.packQty} ${record.drugInfo.miniUnit}`}`),
      },
      { title: '本院库存',
        dataIndex: 'totalSum',
        key: 'totalSum',
        width: '80px',
        render: (text, record) => (`${floor((record.totalSum ? record.totalSum : 0) / record.drugInfo.packQty)} ${record.drugInfo.packUnit} ${
          record.totalSum % record.drugInfo.packQty === 0 ? '' : `${record.totalSum % record.drugInfo.packQty} ${record.drugInfo.miniUnit}`}`),
      },
      { title: '审核数量',
        dataIndex: 'auitdNum',
        key: 'auitdNum',
          ...
          <Card className={styles.infoCard} >
            <Row type="flex" justify="space-between" align="middle">
              ...
              <Col span="3" style={{ textAlign: 'right' }} >
                <Button type="primary" size="large" onClick={() => this.onCommit(id)} icon="check" >同意</Button>
              </Col>
              <Col span="3" style={{ textAlign: 'right' }} >
                <Button type="primary" size="large" onClick={() => this.onCommitBack(id)} icon="close" >退回</Button>
              </Col>
            </Row>
          </Card>
        </div>
