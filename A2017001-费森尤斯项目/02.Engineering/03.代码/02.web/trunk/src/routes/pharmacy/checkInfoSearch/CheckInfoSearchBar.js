...
class CheckInfoSearchBar extends Component {
  ...
  handleExport() {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        const { selectedRowKeys } = this.props.checkInfoSearch;
...
const CheckInfoSearchBarForm = Form.create()(CheckInfoSearchBar);
export default connect(({ checkInfoSearch }) => ({ checkInfoSearch }))(CheckInfoSearchBarForm);
