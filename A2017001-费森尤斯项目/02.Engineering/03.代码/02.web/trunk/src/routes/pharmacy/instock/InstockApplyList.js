...
class InstockApplyList extends Component {
  ...
  onSave() {
   // 校验todo
    const { dataApply } = this.props.instock;
    if (dataApply && dataApply.length > 0) {
      const index = dataApply.findIndex(value => value.appNum === 0 || testNumber(value.appNum) === false);
      for (const d of dataApply) {
        if (d.appNum === 0 || testNumber(d.appNum) === false) {
          notification.error({
            message: '提示',
            description: '申请数量请输入非0的数字！',
          });
          return;
        } else if (d.appNum > floor(d.storeSum / d.packQty)) {
          notification.error({
            message: '提示',
            description: '申请数量不能大于库存数量！',
          });
          return;
        }
      }
  ...
  refreshTable(value, index, key) {
    ...
    if (key === 'appNum') {
      if (!testNumber(value) || value === 0) {
        ...
        return false;
      } else if (value > floor(newData[index].storeSum / newData[index].packQty)) {
        notification.info({
          message: '提示',
          description: '申请数量不能大于库存数量！',
        });
  ...
  render() {
    ...
    const columns = [
      ...
      { title: '库存',
        ...
        key: 'storeSum',
        width: '90px',
        render: (text, record) => (`${floor((record.storeSum ? record.storeSum : 0) / record.packQty)} ${record.appUnit} ${
          record.storeSum % record.packQty === 0 ? '' : `${record.storeSum % record.packQty} ${record.miniUnit}`}`),
      },
