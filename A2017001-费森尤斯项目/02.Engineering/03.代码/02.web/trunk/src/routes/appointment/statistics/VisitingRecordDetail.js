...
class VisitingRecordDetail extends Component {

  render() {
    const { odws, utils, visitRecord } = this.props;
    const { odwsWsHeight } = odws;

    const { reg, diagnosis, medicalRecord, orders, totalAmt } = visitRecord;
    let regData = [] ;
    if ( reg != null) {
      regData.push(reg);
    }
    ...
    const regColumns = [
      {
        title: '就诊流水号',
        dataIndex: 'regInfo.regId',
        width: '15%',
        key: 'regInfo.regId',
      }, {
        title: '就诊卡号',
        dataIndex: 'regInfo.patient.medicalCardNo',
        width: '15%',
        key: 'regInfo.patient.medicalCardNo',
      }, {
        title: '姓名',
        width: '15%',
        dataIndex: 'regInfo.patient.name',
        key: 'regInfo.patient.name',
      }, {
        title: '挂号级别',
        width: '8%',
        dataIndex: 'regInfo.regLevel',
        key: 'regInfo.regLevel',
        render: (value) => {
          return utils.dicts.dis('REG_LEVEL', value);
        },
      }, {
        title: '挂号科室',
        dataIndex: 'regInfo.regDept',
        width: '15%',
        key: 'regInfo.regDept',
        render: (value) => {
          if (value) {
    ...
    return (
      <div className={styles.detailContainer} style={{ height: `${odwsWsHeight}px` }} >
        <Card style={{ marginTop: '0' }} className={styles.detailCard} title="挂号" >
          <CommonTable
        ...
        <Card className={styles.detailCard} title="诊断" >
          <CommonTable
            className={styles.detailTable}
            rowSelection={false}
            data={diagnosis}
        ...
        <Card className={styles.detailCard} title="医嘱" >
          <CommonTable
            ...
            footer={() => {
              ...
              return <div className={styles.totalAmt} >共计：<font>{totalCost}</font> 元</div>;
            }}
          />
        </Card>
      </div>
