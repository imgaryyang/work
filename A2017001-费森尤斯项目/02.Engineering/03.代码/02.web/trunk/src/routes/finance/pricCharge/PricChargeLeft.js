...
class PricChargeLeft extends React.Component {
  ...
  onSelect(keys, e) {
    ...
    if (keys === '1' || keys === '2' || keys === '3') {
      this.props.dispatch({
        type: 'pricChargeModel/initShareLevel',
        shareLevel: keys[0],
      });
    } else {
      this.props.dispatch({
        type: 'pricChargeModel/addItemByItemId',
        comboId,
      });
    }
  }
  ...
  loadTemplates(searchCode) {
    // 动态加载病历模板
    this.props.dispatch({
      type: 'pricChargeModel/loadItemTemplate',
      searchCode,
    });
  }
  render() {
    const { tmpTree } = this.props.pricChargeModel;
...
export default connect(({ pricChargeModel, base }) => ({ pricChargeModel, base }))(PricChargeLeft);
