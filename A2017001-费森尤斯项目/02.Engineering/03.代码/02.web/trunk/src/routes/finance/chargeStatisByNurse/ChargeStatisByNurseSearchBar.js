...
class ChargeStatisByNurseSearchBar extends Component {
  ...
  setSearchObjs(searchObj) {
    this.props.dispatch({
      type: 'chargeStatisByNurse/setSearchObjs',
      payload: searchObj,
    });
  }
  ...
  setTag(selectedTag) {
    this.props.dispatch({
      type: 'chargeStatisByNurse/setState',
      payload: { selectedTag },
    });
  }
  ...
  exportToExcel() {
    ...
    this.props.form.validateFields((err, values) => {
      this.setSearchObjs(_.omit(values, 'dateRange'));
      w.location.href = `/api/hcp/payment/chargeStatis/exportStatisByDeptAndNurse?data=${JSON.stringify(this.props.searchObjs)}`;
    });
  }
...
export default Form.create()(ChargeStatisByNurseSearchBar);
