...
class PricChargeList extends Component {
  ...
  onDelete(record) {
    this.props.dispatch({
      type: 'pricChargeModel/deleteItem',
      record,
    });
  }
  ...
  handleChange(value) {
    if (!isObject(value)) {
      this.props.dispatch({
        type: 'pricChargeModel/loadOptions',
        spellCode: value,
      });
    }
  }
  ...
  handleOk() {
    this.props.dispatch({
      type: 'pricChargeModel/saveItemToTemplate',
    });
  }
  ...
  render() {
    ...
    const { itemData, record } = this.props.pricChargeModel;
...
export default connect(
  ({ pricChargeModel, utils, base }) => ({ pricChargeModel, utils, base }))(editorForm);
