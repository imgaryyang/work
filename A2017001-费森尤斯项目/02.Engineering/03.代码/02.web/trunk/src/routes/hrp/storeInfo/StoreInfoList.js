...
class StoreInfoList extends Component {
  ...
  doPrint(record) {
    const newRecord = { commonName: record.tradeName, instrmCode: record.instrmCode };
    this.props.dispatch({
      type: 'print/getPrint',
      payload: { code: '117', printData: newRecord },
    });
  }
  render() {
    ...
    columns = [
      ...
      { title: '位置', dataIndex: 'location', key: 'location', className: 'text-align-center', width: 110, editable: true },
      { title: '停用标志',
        dataIndex: 'stop',
        key: 'stop',
        editor: 'dictSelect',
        ...
        render: (text, record) => (
          <span>
            <Tooltip placement="top" title={'打印'} >
              <Icon onClick={this.doPrint.bind(this, record)} type="printer" style={{ cursor: 'pointer' }} />
            </Tooltip >
          </span>
        ),
      },
