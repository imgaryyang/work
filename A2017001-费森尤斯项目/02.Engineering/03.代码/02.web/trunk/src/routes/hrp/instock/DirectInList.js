...
class DirectInList extends Component {
  ...
  onCommit() {
    ...
    if (dataApply && dataApply.length > 0) {
      ...
      const indexApprovalNo = dataApply.findIndex(value => value.approvalNo === '');
      const indexProduceDate = dataApply.findIndex(value => value.produceDate == null || value.produceDate > today);
      const code = dataApply.findIndex(value => value.instrmCode === '');
      //        const indexValidDate = dataApply.findIndex(value=>value.validDate == null||value.validDate<today);
      if (code !== -1) {
        notification.error({
          message: '提示',
          description: '固定资产编码不能为空！',
        });
        return;
      }
      if (index !== -1) {
  ...
  render() {
    ...
    const columns = [{
      ...
      width: 250,
      render: (text, record) => {
        return (<div > { `名称：${record.commonName}` }
        </div>
        );
      },
    },
      ...
      //      },
    {
      ...
      render: (text, record) => (<span >
        ...
        >
          <Icon
            onClick={this.doPrint.bind(this, record)}
            type="printer"
    ...
    return (<div style={{ padding: '3px' }}>
        ...
        >
          ...
          <Row >
            ...
            >
              ...
              style={{ lineHeight: '28px' }}
            >
              <div > 总金额： { total ? total.formatMoney() : 0 }
                     </div>
            </Col >
          </Row>
        </Form >
      ...
      >
        <ShadowDiv
          showTopShadow={false}
          style={{ height: `${bottomHeight - 52}px` }}
        >
          <EditTable
            data={dataApply}
            pagination={false}
          ...
          />
        </ShadowDiv > <div style={{ textAlign: 'right', paddingTop: '5px' }}>
          <Button
            size="large"
            onClick={
