...
import { Card, Form, Row, Col, Input, Button, Badge, notification } from 'antd';
...
class DiagnoseList extends Component {
  ...
  onDiagnoseSelect(diagnose) {
    ...
    if (dis && dis.length > 0) {
      for (const item of dis) {
        if (item.diseaseId === diagnose.diagnosisCode) {
          notification.info({
            message: '提示',
            description: '诊断已存在！',
          });
  ...
  render() {
    ...
    const columns = [
      ...
      { title: '诊断时间', dataIndex: 'createTime', key: 'createTime', width: 80 },
      { title: '主诊断',
        dataIndex: 'iscurrent',
        key: 'iscurrent',
        width: 60,
