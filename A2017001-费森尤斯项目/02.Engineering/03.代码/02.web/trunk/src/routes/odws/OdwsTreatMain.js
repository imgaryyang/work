...
class OdwsTreatMain extends Component {
  ...
  render() {
    ...
    const diagnoseDisabled = !(isToday(currentReg.regTime) && (currentReg.regState === '30' || currentReg.regState === '31'));

    // 就诊时间为当天、就诊状态为30（正在就诊）且已开诊断时，医嘱、病历按钮可用
    const disabled = !(isToday(currentReg.regTime) && (currentReg.regState === '30' || currentReg.regState === '31') && diagnosis.length > 0);

    // 就诊时间为当天且就诊状态为30（正在就诊）时，“结束就诊”和“取消就诊”按钮可用
    const showOptBtn = isToday(currentReg.regTime) && (currentReg.regState === '30' || currentReg.regState === '31');
        ...
        <Layout className={styles.mainLayout} style={{ overflow: 'visible' }} >
          <Sider width={100} >
            ...
            >
              ...
              {
                showOptBtn ? (
                  <Menu.Item key="treatDone" className="on-save" ><Icon type="logout" />结束就诊</Menu.Item>
                ) : null
              }
              {
                showOptBtn ? (
