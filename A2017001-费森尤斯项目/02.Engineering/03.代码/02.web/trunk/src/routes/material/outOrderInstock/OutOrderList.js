...
class OutOrderList extends Component {
  ...
  componentWillMount() {
    this.props.dispatch({
      type: 'matOutOrderInstock/loadOutOrderList',
    });
  }
  ...
  onConfirm(record) {
    this.props.dispatch({
      type: 'matOutOrderInstock/loadOutOrderDetail',
      outBill: record[0],
    });
  }

  render() {
    const { outOrderList } = this.props.matOutOrderInstock;
    const columns = [
      { title: '出库单',
        dataIndex: '0',
        key: '0',
...
export default connect(({ base, matOutOrderInstock }) => ({ base, matOutOrderInstock }))(OutOrderList);
