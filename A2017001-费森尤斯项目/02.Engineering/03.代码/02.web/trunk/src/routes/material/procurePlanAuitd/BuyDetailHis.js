...
class ProcurePlanDetail extends Component {
  ...
  render() {
    const { base, matProcureAuitd } = this.props;
    const { buyHis } = matProcureAuitd;
    const { wsHeight } = base;
    ...
    const columns = [
        ...
        render: (text, record) => {
          return (
            <div>
              ...
              {`生产厂商：${record.matInfo.companyInfo ? record.matInfo.companyInfo.companyName : ''}`}<br />
              {`供货商：${record.companyInfo ? record.companyInfo.companyName : ''}`}<br />
            </div>
          );
        },
      },
      // { title: '规格', dataIndex: 'materialSpecs', key: 'materialSpecs', width: '50px', className: 'text-align-center' },
      ...
      { title: '采购数量',
        ...
        render: (text, record) => (`${(record.inSum / record.matInfo.materialQuantity).formatMoney(0)} ${record.matInfo.maxUnit}`),
      },
