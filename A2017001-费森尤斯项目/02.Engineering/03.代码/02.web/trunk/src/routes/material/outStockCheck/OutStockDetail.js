...
// }
  ...
  onPageChange(pageRightNew) {
    const { queryRight } = this.props.matOutStockCheck;
    this.props.dispatch({
  ...
  render() {
    const { dataOutStock, pageRight } = this.props.matOutStockCheck;
    ...
    const columns = [
      ...
      { title: '规格', dataIndex: 'materialSpec', key: 'materialSpec', width: 50, className: 'text-align-right' },*/
      { title: '零售价',
        dataIndex: 'salePrice',
        key: 'salePrice',
        width: 90,
        className: 'text-align-right',
        render: text => (text ? text.formatMoney(4) : ''),
      },
      { title: '批号/批次',
      ...
      { title: '当前库存',
        ...
        key: 'storeSum',
        width: 80,
        render: (text, record) => (record.storeSum ? record.storeSum + '' + (record.matInfo  ? record.matInfo.materialUnit : '') : 0) },
      { title: '出库数量',
        dataIndex: 'appNum',
        key: 'appNum',
        width: 80,
      ...
      { title: '出库金额',
        ...
        width: 80,
        className: 'text-align-right',
        render: text => (text ? text.formatMoney() : ''),
      },
      /* { title: '生产厂家',
        dataIndex: 'producer',
        key: 'producer',
        width: 70,
        render: (text, record) => (record.matInfo.companyInfo ? record.matInfo.companyInfo.companyName : ''),
      },
...
export default connect(
  ({ base, matOutStockCheck }) => ({ base, matOutStockCheck }),
)(OutstockDetail);
