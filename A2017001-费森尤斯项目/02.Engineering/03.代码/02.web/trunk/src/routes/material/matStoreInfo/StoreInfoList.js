    ...
    const storeSumCompute = (record) => {
      if (record.storeSum != null) {
        if (record.storeSum === 0) {
          ...
          return `${(record.storeSum)}${record.materialInfo ? (record.materialInfo.materialUnit ? record.materialInfo.materialUnit : '' ) : ''}`;
        }
      } else {
        return '';
      }
      ...
      columns = [
          ...
          render: (text, record) => {
            return (
              <div>
                ...
                {`${text}(${record.materialInfo ? (record.materialInfo.materialSpecs ? record.materialInfo.materialSpecs : '') : '' || '-'})`}<br />
        ...
        { title: '警戒库存量',
          ...
          addonAfter: (text, record) => {
            return record.materialInfo ? ( record.materialInfo.materialUnit ? record.materialInfo.materialUnit : '') : '';
          },
