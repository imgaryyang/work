...
class MaterialInfoList extends Component {
  componentWillMount() {
    this.props.dispatch({ type: 'material/load' });
  }
  onEdit(record) {
  ...
  render() {
    const { base, material, utils, chanel } = this.props;
    ...
    const columns = [
      ...
      { title: '采购价', dataIndex: 'buyPrice', key: 'buyPrice', width: 77, className: 'text-align-right', render: value => (value ? value.formatMoney(4) : '') },
      { title: '销售价', dataIndex: 'salePrice', key: 'salePrice', width: 77, className: 'text-align-right', render: value => (value ? value.formatMoney(4) : '') },
      {
        title: '停用标志',
        dataIndex: 'stopFlag',
        key: 'stopFlag',
        ...
        render: (text, record) => (
          <span>
            ...
            <RowDelBtn onOk={() => this.onDelete(record)} style={{ display: (record.hosId === base.user.hosId ? '' : 'none') }} />
          </span>
        ),
      },
    ];
