...
class ProcurePlanDetail extends Component {
  ...
  // 查询历史采购信息
    this.props.dispatch({
      type: 'matProcureAuitdSearch/loadBuyHis',
      record,
    });
  }
  ...
  onCommit(id) {
    this.props.dispatch({
      type: 'matProcureAuitdSearch/saveBuy',
  ...
  onPageChange(page) {
    ...
    // 导出
  handleExport() {
    const query = this.props.matProcureAuitdSearch.buyDetail.data[0];
    if (query) {
      ...
      w.location.href = '/api/hcp/material/buyDetail/expertToWord?data=' + JSON.stringify(condition);
    } else {
    ...
    // 打印
  handlePrint() {
    const { query } = this.props.matProcureAuitdSearch.buyDetail;
    Modal.confirm({
      content: '确定要打印采购单吗？',
      okText: '确定',
      cancelText: '取消',
  ...
  /* refreshTable() {
    ...
    const { buyDetail } = this.props.matProcureAuitdSearch;
  ...
  render() {
    const { matProcureAuitdSearch, base, utils } = this.props;
    const { buyDetail } = matProcureAuitdSearch;
    const { wsHeight } = base;
    const { data, page } = buyDetail;
    console.log(data);
    const { deptId, createOper, buyBill } = data && data.length > 0 ? data[0].matBuyBill : {};
    ...
    const columns = [
      ...
      /* { title: '物资名称', dataIndex: 'tradeName', key: 'tradeName', width: '50px', className: 'text-align-center' },
...
export default connect(
  ({ matProcureAuitdSearch, base, utils }) => ({ matProcureAuitdSearch, base, utils }),
)(ProcurePlanDetail);
