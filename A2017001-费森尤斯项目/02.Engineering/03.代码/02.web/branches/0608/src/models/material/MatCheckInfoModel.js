...
import { loadCheckInfoPage, loadCheckPage, updateCheckInfo, addCheckInfo, deleteCheck, finishCheck, getBill } from '../../services/material/MatCheckInfoService';

export default {
  namespace: 'matCheckInfo',

  state: {
    query: {},
    page: { total: 0, pageSize: 10, pageNo: 1 },
    matCheckInfoPage: { total: 0, pageSize: 10, pageNo: 1 },
    searchObjs: {},
    checkInfoSearchObjs: {},
    matCheckInfoData: [],
    data: [],
  ...
  effects: {
    *load({ payload }, { select, call, put }) {
      ...
      const defaultPage = yield select(state => state.matCheckInfo.page);
      const defaultQuery = yield select(state => state.matCheckInfo.query);
    ...
    *loadCheckInfo({ payload }, { select, call, put }) {
      const { page, query } = (payload || {});
      const defaultPage = yield select(state => state.matCheckInfo.checkInfoPage);
      const defaultQuery = yield select(state => state.matCheckInfo.checkInfoSearchObjs);
      const newPage = { ...defaultPage, ...page };
      const { pageNo, pageSize } = newPage;
      const start = (pageNo - 1) * pageSize;
      yield put({ type: 'setState', payload: { spin: true } });
    ...
    *finishCheck({ payload }, { select, call, put }) {
      const olddata = yield select(state => state.matCheckInfo.data);
      yield put({ type: 'setState', payload: { spin: true } });
    ...
    *addCheckInfo({ payload }, { select, call, put }) {
      const bill = yield select(state => state.matCheckInfo.bill);
  ...
  reducers: {
    ...
    initCheckInfo(state, { data, page }) {
      ...
      const datatmp = result || [];
      return { ...state, matCheckInfoData: datatmp, matCheckInfoPage: p };
    },
