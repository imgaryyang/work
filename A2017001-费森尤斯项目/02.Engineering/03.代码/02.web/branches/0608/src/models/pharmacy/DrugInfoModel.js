...
export default {
  ...
  effects: {
    *load({ payload }, { select, call, put }) {
      const { query } = (payload || {});
      let { page } = (payload || {});
      const defaultPage = yield select(state => state.drugInfo.defaultPage);
      ...
      const { data } = yield call(drugInfoService.loadDrugInfoPage, start, pageSize, searchObjs);
      yield put({ type: 'toggleSpin' });
    ...
    *save({ params }, { select, call, put }) {
      ...
      const { data } = yield call(drugInfoService.saveDrugInfo, params);
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) {
      history.listen(({ pathname }) => {
        if (pathname === '/pharmacy/settings/medicineMng') {
          dispatch({
