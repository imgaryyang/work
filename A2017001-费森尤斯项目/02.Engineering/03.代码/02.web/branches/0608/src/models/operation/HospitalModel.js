import { loadHospitalPage, saveHospital, deleteHospital, deleteAllHospitals, loadTypes } from '../../services/operation/HospitalService';

export default {
  namespace: 'optHospital',
  state: {
    selectedType: {},
    selectedGroup: '',
    selectedColumnName: '',
  ...
  effects: {
    ...
    *load({ payload }, { select, call, put }) {
      ...
      const defaultPage = yield select(state => state.optHospital.page);
    ...
    *deleteSelected({ payload }, { select, call, put }) {
      const selectedRowKeys = yield select(state => state.optHospital.selectedRowKeys);
      yield put({ type: 'setState', payload: { spin: true } });
