...
export default {
	...
	effects: {
		...
		*load({ payload }, { select, call, put }) {
			...
			//console.info('commonName = ',commonName);
	    ...
	    *loadApply({ payload }, { select, call, put }) {
	    	  ...
	    	  if (data&&data.success){
		    	  if( data.result && data.result.length > 0 ){
		    		 ...
		    		 //console.info('heiheiheihie',company)
		    		 ...
		    		 for( let i = 0; i < result.length ; i++){
		    			 ...
		    			 dataApply.push({
				            	...
				            	producerName: result[i].matInfo.companyInfo.companyName,
