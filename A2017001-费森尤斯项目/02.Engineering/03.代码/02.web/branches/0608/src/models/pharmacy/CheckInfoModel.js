...
import { loadCheckInfoPage, loadCheckPage, updateCheckInfo, addCheckInfo, deleteCheck, finishCheck, getBill } from '../../services/pharmacy/CheckInfoService';
...
export default {
  ...
  effects: {
    *load({ payload }, { select, call, put }) {
      ...
      const defaultPage = yield select(state => state.checkInfo.page);
      const defaultQuery = yield select(state => state.checkInfo.query);
    ...
    *loadCheckInfo({ payload }, { select, call, put }) {
      const { page, query } = (payload || {});
      const defaultPage = yield select(state => state.checkInfo.checkInfoPage);
      const defaultQuery = yield select(state => state.checkInfo.query);
      const newPage = { ...defaultPage, ...page };
      const { pageNo, pageSize } = newPage;
      const start = (pageNo - 1) * pageSize;
    ...
    *finishCheck({ payload }, { select, call, put }) {
      const olddata = yield select(state => state.checkInfo.data);
      yield put({ type: 'setState', payload: { spin: true } });
    ...
    *addCheckInfo({ payload }, { select, call, put }) {
      const bill = yield select(state => state.checkInfo.bill);
