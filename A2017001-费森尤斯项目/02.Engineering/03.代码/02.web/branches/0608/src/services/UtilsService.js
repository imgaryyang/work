...
export async function getOptions(dictColumnNames) {
  ...
  if (notInCache.length > 0) {  // 如果有缓存中不存在的项，发起后台请求
    const { data } = await ajax.GET(`${apiRoot.dictionary}listByColName/`, notInCache);
...
async function loadDepts() {
  // 取所有科室类型
  const deptTypes = await ajax.GET(`${apiRoot.dictionary}listByColName/`, ['DEPT_TYPE']);
  // 取所有科室信息
  const depts = await ajax.GET(`${apiRoot.dept}list/`);
 ...
 * 重新载入科室信息
 */
export async function reloadDepts() {
  DEPT_OPTIONS = {};
  DEPT_IDX = {};
  loadDepts();
  return { depts: DEPT_OPTIONS, deptsIdx: DEPT_IDX };
...
export async function addDeptsToCache(deptIds) {
  ...
  // 取所有科室信息
  const depts = await ajax.GET(`${apiRoot.dept}findByIds/`, deptIds);
  // 将科室信息放入缓存
  if (depts.data && depts.data.success) {
    for (let i = 0; i < depts.data.result.length; i += 1) {
      const item = depts.data.result[i];
...
export async function initTrees(dictTypes) {
  ...
  if (notInCache.length > 0) {  // 如果有缓存中不存在的项，发起后台请求
    const { data } = await ajax.GET(`${apiRoot.tree}listFirstLevel/`, notInCache);
    ...
    if (notInCache.length > 0) {
      const rst = await ajax.GET(`${apiRoot.tree}listByDictType/`, notInCache);
...
export async function loadTreeItemChildren(dictType, selectedOptions) {
  ...
  const { data } = await ajax.GET(`${apiRoot.tree}list`, { parentId: targetOption.id });
...
export async function loadItemInfo(data) {
  return ajax.POST(`${apiRoot.base}commonItem/list`, data);
}
...
export async function loadDiagnosis(data) {
  return ajax.POST(`${apiRoot.base}diagnosis/list`, data);
}
...
export async function loadFreqs(data) {
  return ajax.POST(`${apiRoot.base}freq/list`, data);
}
...
export async function loadRegInfo(data) {
  return ajax.POST(`${apiRoot.base}regInfo/list`, data);
}
/**
 * 获取药品信息
 */
export async function loadDrugInfo(data) {
  return ajax.POST(`${apiRoot.base}drugInfo/list`, data);
}
...
export async function loadMaterialCompany(data) {
  return ajax.POST(`${apiRoot.base}materialCompany/list`, data);
}
...
export async function loadMaterialCertificate(data) {
  return ajax.POST(`${apiRoot.base}materialCertificate/list`, data);
}
...
export async function loadMaterials(data) {
  return ajax.POST(`${apiRoot.base}materials/list`, data);
}
...
export async function loadPatientInfoByPatientId(patientId) {
  return ajax.GET(`${apiRoot.patient}patientId/${patientId}`);
}
...
export async function loadCompanies(data) {
  return ajax.GET(`${apiRoot.base}companies/list`, data);
}
...
export async function getDataSource(tableNames) {
  ...
  tableNames.map(async (item) => {
    const { data } = await ajax.GET(`${apiRoot.base}searchInput/${item}`);
    if (data && data.success) {
