...
class ChargeStatisByDocSearchBar extends Component {
  ...
  exportToExcel() {
    ...
    this.props.form.validateFields((err, values) => {
      this.setSearchObjs(_.omit(values, 'dateRange'));
      w.location.href = `/api/hcp/payment/chargeStatis/exportStatisByDeptAndDoc?data=${JSON.stringify(this.props.searchObjs)}`;
    });
  }
        ...
        <Row type="flex">
          <Col span={20} className="action-form-searchbar">
            <Form inline >
              ...
              <FormItem>
                {getFieldDecorator('deptId')(<DeptSelect showSearch style={{ width: '100px' }} deptType={['001']} placeholder="开单科室" />)}
              </FormItem>
              <FormItem>
                <span>收费日期： </span>
              </FormItem>
          ...
          <Col span={4} className="action-form-operating">
            <Button type="primary" size="large" className="on-add" icon="printer">
              打印
            </Button>
            <Button size="large" onClick={this.exportToExcel} className="on-add" icon="export">
              导出
            </Button>
          </Col>
        </Row>
...
export default Form.create()(ChargeStatisByDocSearchBar);
