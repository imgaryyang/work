...
import CompanySearchInput from
    '../../../components/searchInput/CompanySearchInput';
    ...
    const columns = [
      ...
      { title: '进价/零售价',
        ...
        render: (text, record) => {
          return (
            <div>
              <font style={{ color: '#bfbfbf' }} >(进) </font>{text.formatMoney(4)}<br />
              <font style={{ color: '#bfbfbf' }} >(售) </font>{record.salePrice.formatMoney(4)}
            </div>
          );
        },
      },
    ...
    return (
      <div style={{ padding: '3px' }} >
        <Card className={styles.infoCard} >
          {/* <Row>
            <Col span={24} ><SearchBar onSearch={
      this.onSearch.bind(this)} /></Col>
          </Row>*/
  }
  <Form inline style = {{marginBottom : '5px'}}>

      <Row><Col span = {8}><div style = {{lineHeight : '28px'}}>供货商：{/*
            	...
            	showSearch filterOption={(input, option) => this.filterUser(input, option)}>
						{companyOptions}
				</Select>*/}<FormItem> {getFieldDecorator('companyName')(
            	<CompanySearchInput
    placeholder = "供货商"

    companyType = { ['2'] } services = { ['1'] } onSelect =
    { this.companySelectEvent } style =
    {
      { width: '200px' }
    } />
              )}
        ...
        <Card className={styles.bottomCard} style={{ height: `${bottomHeight}px` }} >
          ...
          <div style={{ textAlign: 'right', paddingTop: '5px' }} >
            ...
            Button size = "large" onClick = { () => this.onSave() } style =
            {
              { marginRight: '10px' }
            } icon = "cloud-upload-o">
            暂存<
