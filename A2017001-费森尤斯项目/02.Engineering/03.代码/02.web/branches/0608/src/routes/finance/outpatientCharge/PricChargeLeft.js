...
class PricChargeLeft extends React.Component {
  ...
  onSelect(keys, e) {
    ...
    if (keys === '1' || keys === '2' || keys === '3') {
      this.props.dispatch({
        type: 'outpatientCharge/initShareLevel',
        shareLevel: keys[0],
      });
    } else {
      this.props.dispatch({
        type: 'outpatientCharge/addItemByItemId',
        comboId,
      });
    }
  }
  ...
  render() {
    const { tmpTree } = this.props.outpatientCharge;
    ...
    return (
      <Card style={{ height: `${cardHeight}px` }} className="card-padding-5" >
        <div >
          <Input
            placeholder="搜索病历模板"
            maxLength={10}
...
export default connect(({ outpatientCharge, base }) => ({ outpatientCharge, base }))(PricChargeLeft);
