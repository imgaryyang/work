    ...
    if (!isToday(record.regTime) || record.regState !== '21') {
      this.props.dispatch({
        ...
        payload: {
          ...
          treatStep: isToday(record.regTime) && record.regState === '30' ? 'diagnose' : 'history', // 继续就诊导向“就诊”，查看导向“就诊记录”
        },
      });
      return;
    }
  ...
  render() {
    ...
    const columns = [
      ...
      { title: '姓名',
        ...
        key: 'patientName',
        render: (text, record) => {
          return `${text}（${utils.dicts.dis('SEX', record.patient.sex)} | ${moment().diff(moment(record.patient.birthday), 'years')}岁）`;
        },
      },
      ...
      { title: '操作',
        ...
        render: (text, record, idx) => {
          ...
          if (moment(this.props.odws.query.regTime).format('YYYY-MM-DD') === moment(new Date()).format('YYYY-MM-DD')) {
            ...
            else if (record.regState === '30') btnText = '继续就诊';
            else btnText = '查看';
          } else {
            btnText = '查看';
          }
