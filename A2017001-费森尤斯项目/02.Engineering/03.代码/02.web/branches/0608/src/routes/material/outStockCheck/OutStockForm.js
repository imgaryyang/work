...
class SelectForm extends Component {
  ...
  handleSubmit() {
    this.props.form.validateFields((err, values) => {
      if (!err) {
        this.props.dispatch({
          type: 'matOutStockCheck/saveOutStockInfo',
          value: values,
        });
      }
    });
  ...
  handleSubmitBack() {
    this.props.form.validateFields((err, values) => {
      if (!err) {
        this.props.dispatch({
          type: 'matOutStockCheck/backOutStockInfo',
          value: values,
        });
      }
    });
  ...
  toSearch(event) {
    const { record } = this.props.matOutStockCheck;
    this.props.dispatch({
  ...
  render() {
    const { record, sum, searchSwitch } = this.props.matOutStockCheck;
    ...
    return (
      <Form inline style={{ paddingBottom: '5px' }} >
        ...
        <Row>
          <Col span="8">
            <FormItem label="物资过滤" {...formItemLayout}>
              { getFieldDecorator('tradeName')(
                <Input placeholder="名称" onPressEnter={this.toSearch.bind(this)} disabled={searchSwitch} style={{ width: '100%' }} />,
              )}
            </FormItem>
          </Col>
          <Col span="8">
