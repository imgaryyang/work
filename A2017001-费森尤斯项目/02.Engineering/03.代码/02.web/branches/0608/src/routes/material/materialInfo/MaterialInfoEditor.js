...
class EditorForm extends Component {
  ...
  render() {
    const { editorSpin, visible, record } = this.props.material;
    const { getFieldDecorator } = this.props.form;
    const title = record.drugCode ? `修改物资信息：${record.commonName}` : '新增物资信息';
              ...
              <Col span={6} >
                <FormItem label="常规数量" {...formItemLayout} >
                  { getFieldDecorator('materialQuantity', {
                    ...
                    <InputNumber style={{ width: '100%' }} maxLength={8} max={99999999} />,
                  )}
                </FormItem>
              </Col>
              <Col span={6} >
              ...
              <Col span={6} >
                <FormItem label="批发价" {...formItemLayout} >
                  { getFieldDecorator('wholePrice', {
                    initialValue: typeof record.wholePrice === 'undefined' || record.wholePrice === null ? '' : record.wholePrice,
                  })(<InputNumber style={{ width: '100%' }} maxLength={8} max={99999999999999.9999} />) }
                </FormItem>
              </Col>
              <Col span={6} >
