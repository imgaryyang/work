...
class DirectInList extends Component {
  ...
  refreshTable(value, index, key) {
    ...
    //console.info('zhouminnewData', newData);
    let reg = '';
    if (key === 'inSum') {
      reg = /^-?([1-9][0-9]*)?$/;
      if (!reg.test(value)) {
  ...
  render() {
    ...
    const { dataApply,companyInfo,company } = materialDirectIn;
    console.info('directIn+++++++++++++++',materialDirectIn)
