        ...
        render: (text, record) => {
          return (
            <div>
              ...
              {`生产厂商：${record.matInfo ? (record.matInfo.companyInfo.companyName || '-') : '-'}`}<br />
            </div>
          );
        },
      },
...
//      { title: '申请单位', dataIndex: 'matInfo.materialUnit', key: 'matInfo.materialUnit', width: 65, className: 'text-no-wrap' },
      { title: '申请金额', dataIndex: 'buyCost', key: 'buyCost', width: 120, className: 'text-align-right text-no-wrap', render: text => text.formatMoney() },
    ];
      ...
      <div style={{ padding: '3px' }} >
        <Card className={styles.infoCard} >
          <Row type="flex" justify="space-between" align="middle" >
            ...
            <Col span={5}>
              <div >请领总金额：{total.formatMoney()}</div>
            </Col>
