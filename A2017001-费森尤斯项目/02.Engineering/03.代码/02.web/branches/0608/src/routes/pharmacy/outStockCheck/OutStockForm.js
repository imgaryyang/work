...
class SelectForm extends Component {
  ...
  handleSubmit() {
    this.props.form.validateFields((err, values) => {
      if (!err) {
        this.props.dispatch({
          type: 'outStockCheck/saveOutStockInfo',
          value: values,
        });
      }
    });
  ...
  handleSubmitBack() {
    this.props.form.validateFields((err, values) => {
      if (!err) {
        this.props.dispatch({
          type: 'outStockCheck/backOutStockInfo',
          value: values,
        });
      }
    });
  ...
  toSearch(event) {
    const { record } = this.props.outStockCheck;
    this.props.dispatch({
  ...
  render() {
    const { record, sum, searchSwitch } = this.props.outStockCheck;
