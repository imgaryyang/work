...
class ProcurePlanDetail extends Component {
  ...
  render() {
    ...
    const { buyDetail } = matProcureAuitd;
    const { wsHeight } = base;
    const topHeight = ((wsHeight - 3 - 10 - 42) * 2) / 4;

    const { data, page } = buyDetail;
    const { deptId, createOper, buyBill, id } = data && data.length > 0 ? data[0].matBuyBill : {};
    ...
    const columns = [
      ...
      { title: '本科库存',
        ...
        key: 'deptSum',
        width: '80px',
        render: (text, record) => (record.deptSum ? record.deptSum + record.matInfo.materialUnit : 0),
      },
      { title: '本院库存',
        dataIndex: 'totalSum',
        key: 'totalSum',
        width: '80px',
        render: (text, record) => (record.totalSum ? record.totalSum + record.matInfo.materialUnit : 0),
      },
      { title: '审核数量',
        dataIndex: 'auitdNum',
        key: 'auitdNum',
