...
/**
 * @Author xlbd
 * @Desc LodopPrint Lodop 打印公共函数
 */

const initPrint = () => {
  if (_.isFunction(needCLodop)) {
    const LODAP = document.getElementById(LODAPID);
    if (!LODAP) throw new Error('找不到报告打印机控件');
  }
};

const sendPrint = (template, data, map) => {
    LODOP=getLodop();
    eval(template);
};
