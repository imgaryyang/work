...
export default {
  ...
  effects: {
    *load({ payload }, { select, call, put }) {
      ...
      const chanel = yield select(state => state.invoiceReprint.chanel);
      let newQuery = query || defaultQuery;
  ...
  reducers: {
    ...
    initReFund(state, { dataReFund, page }) {
      const { result, total } = dataReFund;
      const p = { ...state.page, ...page, total };
      const datatmp = result || [];
      return { ...state, dataReFund: datatmp, page: p };
    },
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(({ pathname }) => {
        if (pathname === '/charge/reFund') {
          dispatch({
            type: 'setState',
            payload: { chanel: 'reFund' },
          });
        }
        if (pathname === '/charge/invoiceReprint') {
          dispatch({
            type: 'setState',
            payload: { chanel: 'rePrint' },
          });
        }
      });
    },
