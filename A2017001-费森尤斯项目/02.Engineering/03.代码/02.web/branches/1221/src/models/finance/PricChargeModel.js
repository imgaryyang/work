...
export default {
  ...
  effects: {
    ...
    *getUserInfoByRegId({ regId }, { select, call, put }) {
      const userInfo = yield select(state => state.pricChargeModel.userInfo);
    ...
    *loadRegInfoPage({ payload }, { select, call, put }) {
      let { page } = (payload || {});
      const defaultPage = yield select(state => state.pricChargeModel.page);
    ...
    *subItem({ subData }, { select, call, put }) {
      if (subData && subData.length > 0) {
        const regNo = yield select(state => state.pricChargeModel.userInfo.regNo);
        const patientId = yield select(state => state.pricChargeModel.userInfo.patientId);
        const regId = yield select(state => state.pricChargeModel.userInfo.regId);
        const userInfo = {
          regNo,
          patientId,
          regId,
    ...
    *saveCharge({ subData }, { select, call, put }) {
      const itemData = yield select(state => state.pricChargeModel.itemData);
      if (itemData && itemData.length > 0) {
        const regNo = yield select(state => state.pricChargeModel.userInfo.regNo);
        const patientId = yield select(state => state.pricChargeModel.userInfo.patientId);
        const regId = yield select(state => state.pricChargeModel.userInfo.regId);
        const itemCost = yield select(state => state.pricChargeModel.itemCost);
        const userInfo = {
          regNo,
          patientId,
          regId,
    ...
    *updateItem({ update }, { select, put }) {
      const tmpItem = yield select(state => state.pricChargeModel.tmpItem);
    ...
    *addCharge({ exeDept }, { select, call, put }) {
      const tmpItem = yield select(state => state.pricChargeModel.tmpItem);
      let itemData = yield select(state => state.pricChargeModel.itemData);
      const userInfo = yield select(state => state.pricChargeModel.userInfo);
      const currentRecipe = yield select(state => state.pricChargeModel.currentRecipe);
    ...
    *saveItemToTemplate({ itemName }, { select, call, put }) {
      let itemData = yield select(state => state.pricChargeModel.itemData);
      const shareLevel = yield select(state => state.pricChargeModel.shareLevel);
      const templateName = yield select(state => state.pricChargeModel.templateName);
