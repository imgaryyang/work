...
export default {
  ...
  effects: {
    ...
    *loadMaterial({ payload }, { select, call, put }) {
      ...
      // 获取药品信息，翻页
      const { data } = yield call(materialInfoService.loadMaterialInfoPage, null, start, pageSize, query);
      if (data) {
      // 更新state，刷新页面
      ...
      if (buy.buyDetail && buy.buyDetail.length > 0) {
        ...
        for (const item of buy.buyDetail) {
          if (item.buyPrice <= 0 || !item.buyNum || item.buyNum <= 0) {
            notification.info({
              message: '提示',
              description: `请输入【${item.tradeName}】的采购单价和数量`,
            });
        ...
        const newBuy = {
          ...buy, deptId: user.loginDepartment.id, createOper: user.name, buyState, buyCost,
        };
        ...
        if (ret) {
          if (ret.success) {
            if (buyState === '0') {
              ...
              yield put({ type: 'setState', payload: { buy: { buyDetail: [] } } });
            }
