...
import { loadOutputDetailPage, outputExpertToExcel, loadTotalSum, loadRecord } from '../../services/pharmacy/OutputInfoService';

export default {
  namespace: 'outputDetailInfo',
  state: {
    recordPage: {
      total: 0,
      pageSize: 10,
  ...
  effects: {
    ...
    *loadRecord({ payload }, { select, call, put }) {
      const { query } = (payload || {});
      let { page } = (payload || {});
      const defaultPage = yield select(state => state.outputDetailInfo.recordPage);
      let searchObjs = yield select(state => state.outputDetailInfo.searchObjs);
    ...
    *loadOutputDetail({ payload }, { select, call, put }) {
      ...
      const outputDetailInfo = yield select(state => state.outputDetailInfo);
      const oldPage = yield select(state => state.outputDetailInfo.page);
    ...
    *loadOutStockDetail({ payload }, { select, call, put }) {
      ...
      const defQuery = yield select(state => state.outputDetailInfo.detailQuery);
      const oldPage = yield select(state => state.outputDetailInfo.page);
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(({ pathname }) => {
        if (pathname === '/pharmacy/outStockDetailInfo') {
          dispatch({
