...
import { inStoreDetailPage } from '../../services/material/DirectInService';

export default {
  namespace: 'matInStoreDetail',
  ...
  effects: {
    *load({ payload }, { select, call, put }) {
      const { query } = (payload || {});
      let { page } = (payload || {});
      const defaultPage = yield select(state => state.matInStoreDetail.page);
      let searchObjs = yield select(state => state.matInStoreDetail.searchObjs);
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(({ pathname }) => {
        if (pathname === '/material/inStoreDetail') {
          dispatch({
            type: 'load',
          });
          dispatch({
            ...
            payload: [
              'MATERIAL_TYPE', 'IN_TYPE', 'COMPANY_TYPE',
            ],
          });
        }
      });
