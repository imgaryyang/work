...
class InStoreDetailList extends Component {
  ...
  doPrint(record) {
    let printData = {};
    printData.commonName = record.tradeName;
    printData.instrmCode = record.instrmCode;
    console.info(printData);
    this.props.dispatch({
        type: 'print/getPrint',
        payload: { code: '117', printData: printData },
    });
  }
  ...
  render() {
    ...
    const columns = [
     ...
     /* { title: '折旧(月)', dataIndex: 'instrmInfo.limitMonth', key: 'instrmInfo.limitMonth', width: '60px' },*/
      ...
      { title: '操作', width: 58, key: 'action', className: 'text-align-center',
          render: (text, record) => ( 
            <span >
