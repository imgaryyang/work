...
class InStoreSearchBar extends Component {
  handlePrint() {
    const { record } = this.props.outputDetailInfo;
    if (record.id) {
      Modal.confirm({
        content: '确定要打印出库单吗？',
        okText: '确定',
        ...
        onOk: () => {
          this.props.dispatch({
            ...
            payload: { code: '010', bizId: record.id },
          });
        },
      });
    } else {
  ...
  handleExport() {
    ...
    const { query } = this.props.outputDetailInfo;
    w.location.href = `/api/hcp/pharmacy/outputInfo/expertToExcel?data= ${JSON.stringify(query)}`;
  }
  render() {
    const { record } = this.props.outputDetailInfo;
...
export default connect(
  ({ outputDetailInfo, utils }) => ({ outputDetailInfo, utils }),
)(InStoreSearchBar);
