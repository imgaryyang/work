...
import Styles from './styleInfo.less';
...
class HospitalList extends Component {
  ...
  forDeleteSelected() {
    const { selectedRowKeys } = this.props.optHospital;
    if (selectedRowKeys && selectedRowKeys.length > 0) {
      this.props.dispatch({
        type: 'optHospital/deleteSelected',
      });
    }
  }
  ...
  render() {
    const { optHospital, utils, base } = this.props;
    const { page, data, selectedRowKeys } = optHospital;
    const { dicts } = utils;
    const { wsHeight } = base;
    const containerHeight = wsHeight - 6;
    const rowClassName = (record) => {
      if (record.hosId === 'H0027') {
        return Styles.stop;
      }
    };
    const columns = [
    ...
    const expandTable = (record) => {
      const expandColumns = [
        ...
        { title: '开业时间', dataIndex: 'busiTime', key: 'busiTime', width: 90, className: 'text-align-center', render: value => (value ? moment(value).format('YYYY-MM-DD') : '') },
...
export default connect(
  ({ optHospital, utils, base }) => ({ optHospital, utils, base }),
)(HospitalList);
