...
class ProcurePlanDetail extends React.Component {
  ...
  render() {
    const { base, matProcureAuitdSearch, utils } = this.props;
    const { buyHis } = matProcureAuitdSearch;
    const { wsHeight } = base;
...
export default connect(({ matProcureAuitdSearch, base, utils }) => ({ matProcureAuitdSearch, base, utils }))(ProcurePlanDetail);
