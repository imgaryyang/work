...
class OutstockDetail extends React.Component {
  ...
  onPageChange(pageNew) {
    const { query } = this.props.outputSummary;
    this.props.dispatch({
  ...
  render() {
    const { outputSummary, base } = this.props;
    const { wsHeight } = base;
    const leftHeight = wsHeight - (90);
    const { data, page } = outputSummary;
    const columns = [
      { title: '出库科室',
        dataIndex: '3',
        key: 'deptName',
...
export default connect(({ outputSummary, base }) => ({ outputSummary, base }))(OutstockDetail);
