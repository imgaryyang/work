...
class OutstockDetail extends Component {
  ...
  render() {
    ...
    const columns = [{
      ...
      render: (text, record) => {
        return ( < div > { `${text}(${record.instrmSpecs || '-'})` } < br / >
          厂商： { record.instrmInfo.companyInfo.companyName ? (record.instrmInfo.companyInfo.companyName || '-') : '-' } < /div>
        );
      },
    }, {
      title: '资产编码',
      ...
      render: (text, record, index) => ( < Tooltip placement = "top"
        title = { '删除' } >
        < Icon type = "delete"
        style = {
    ...
    return ( < div style = {
      ...
      < OutStockForm / >
      < EditTable data = { dataOutStock }
      pagination = { false }
      columns = { columns }
      bordered rowSelection = { false }
      page = { page }
      ref = "commonTable"
      onChange = { this.refreshTable.bind(this) }
      scroll = {
        { y: (rightCardHeight - 38 - 34) } }
      /> < /Card> < /div>
    );
  }
}
