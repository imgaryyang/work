...
class OrderList extends Component {
  ...
  makeGroup() {
    ...
    if (selectedItemRowKeys.length < 2) {
      ...
      for (let i = 0; i < orders.length; i++) {
        if (selectedItemRowKeys.contains(orders[i].id)) {
          if (orders[i].usage === '67') {
            notification.warning({
              message: '警告',
              description: '口服药品不能成组！',
            });
            return;
          }
        }
  ...
  render() {
    ...
    const { orders, listIdx, order, totalAmt } = odwsOrder;
    ...
    const columns = [
      { title: '选择',
        ...
        render: (text, record) => {
          return {
            children: (
              <Checkbox onChange={e => this.selectItem(e, text)} />
            ),
            props: {
