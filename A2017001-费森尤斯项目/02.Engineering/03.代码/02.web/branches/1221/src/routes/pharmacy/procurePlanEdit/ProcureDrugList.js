...
import SearchBar from './ProcurePlanEditDrugSearchBar';
...
class ProcureDrugList extends Component { 
  ...
  onSearch(s) {
    const { page } = this.props.procurePlanEdit.drug;
    page.pageNo = 1;
    this.props.dispatch({
      type: 'procurePlanEdit/loadDrug',
  ...
  autoClick() {
    const { newData } = this.props.procurePlanEdit;
  ...
  onConfirm(record) {
    ...
    this.props.dispatch({
      type: 'procurePlanEdit/forAddBuy',
      record,
    });
  }
  ...
  onPageChange(page) {
    ...
    const { query } = this.props.procurePlanEdit.drug;
    this.props.dispatch({
      type: 'procurePlanEdit/loadDrug',
      payload: { page, query },
    });
  ...
  render() {
    const { procurePlanEdit, base } = this.props;
    const { drug } = procurePlanEdit || {};
    const { wsHeight } = base;
...
export default connect(
  ({ procurePlanEdit, base }) => ({ procurePlanEdit, base }),
)(ProcureDrugList);
