...
class AccountList extends Component {
  ...
  componentWillMount() {
	const editUserId = this.props.user4Opt.editUserId;
	console.log('editUserId:',editUserId);
    this.props.dispatch({
      type: 'user4Opt/loadUserAccounts'
    });
  }

  onCancel() {
    const data = this.props.user4Opt.accounts;
    const newData = [];
  ...
  onDelete(record) {
    this.props.dispatch({
      type: 'user4Opt/deleteUserAccount',
      id: record.id,
    });
  }
...
export default connect(({ user4Opt }) => ({ user4Opt }))(AccountList);
