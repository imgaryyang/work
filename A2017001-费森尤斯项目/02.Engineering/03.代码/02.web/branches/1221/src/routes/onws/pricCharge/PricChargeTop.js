...
class PricChargeTop extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    const clear = nextProps.pricCharge.record;
    if (clear === 'clear') {
  ...
  onSelectRegInfo(record) {
    this.props.dispatch({
      type: 'pricCharge/getUserInfoByRegId',
      regId: record.regId,
    });
  }
  ...
  onChangeAmount(e) {
    const stock = this.props.pricCharge.tmpItem.stock;
  ...
  onFeeType(value) {
    this.props.dispatch({
      type: 'pricCharge/updateUserInfo',
      user: { feeType: value },
    });
  }
  ...
  selectDept(value) {
    ...
    this.props.dispatch({
      type: 'pricCharge/finDoctorsByDept',
      deptId: value,
    });
  }
  selectDoc(value, e) {
  ...
  render() {
    ...
    const { userInfo, tmpItem, deptDocList, currentRecipe } = this.props.pricCharge;
...
export default connect(
  ({ pricCharge, utils }) => ({ pricCharge, utils }))(editorForm);
