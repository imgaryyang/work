...
class EditorForm extends Component {
  ...
  handleDeptChange(value) {
    ...
    for (let i = 0; i < dptListData.length; i++) {
      const dptData = dptListData[i];

      if (value === dptData.id) {
        this.props.form.setFieldsValue({
          deptCode: dptData.deptId,
          deptName: dptData.deptName,
        });
        break;
      }
    }
  ...
  render() {
    ...
    const dptOptios = dptListData.map(elm =>
                  ...
                  <FormItem label="所属科室" {...formItemLayout} >
                    {
                      getFieldDecorator('deptId', {
                        ...
                        <Select placeholder="所属科室" style={{ width: '100%' }} onSelect={value => this.handleDeptChange(value)} allowClear>
                          {dptOptios}
                        </Select>,
                      )
                    }
