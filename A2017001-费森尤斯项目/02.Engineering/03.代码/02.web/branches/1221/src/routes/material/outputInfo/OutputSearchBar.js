...
class OutputSearchBar extends Component {
  ...
  handleExport() {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        const { selectedRowKeys } = this.props.outputdetail;
...
const OutputSearchBarForm = Form.create()(OutputSearchBar);
export default connect(({ outputdetail }) => ({ outputdetail }))(OutputSearchBarForm);
