...
class DrugInfoList extends Component {
  ...
  componentWillMount() {
    this.props.dispatch({ type: 'drugInfo/load' });
  }
  ...
  render() {
    const { page, data, dicts, wsHeight, base } = this.props;
    const columns = [
        ...
        render: (text, record) => (
          <span>
            ...
            <Popconfirm placement="left" title={'您确定要删除此项么?'} cancelText={'否'} okText={'是'} onConfirm={this.onDelete.bind(this, record)}>
              <Icon type="delete" className="tableDeleteIcon" style={{ display: (record.hosId === base.user.hosId ? '' : 'none') }} />
            </Popconfirm>
          </span>
        ),
      },
