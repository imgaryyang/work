...
class ProcureSearchBar extends Component {
  ...
  handleExport() {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        const { selectedRowKeys } = this.props.matProcureAuitdSearch;
  ...
  render() {
    const { buyState } = this.props.matProcureAuitdSearch;
...
export default connect(({ matProcureAuitdSearch }) => ({ matProcureAuitdSearch }))(ProcureSearchBarForm);
