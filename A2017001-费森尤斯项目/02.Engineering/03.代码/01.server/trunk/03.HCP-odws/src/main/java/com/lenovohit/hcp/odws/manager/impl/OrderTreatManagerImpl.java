	...
	public String orderBack(List<MedicalOrder> list,HcpUser user) {
		try{
			...
			if(!"".equals(recipeIds)){
				...
				//判断是医生登记还是预约挂号（看是否是同一张发票）是同一张则修改挂号明细的状态apply_state为0，不是则忽略。
				for(int i= 0;i<rList.size();i++){
					OutpatientChargeDetail chargeDetail=rList.get(i);
					RegInfo reg=regInfoManager.get(chargeDetail.getRegId());
					//如果是未发药，则继续执行
					if(!chargeDetail.getApplyState().equals("2")){
						String regJql="from OutpatientChargeDetail where invoiceNo = ? and itemCode in (select f.itemInfo.id from RegFree  f where regLevel = ? and hosId = ? ) ";
						List<OutpatientChargeDetail> regChargeDetail =(List<OutpatientChargeDetail>) outpatientChargeDetailManager.find(regJql, chargeDetail.getInvoiceNo(),reg.getRegLevel(),user.getHosId());
						if(regChargeDetail.size()>0){
							//医生登记，修改apply_state为0
							for(int j= 0;j<regChargeDetail.size();j++){
								regChargeDetail.get(j).setApplyState("0");
							}
							outpatientChargeDetailManager.batchSave(regChargeDetail);
						}
					}
			...
			for(MedicalOrder order :map.get(key)){
				for(OutpatientChargeDetail detail : chargeList){
					if(key.equals(detail.getRecipeId()) && order.getItemId().equals(detail.getItemCode())){
						...
						//生成新收费明细
						if(order.getQty().compareTo(detail.getQty())!=0){
							BeanUtils.copyProperties(detail, newDetail);
							newDetail.setPackQty(order.getPackQty());
							...
							newDetail.setTotCost(newDetail.getSalePrice().multiply(newDetail.getPackQty()).setScale(1, BigDecimal.ROUND_HALF_UP));
						...
						outDetail.setTotCost(outDetail.getSalePrice().multiply(outDetail.getPackQty()).setScale(1, BigDecimal.ROUND_HALF_UP).negate());
						...
						outDetail.setPackQty(outDetail.getPackQty().negate());
			...
			for(MedicalOrder order :map.get(key)){
				for(PhaRecipe detail : recipeList){
					if(key.equals(detail.getRecipeId()) && order.getItemId().equals(detail.getDrugCode())){
						...
						//生成新收费明细
						if(order.getQty().compareTo(detail.getApplyNum())!=0){
							BeanUtils.copyProperties(detail, newDetail);
							newDetail.setApplyNum(order.getPackQty());
