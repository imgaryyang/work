package com.lenovohit.hcp.hrp.manager.print.impl;
...
//入库单打印
@Service("hrpInputPrintDataManager")
public class HrpInputPrintDataManager extends AbstractPrintDataManagerImpl {
	@Autowired
	...
	public PrintData getPrintData(String bizId, HcpUser user) {
		...
		String _sql = ""
				+ "select top 1 ins.IN_TYPE a0,"
				+" ins.IN_TIME a1,"
				+" dept.DEPT_NAME a2,"
				+" ins.IN_OPER a3,"
				+" COLUMN_VAL a4,"
				+" ins.BUY_BILL a5 from  PHA_INPUTINFO ins,b_deptinfo dept,B_DICVALUE dic "
		...
		if (!"undefined".equals(bizId) && bizId != null) {
			...
			for (MatInputInfo _detail : _list) {
				if (_detail.getBuyCost() != null && _detail.getInSum() != null) {
				...
				_outHtml += "<tr>" + "  <td>" + _detail.getTradeName() + "</td>" + "  <td>"
						...
						+ PrintUtil.getNotNull(_detail.getCompany()) + "</td>" + "  <td>"
