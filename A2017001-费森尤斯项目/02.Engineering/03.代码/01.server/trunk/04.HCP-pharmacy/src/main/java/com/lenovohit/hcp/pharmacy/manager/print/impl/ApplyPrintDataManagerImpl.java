...
//请领计划单-药品
@Service("applyPrintDataManagerImpl")
public class ApplyPrintDataManagerImpl extends AbstractPrintDataManagerImpl {
	@Autowired
	...
	public PrintData getPrintData(String bizId, HcpUser user) {
		String hql = "from PhaApplyIn a where a.appBill=?";
		String sql = "select app.UPDATE_OPER a1,"
				...
				+" outI.OUT_OPER a7,"
				+" outI.APP_BILL a8,"
				+ "app.APP_STATE from  pha_applyin app "
				+ "LEFT JOIN b_deptinfo dept1 ON app.DEPT_ID = dept1.ID "
				+ "LEFT JOIN b_deptinfo dept2 ON app.FROM_DEPT_ID = dept2.ID "
				+ "LEFT JOIN pha_outputinfo outI ON app.APP_BILL = outI.APP_BILL"
				+" where app.APP_BILL='" + bizId + "'";
		...
		if (!"undefined".equals(bizId) && bizId != null) {
			...
			String _UPDATE_OPER = "", _DEPT_NAME = "", _APP_OPER, _APP_TIME, _UPDATE_TIME = "", _FROM_DEPT_NAME = "",
					_OUT_OPER = "", _APP_BILL = "",_APP_STATE="";
			...
			if (objList != null && objList.size() > 0) {
				...
				_APP_STATE = (String)obj[8];
				_UPDATE_OPER = (StringUtils.isNotBlank(_APP_STATE)&&"4".equals(_APP_STATE)) ? (String) obj[0] : "";//入库人
			}

			String _outHtml = "<style> table,td,th {border: 0px solid black;border-style: solid;border-collapse: collapse}</style>"
					+ "<table border=\"0\" width=\"100%\">" + "<tr>" + "<th>请领单:</th><td>" + bizId + "</td><th>科室:</th><td>" + _FROM_DEPT_NAME
			...
			for (PhaApplyIn _detail : _list) {
				...
				_outHtml += "<tr>" + "  <td>" + _detail.getTradeName() + "</td>" + "  <td>"
						...
						+ "</td>" + "  <td>" + (_detail.getProduceDate()!=null ? formatDate.format(_detail.getProduceDate()):"") + "</td><td>"
