...
//	出库单打印-药品
@Service("pharmacyOutputPrintDataManagerImpl")
public class PharmacyOutputPrintDataManagerImpl extends AbstractPrintDataManagerImpl {
	@Autowired
	...
	public PrintData getPrintData(String bizId, HcpUser user) {
		...
		String _sql = "select top 1 outs.OUT_TYPE a0," 
				+ " outs.APP_BILL a1," 
				+ " outs.OUT_TIME a2,"
				+ " dept.DEPT_NAME a3," 
				+ " outs.OUT_OPER a4," 
				+ " COLUMN_VAL a5"
				+ " from  PHA_OUTPUTINFO outs,b_deptinfo dept,B_DICVALUE dic"
		...
		if (!"undefined".equals(bizId) && bizId != null) {
			...
			for (PhaOutputInfo _detail : _list) {
				if (_detail.getBuyPrice() != null && _detail.getOutSum() != null&& _detail.getDrugInfo()!=null && _detail.getDrugInfo().getPackQty()!=null) {
				...
				_outHtml += "<tr>" + "  <td>" + _detail.getDrugInfo().getCommonName() + "</td>" + "  <td>"
						...
						+ PrintUtil.getNotNull(_detail.getProducerInfo().getCompanyName()) + "</td>" + "  <td>"
						+ (_detail.getProduceDate() !=null ? PrintUtil.getDate(_detail.getProduceDate()) :"") + "</td><td>" + PrintUtil.getDate(_detail.getValidDate()) + "</td>"
						+ "  <td>" + _detail.getOutSum().divide(new BigDecimal(_detail.getDrugInfo().getPackQty())).longValue() + "</td>" + "  <td>" + PrintUtil.getAmount(_detail.getBuyPrice()) + "</td><td>"
						+ PrintUtil.getAmount(_subtotal1) + "</td><td>" + PrintUtil.getAmount(_detail.getSalePrice()) + "</td><td>" + PrintUtil.getAmount(_subtotal2) + "</td></tr>\n";
