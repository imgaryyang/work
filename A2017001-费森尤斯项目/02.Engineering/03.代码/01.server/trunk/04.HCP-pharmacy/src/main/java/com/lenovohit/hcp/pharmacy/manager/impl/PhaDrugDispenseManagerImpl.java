...
public class PhaDrugDispenseManagerImpl implements PhaDrugDispenseManager {
	...
	public void dispense(String recipeId, HcpUser user) {
		...
		try {
			...
			medicalOrderManager.executeSql("UPDATE PHA_RECIPE SET APPLY_STATE = '2', SENT_TIME = getdate()  WHERE RECIPE_ID = ?", recipeId);
			//step 4 : 更新库存
			//取医嘱信息
			//取药品处方请领信息
			...
			List<PhaRecipe> recipes =  (List<PhaRecipe>)phaRecipeManager.find("from PhaRecipe where recipeId = '" + recipeId + "' and hosId = '" + hosId + "' order by recipeNo");
			for(int i = 0; i < recipes.size(); i++){
				PhaRecipe recipe = recipes.get(i);
				recipe.setSentOper(user.getName());
				phaRecipeManager.save(recipe);
	        }
			...
			for(int i = 0; i < recipes.size(); i++){
				...
				String jql="from PhaDrugPrice where drugInfo.id = ? ";
				List<Object> values=new ArrayList<Object>();
				values.add(recipe.getDrugCode());
				List<PhaDrugPrice> durgPrice= (List<PhaDrugPrice>) phaDrugPriceManager.findByJql(jql, values.toArray());
				outputInfo.setSalePrice(durgPrice.size()!=0 ? durgPrice.get(0).getSalePrice():null); // 零售价
				
				outputInfo.setOutSum(recipe.getMinNum()); // 出库数量
				// 药品信息
