...
public class ProcureInstockPrintDataManagerImpl extends AbstractPrintDataManagerImpl {
	...
	public PrintData getPrintData(String bizId, HcpUser user) {
		...
		String sql = "select dic.COLUMN_VAL,ins.IN_BILL,ins.IN_TIME,dept.DEPT_NAME,ins.IN_OPER,BUY_BILL from  PHA_INPUTINFO ins, b_deptinfo dept,B_DICVALUE dic"
				+ " where ins.IN_TYPE=dic.COLUMN_KEY and ins.DEPT_ID=dept.ID and dic.COLUMN_NAME = 'IN_TYPE' and ins.in_BILL='"
				+ bizId + "'";
		...
		if (!"undefined".equals(bizId) && bizId != null) {
			...
			for (PhaInputInfo _detail : _list) {
				...
				_outHtml += "<tr>" + "  <td>" + _detail.getTradeName() + "</td>" + "  <td>"
						...
						+ PrintUtil.getNotNull(_detail.getBatchNo()) + "</td>" + "  <td>" + (_detail.getCompanyInfo()!=null ? _detail.getCompanyInfo().getCompanyName() : "")
						+ "</td>" + "  <td>" + (_detail.getProduceDate()!=null?formatDate.format(_detail.getProduceDate()) : "") + "</td><td>"
						+ formatDate.format(_detail.getValidDate()) + "</td>" + "  <td>" + (_detail.getInSum()!=null&&_detail.getDrugInfo()!=null&&_detail.getDrugInfo().getPackQty()!=null ? _detail.getInSum().divide(new BigDecimal(_detail.getDrugInfo().getPackQty())) : '0') + "</td>"
