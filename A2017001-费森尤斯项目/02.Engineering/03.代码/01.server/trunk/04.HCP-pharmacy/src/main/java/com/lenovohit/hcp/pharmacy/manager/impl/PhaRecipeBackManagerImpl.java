...
public class PhaRecipeBackManagerImpl implements PhaRecipeBackManager{
	...
	public String phaRecipe(String regId, HcpUser user) {
		try {
		    ...
		    //修改收费明细状态
		    StringBuilder s = new StringBuilder("from OutpatientChargeDetail re where re.regId = ? and re.applyState = ? ");
		    values.clear();
		    values.add(regId);
		    values.add(PhaRecipe.APPLY_STATE_APPLY);
		    List<OutpatientChargeDetail> oList = outpatientChargeDetailManager.find(s.toString(), values.toArray());
		    
			//判断是医生登记还是预约挂号（看是否是同一张发票）是同一张则修改挂号明细的状态apply_state为0，不是则忽略。
			for(int i= 0;i<oList.size();i++){
				OutpatientChargeDetail chargeDetail=oList.get(i);
			...
			e.printStackTrace();
			System.out.println(e.getMessage());
			throw new RuntimeException("退药失败！");
		}
