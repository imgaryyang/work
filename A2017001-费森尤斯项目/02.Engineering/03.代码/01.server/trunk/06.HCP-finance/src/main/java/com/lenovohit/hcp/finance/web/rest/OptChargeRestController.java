	...
	public Result submitCharge(@RequestBody String data) {
		...
		if (chargeList != null && chargeList.size() > 0) {
			try {
				...
				return ResultUtils.renderFailureResult("收费失败：" + e.getMessage());
			}
		} else {
			return ResultUtils.renderFailureResult("未查询到对应收费明细！");
		}
	}
	/**    
	 * 功能描述：划价保存
	...
	public Result submitItemCharge(@RequestBody String data) {
		...
		if (chargeList != null && chargeList.size() > 0) {
			for(int i=0;i<chargeList.size();i++){
				...
				detail.setItemCode(itemInfo.getItemId());		//项目编码
	...
	public Result saveChargeToTemplate(@RequestBody String data) {
		...
		if (itemInfoList != null && itemInfoList.size() > 0) {
			...
			for(int i=0;i<itemInfoList.size();i++){
				...
				detail.setItemCode(info.getItemId());
				...
				if(info.getItemId()!=null){
					PhaDrugInfo model= phaDrugInfoManager.get(info.getItemId());
					if(model!=null){
