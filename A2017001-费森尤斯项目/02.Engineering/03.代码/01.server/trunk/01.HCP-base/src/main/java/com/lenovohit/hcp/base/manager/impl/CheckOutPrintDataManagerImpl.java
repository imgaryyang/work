...
public class CheckOutPrintDataManagerImpl extends AbstractPrintDataManagerImpl {
	...
	public PrintData getPrintData(String bizId, HcpUser user) {
		...
		PrintData data = buildPrintData(balance,user);
		return data;
	}

	private PrintData buildPrintData(OperBalance balance, HcpUser user) {
		...
		List<InnerData> payWays = getPayWays(balance.getId(),user);
		List<InnerData> feeTypes = getFeeTypes(balance.getId(),user);
	...
	private List<InnerData> getPayWays(String balanceId,HcpUser user) {
		...
		for (InvoiceInfo info : infos) {
			...
			for (PayWay payWay : payWays) {
				...
				innerData.setT1(getDictName("PAY_MODE", payWay.getPayWay(),user.getHosId()));
	...
	private List<InnerData> getFeeTypes(String balanceId,HcpUser user) {
		List<InvoiceInfo> infos = invoiceInfoManager.findByProp("balanceId", balanceId);
		...
		for (Map.Entry<String, BigDecimal> entry : map.entrySet()) {
			...
			data.setT1(getDictName("FEE_CODE", entry.getKey(),user.getHosId()));
