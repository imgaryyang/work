...
public class ClinicChargeDetailsPrintDataManagerImpl extends AbstractPrintDataManagerImpl {
	...
	public PrintData getPrintData(String bizId, HcpUser user) {
		...
		BigDecimal acturalFee=new BigDecimal(0);
		for (OutpatientChargeDetail o : resultDetails) {
			InnerData innerData = new InnerData();
			innerData.setT1(getDictName("DRUG_TYPE", o.getDrugFlag(),user.getHosId()));
			...
			totalFee=totalFee.add(o.getTotCost());
			acturalFee=acturalFee.add(o.getTotCost());
		...
		data.setT2(getDictName("FEE_TYPE", regInfo.getFeeType(),user.getHosId()));
		...
		data.setMap(map);
		data.setT10(formatBigDecimal(totalFee));
		data.setT11(formatBigDecimal(acturalFee));
		return data;
	}
