...
public class HisRegisterManagerImpl implements RegisterManager {
	...
	public HisOrder registerToPay(RegInfo info) {
		...
		HisOrder order = null;
		try {
			order = hisInterChargeManager.handleChargeToPay(info.getCreateOper(), getTotalFee(info.getRegLevel()),
	...
	private List<OutpatientChargeDetail> buildAndSaveChargeDetail(RegInfo info) {
		List<RegFree> result = regFreeManager.findByProp("regLevel", info.getRegLevel());
	...
	private BigDecimal getTotalFee(String regLevel) {
		List<RegFree> result = regFreeManager.findByProp("regLevel", regLevel);
