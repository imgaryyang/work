...
/**
 * @author duanyanshan
 * @date 2017年9月30日 上午11:49:18
 */
public class SendCheckJob extends BaseJob{
	
	@Autowired
	private PatLisManager patLisManagerImp;
	/* (non-Javadoc)
	 * @see com.lenovohit.hcp.odws.job.BaseJob#doJob()
	 */
	@Override
	protected void doJob() {
		System.out.println("==================获取当天送检结果开始===================");
		//获取当前日期 yyyy-MM-dd
		...
		patLisManagerImp.saveResultList(mapList);
		
		System.out.println("==================获取当天送检结果结束===================");
	 ...
	 * @return
	 */
	private String getSoapRequest(String hospitalName, String password, String startDate, String endDate) {
	 ...
	 * @throws Exception
	 */
	private String getSoapInputStream(String hospitalName, String password, String startDate, String endDate)
			throws Exception {
		try {
	 ...
	 * @return 字符串 用#分割
	 */
	private List<Map<String, Object>> getDataFromXml(String hospitalName, String password, String startDate,
			String endDate) {
						...
						while (rt.hasNext()) {
							...
							while (returnIt.hasNext()) {
								...
								while (itemIt.hasNext()) {
									...
									String shortname = item.elementTextTrim("SHORTNAME");
									String units = item.elementTextTrim("UNITS");
									String displowhigh = item.elementTextTrim("DISPLOWHIGH");
									map.put("displowhigh", displowhigh);
									String displowhighF = item.elementTextTrim("DISPLOWHIGH_F");
									map.put("displowhighF", displowhighF);
									String displowhighM = item.elementTextTrim("DISPLOWHIGH_M");
									map.put("displowhighM", displowhighM);
