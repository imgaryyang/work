...
public class TreatRecordPrintDataManagerImpl extends AbstractPrintDataManagerImpl {
	...
	public PrintData getPrintData(String bizId, HcpUser user) {
		...
		PrintData data = buildBasePrintData(info, diagnose,user);
	...
	private PrintData buildBasePrintData(RegInfo info, Diagnose diagnose, HcpUser user) {
		PrintData data = new PrintData();
		data.setT1(info.getPatient().getName());
		data.setT2(getDictName("SEX", info.getPatient().getSex(),user.getHosId()));
		data.setT3(getDictName("FEE_TYPE", info.getFeeType(),user.getHosId()));
