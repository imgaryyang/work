...
import com.lenovohit.hcp.finance.model.PayWayContent;
...
public class CheckOutStatisticsManagerImpl implements CheckOutStatisticsManager {
	 ...
	 * 功能描述：按照支付方式统计
	 *@param balances 结账集合
	 *@return   List<PayWayStatisticsDto> 数据汇总集合    
	 *@author 
	 *@date 2017年6月20日             
	*/
	public List<PayWayStatisticsDto> listPayWayStatistics(List<OperBalance> balances) {
		...
		for (OperBalance o : balances) {
			...
			// 检索本次结账的所有收费信息
			List<InvoiceInfo> infos = invoiceInfoManager.findByProp("balanceId", o.getId());
			for (InvoiceInfo info : infos) {//循环本次结账单的所有收费信息
				// 本次发票的所有收费项
				List<PayWay> payWays = payWayManager.findByProp("invoiceNo", info.getInvoiceNo());
				for (PayWay payWay : payWays) {//计算收费和
					BigDecimal amt = amtMap.get(payWay.getPayWay());
					if (StringUtils.isBlank(amt)) {
						amtMap.put(payWay.getPayWay(), payWay.getPayCost());
					} else {
						amtMap.put(payWay.getPayWay(), amt.add(payWay.getPayCost()));
					}
				}
			}
			// 计算各收费项和
			buildPayWayDto(payWayStatisticsDto, amtMap);
		}
		for (Map.Entry<String, PayWayStatisticsDto> entry : resultMap.entrySet()) {// 将结果封装成数组
	...
	*/
	private void countOneSumAndBuildTotalDto(List<PayWayStatisticsDto> result, PayWayStatisticsDto sumDto) {
		for (PayWayStatisticsDto dto : result) {//计算各项收费的合计
			BigDecimal oneSum = new BigDecimal("0");
	 ...
	 *@date 2017年6月20日             
	*/
	public List<Map<String,String>> getPayWay(String hosId,String columnName,Boolean stop){
		List<Map<String,String>> payWayList = new ArrayList<>();
		StringBuilder jql = new StringBuilder("from Dictionary where 1=1 ");
		List<Object> values = new ArrayList<Object>();
		jql.append("and hosId = ? and columnName = ? ");
		values.add(hosId);
		...
		if(dicList!=null && dicList.size()>0){
			Map<String,Map<String,String>> map = PayWayContent.payWayMap;
			if(map!=null){
				for(Dictionary dic: dicList){//查询出需要显示的支付方式
					if(map.get(dic.getColumnKey())!=null){
						payWayList.add(map.get(dic.getColumnKey()));
					}
				}
			}
		}
