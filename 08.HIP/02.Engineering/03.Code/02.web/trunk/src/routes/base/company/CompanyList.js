...
class CompanyList extends Component {
  ...
  render() {
    ...
    // const { dicts } = utils;
    const { wsHeight } = base;
    const { page, data, chanel } = company;
    ...
    const columns = [
      /* {
        ...
        title: '证照时间',
        dataIndex: 'licenseSdate',
        key: 'licenseSdate',
        width: 130,
        render: (value, record) => {
          return (
            <div>
              {`${value ? value : '-'}`} ~ {`${record.licenseEdate ? record.licenseEdate : '-'}`}
            </div>
          );
        },
      }, {
        ...
        render: (text, record) => (
          <span style={
            /* { display: ((chanel === 'group' && record.level === 0) || (chanel !== 'group' && record.level !== 0) ? '' : 'none') }} */
