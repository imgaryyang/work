...
class DictRadioGroup extends Component {
  ...
  render() {
    const {
      columnName, utils, buttonMode, buttonStyle, buttonClass,
    } = this.props;
    ...
    for (let i = 0; dictItems && i < dictItems.length; i += 1) {
      ...
      if (!buttonMode) {
        options.push(<Radio key={item.columnKey + i} value={valueField} originData={item} >
          {item.columnVal}
        </Radio>);
      } else {
        options.push(<RadioButton key={item.columnKey + i} value={valueField} originData={item} style={buttonStyle} className={buttonClass} >
          {item.columnVal}
        </RadioButton>);
      }
    }
