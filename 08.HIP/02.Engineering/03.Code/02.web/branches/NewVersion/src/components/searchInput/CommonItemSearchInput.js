...
class CommonItemSearchInput extends Component {
  ...
  render() {
    ...
    // console.log('before:', commonItems);
    for(let item of commonItems) {
      // console.log(item);
      if (item.exeDept) item.deptName = utils.depts.disDeptName(utils.deptsIdx, item.exeDept);
    }
    // console.log('after:', commonItems);
    ...
    const optionRender = (option) => {
      return itemType === '0' || drugFlag === '3' ? (
        ...
        <div className={`${styles.searchInput} ${styles.commonItemDrug}`} >
          <span className={styles.itemName} >{option.itemName}<br/>{option.itemSpecs}</span> {/* 项目名称/药品名称 */}
          {/* <span className={styles.itemSpecs} >{option.itemSpecs}</span>*/} {/* 规格 */}
          <span className={styles.itemUnit} >{option.itemUnit}</span> {/* 单位 */}
          <span className={styles.salePrice} >{option.salePrice ? option.salePrice.formatMoney(2) : ''}</span> {/* 售价 */}
          <span className={styles.manufacturer} >{option.companyName}</span> {/* 厂家 */}
    ...
    return (
      <SearchInput
        ...
        valueRender={option => option ? `${option.itemName}（${option.itemSpecs || '-'}）${option.deptName ? option.deptName : ''}` : ''}
        fetching={commonItemsFetching}
        dropdownStyle={{ width: `${itemType === '0' || drugFlag === '3' ? 430 : 630}px` }}
        // value={value}
