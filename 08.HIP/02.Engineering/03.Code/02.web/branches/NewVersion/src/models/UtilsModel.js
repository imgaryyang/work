...
import {
  ...
  loadPatientInfoByPatientId, loadPatientInfoByIdNo, loadCompanies,
...
export default {
  ...
  effects: {
    ...
    *reloadDepts({ payload }, { put, call }) {
      ...
      const { query } = (payload || {});
      const { depts, deptsIdx } = yield call(reloadDepts, query);
      if (depts && deptsIdx) {
        yield put({
          type: 'setState',
          payload: {
    ...
    *loadCommonItem({ payload }, { call, put }) {
      const {
        searchCode, type, drugFlag, deptId,
      } = payload;
      // if (searchCode) {
      yield put({
        type: 'setState',
      ...
      const { data } = yield call(loadItemInfo, {
        searchCode, type, drugFlag, deptId,
      });
    ...
    *loadDiagnosis({ payload }, { call, put }) {
      ...
      const { data } = yield call(loadDiagnosis, { searchCode });
      if (data && data.success) {
        const { result } = data;
        const datatmp = result || [];
    ...
    *loadFreqs({ payload }, { call, put }) {
      ...
      const { data } = yield call(loadFreqs, { searchCode });
      if (data && data.success) {
        const { result } = data;
        const datatmp = result || [];
    ...
    *loadRegInfo({ payload }, { call, put }) {
      ...
      if (searchCode) {
        ...
        const { data } = yield call(loadRegInfo, { searchCode });
        if (data && data.success) {
          const { result } = data;
          const datatmp = result || [];
          // console.log('UtilsModel loadfreq() receive datatmp:', datatmp);
    ...
    *loadDrugInfo({ payload }, { call, put }) {
      ...
      if (searchCode) {
        ...
        const { data } = yield call(loadDrugInfo, { searchCode });
        if (data && data.success) {
          const { result } = data;
          const datatmp = result || [];
          yield put({
    ...
    *loadMaterialCompany({ payload }, { call, put }) {
      ...
      const { data } = yield call(loadMaterialCompany, { searchCode, ...rest });
      if (data && data.success) {
        const { result } = data;
        const datatmp = result || [];
        // console.log('UtilsModel loadfreq() receive datatmp:', datatmp);
    ...
    *loadCompanies({ payload }, { call, put }) {
      ...
      const { data } = yield call(loadCompanies, { searchCode, ...rest });
      if (data && data.success) {
        const { result } = data;
        const datatmp = result || [];
        // console.log('UtilsModel loadfreq() receive datatmp:', datatmp);
    ...
    *loadMaterialCertificate({ payload }, { call, put }) {
      ...
      const { data } = yield call(loadMaterialCertificate, { searchCode, ...rest });
      if (data && data.success) {
        const { result } = data;
        const datatmp = result || [];
        // console.log('UtilsModel loadfreq() receive datatmp:', datatmp);
    ...
    *loadMaterials({ payload }, { call, put }) {
      ...
      const { data } = yield call(loadMaterials, { searchCode, ...rest });
      if (data && data.success) {
        const { result } = data;
        const datatmp = result || [];
        // console.log('UtilsModel loadfreq() receive datatmp:', datatmp);
    ...
    *loadPatientInfoByPatientId({ patientId }, { call, put }) {
      const { data } = yield call(loadPatientInfoByPatientId, patientId);
      if (data && data.success) {
        const { result } = data;
        yield put({
          type: 'setState',
     ...
     * 根据身份证号获取患者信息
     */
    *loadPatientInfoByIdNo({ idNo }, { call, put }) {
      const { data } = yield call(loadPatientInfoByIdNo, idNo);
      if (data && data.success) {
        const { result } = data;
        yield put({
          type: 'setState',
