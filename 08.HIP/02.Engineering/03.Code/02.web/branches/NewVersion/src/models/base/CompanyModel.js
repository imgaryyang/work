...
export default {
  ...
  effects: {
    ...
    *load({ payload }, { select, call, put }) {
      const chanel = yield select(state => state.company.chanel);
      ...
      // 调用载入数据
      const { data } = yield call(companyService.loadPage, start, pageSize, query || {}, chanel);
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(({ pathname }) => {
        if (pathname === '/base/company/group' ||
          pathname === '/operation/company/group' ||
          pathname === '/pharmacy/settings/company/drug' ||
          pathname === '/operation/company/operate' ||
          pathname === '/material/company/material' ||
          pathname === '/hrp/company/hrp') {
          dispatch({
