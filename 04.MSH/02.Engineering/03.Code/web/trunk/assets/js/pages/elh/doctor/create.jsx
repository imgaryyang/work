...
class CreateForm extends Component{
	...
	save(data){
		...
		let fetch = Ajax.post('api/elh/doctor/create', doctor, {catch: 3600,dataType:'json'});
		fetch.then(res => {
			if(res.success){
	...
	render () {
		...
		return (
			<Form horizontal>
		    	...
		    	<FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 8 }} label="上传头像" >
				...
				<FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 8 }} label="科室选择" >
			     <div>
			        <Select placeholder="科室类型" style={{ width: 100 }} onChange={scope.handleDptChange.bind(scope)}>
			          {dptOptions}
			        </Select>
	        	...
	        	<FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 6 }} label="职称" >
	        		<Input placeholder="职称" {...getFieldProps('jobTitle')} />
	        	</FormItem>
	        	...
	        	<FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 8 }} label="出生日期" >
	        		<DatePicker  format="yyyy-MM-dd" {...getFieldProps('birthDay')}/>
	        	</FormItem>
	        	<FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 8 }} label="从医日期" >
	        		<DatePicker format="yyyy-MM-dd" {...getFieldProps('entryDate')}/>
	        	</FormItem>
		    	<FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 8 }} label="出诊时间" >
			     <div>
			     <Select multiple style={{ width: 300 }} placeholder="星期" {...getFieldProps('clinic')}>
        		...
        		<Input type="textarea" rows={4} {...getFieldProps('speciality')}/>
        		</FormItem>
