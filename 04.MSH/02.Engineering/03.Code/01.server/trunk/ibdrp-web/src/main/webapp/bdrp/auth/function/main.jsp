...
<script type="text/javascript">
	$(document).ready(function(){
		...
		//	iconCls : "icon-forward",	// 左上边出现一个空白图片，这样是标题文字有一一格，为了样式好看些
			...
			url : '${ctx}/bdrp/auth/function/page',
			...
			columns : [[
				...
				{field : 'desc', title : '描述', width : 280},
				{field : 'operate', title : '添加', width : 30},
				{field : 'change', title : '转换', width : 27}
			]],
		...
		$("#accessName").blur(function(){
			...
			if(accessName == null || accessName.trim() == ""){
				$("#accessName1").show();
				$("#accessName2").hide();
				$("#accessName").val("");	// 将输入框中的空格去掉
				return ;
			}else{
				$("#accessName1").hide();
			}
	...
	// 翻页功能
	function pagerFilter(data) {3
		var method = $("#method").val();	// 查询的条件
		var context = $("#context").val();	// 条件的具体值
		$('#tree').treegrid('options').url = '${ctx}/bdrp/auth/function/page';
	...
	function loadChildren(row) {
		if (row) {
			$('#tree').treegrid('options').url = '${ctx}/bdrp/auth/function/page?parentId=' + row.id;
		}
	}
	...
	// 新增功能----跳转到新增页面
	function doAdd(){
		$('.one-center-panel').load('${ctx}/bdrp/auth/function/add.jsp');
	...
	function doClickCell(field, row){
		...
		if(field == "operate"){	// 如果点击的是添加那列
			...
			$('.one-center-panel').load('${ctx}/bdrp/auth/function/add.jsp?parentId=' + id);
	...
	function doDblClickRow(row){
		var id = row.id;
		$('.one-center-panel').load('${ctx}/bdrp/auth/function/edit/' + id);
	...
	function doDelete(){
		...
		$.messager.confirm("确认删除", "你确定要删除选中项吗？", function(flag){
			if(flag == true){
				// 得到所有选中的ID值，并拼装字符串
				var id = arr[0].id;
				var url = "${ctx}/bdrp/auth/function/remove/" + id;				
				// Ajax异步提交
				$.ajax({
					type : "DELETE",
					url : url,
	...
	function toSave(){
		...
		$.ajax({
			type : "POST",
			url : "${ctx}/bdrp/auth/function/existNameAuthzFunction.do",
			data : "name="+accessName+"&fid="+fid,
			success : function(msg){
				if(msg == "success"){
					$("#accessName2").hide();
					...
					$.ajax({
						type : "POST",
						url : "${ctx}/bdrp/auth/function/existNameMenu.do",
						data : "code="+menuCode,
						success : function(msg){
							if(msg == "success"){
								$("#menuCode2").hide();
									...
									$("#converToMenu").form({
										...
										success : function(msg){
											if(msg == "success"){
												...
												$('.one-center-panel').load('${ctx}/bdrp/auth/function/functionMain.ihp');
											}
											if(msg == "exits1"){
												$("#accessName2").show();
											}
					...
					<table  width=100% border=0 cellspacing=0 cellpadding=0>
						...
						<tr class=f-tab-row>
							...
							<td class=f-tab-col-input-nowidth width="20%">
								...
								<input id="accessName" type="text" class=c-ff-input name="accessName">
								<p id="accessName1" style="display : none;"><font color="red">* 服务名必须填写</font></p>
								<p id="accessName2" style="display:none;"><font color="red">* 该服务名已存在,请重新填写!</font></p>
							</td>
						</tr>
						<tr class=f-tab-row>
							...
							<td class=f-tab-col-input-nowidth width="20%">
								<select id="menuParentId" name="menuParentId" class="easyui-combotree"
										data-options="width:226, height:32, url:'${ctx}/bdrp/auth/function/combotreeMenu.do'">
